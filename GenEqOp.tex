...
For $G$-operads $\V\Op^G$, we will require the full strength and subtly of Proposition \ref{AUTTCOFPUSH PROP}.
...
\begin{definition}
  ...
  \item \textit{$\F$-level-cofibration} if $f(n)$ is a cofibration in $\V^{G\times \Sigma_n}_{\F_n}$ for all $n$.
  \end{itemize}
In particular, $\O$ is \textit{$\F$-cofibrant} if $\varnothing \to \O$ is an $\F$-cofibration, where $\varnothing$ is the initial object of $\V$.
\end{definition}

\begin{definition}
  The \textit{$\F$-model structure} is the unique model structure on $\V\Op^G$, if it exists, with (co)fibrations and weak equivalences as just defined above. 
\end{definition}
...
\begin{corollary}
  ...
  For $\V = \Top$ and $\F$ any weak indexing system, there exists an operad $N\F$ such that $N\F(n)^\Gamma \simeq *$ if $\Gamma \in \F(n)$, and is empty otherwise. In particular, $\mathrm{Ho}(N_\infty$-$\Op) \to \mathbb I$ is an equivalence of categories.%, proving Conjecture \ref{N_INFINITY_REALIZATION_CONJ}.
\end{corollary}
\begin{proof}
...
\todo[inline]{come back: update notations}
\[
...
\begin{corollary}[{cf. \cite[Corollary 5.2, Proposition 4.3]{BM03}}]
...
\todo[inline]{Luis: don't we need that $\Sigma_{\F}$ is a \textit{sieve}? I.e. that it has restrictions?}
...
<<<<<<< HEAD
Modeled on the proof of \cite[Theorem 2.10]{Stephan16}, we have the following result.
=======
