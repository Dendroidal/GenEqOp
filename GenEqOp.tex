...
consists of sets or spaces $\O(n)$ of ``$n$-ary operations'' carrying a $\Sigma_n$-action recording ``reordering the inputs of the operations'', and a suitable notion of ``composition of operations''.
The purpose of the theory is then the study of 
``objects $X$ with operations indexed by $\O$'',
referred to as \textit{algebras}, with the notions of monoid, commutative monoid, Lie algebra, algebra with a module, loops spaces, and more,
...
which are more easily described in terms of the associated $H$-sets $X$ produced from the graph subgroups,
...
(modernized and reproved by Piacenza in \cite{Pia91})
...
by those fixed-point spaces which appear in 
...
seen earlier as the $H$-sets in (\ref{GENGENMULT EQ}),
...
give rise to a lattice of sieves 
...
\coprod_{T \in 
\mathsf{Iso}(C \downarrow_{\mathsf{r}} \Omega_G)}
\left(
\prod_{v \in V_G(T)}
\left(
