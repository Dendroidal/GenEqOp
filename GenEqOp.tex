...
referred to as \textit{algebras}, with the notions of monoid, commutative monoid, Lie algebra, algebra with a module, loop spaces, and more,
...
but rather a genuine equivariant operad.
...
In upcoming work we intend to connect the two papers by
...
This paper roughly consists of two major halves, 
with 
...
\S \ref{WREATH SEC}, which we use when defining
...
is that such substitution data are encoded by special maps of $G$-trees that we call planar tall maps. The bulk of the section is spent defining these terms, culminating in the concept of planar strings in \S \ref{PLANARSTRING SEC}, which encoded iterated substitution.

\S \ref{GENUINE_OP_MONAD_SECTION} then provides the formal definition of the category 
$\mathsf{Op}_G(\mathcal{V})$
of genuine equivariant operads in a 
two step process in 
\S \ref{MONSPAN SEC}, \S \ref{FGMON SEC}.
