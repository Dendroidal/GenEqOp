...
We now focus on a particular construction in the category $\V\Op$ of $\V$-enriched operads. In particular, given any $\P\in\V\Op$, and $X,Y\in \V\Sym$, consider the following pushout.
\[
\begin{tikzcd}
...
\begin{definition}
  We call both the map $\P \to \P[u]$ and the operad $\P[u]$ itself a \textit{cellular extension} of $\P$.   
\end{definition}

Cellular extensions in various forms have been extensively studied, as they are of paramount importance to lifting model structres across free-forgetful adjunctions (as we will exploit in the next section). As has been done previously (see, for example, \cite{SS00,Spitz01, BM03, White14a, White14b, WY15, Per16, HP15, Har09, Har10}), we will construct a filtration of the map $\P \to \P[u]$ in the underlying category of sequences.

As we showed above in Example \todo{doesn't exist yet}, we have that $\P[u] \simeq \Lan_{\Omega_{G,e}^{op}}N^e$. Thus, we are able to decompose $\P[u]$ by a filtration of $\Omega_{G,e}$.

\subsection{Stuff Which Probably Will Be Included in an Above Section}



We recall a generator-relation description of the category $\Omega_G^e$. Objects are $(X,Y; \P$)-alternating $G$-trees: odd or \textit{active} nodes are $\P$-labeled, and even or \textit{passive} nodes are either $X$- or $Y$-labeled, such that $G$ acts by labeled-preserving isomorphisms of the underlying $G$-tree. Maps come in three generating flavors:
\begin{enumerate}
\item quotient maps which preserve the labelings of the nodes;
\item ``passive relabeling'' maps $\partial_Y = \partial_{Y}^{(v_i)}$, which are the identity on the underlying tree, but change the labelings on the vertices $v_1,\ldots, v_n$ from $Y$ to $X$;
\item ``active substitution'' maps $\partial_\P = \partial_\P^{(v_i, S^{v_i})}$, which are underlying inner face maps, that take an active ($\P$-labeled) nodes $T_{v_i}$ and substitute them with $(X;\P)$-alternating $G$-trees $S^{v_i}$ of height 3 such that we have a (unique) planar-tall maps $T_{v_i} \to S^{v_i}$ on the underlying $G$-trees.
\end{enumerate}
Relations are generated by the following:
\begin{enumerate}
\item any two passive relabeling maps commute;
\item compositions of active substitutions are identified if they induce the same underlying map in $\Omega_G$ after forgetting labels; and
\item if $\partial_Y$ and $\partial_\P$ affect different nodes of $T$, then the composites $\partial_Y\partial_\P$ and $\partial_\P\partial_Y$ exist and are equal. 
\end{enumerate}

\begin{example}
  The following is a composite of passive relabelings followed by some active substitutions:
\[
  \begin{tikzpicture}
    [grow=up, level distance = .8cm, auto, every node/.style = {font = \small}, dummy/.style={circle,draw,inner sep=0.4mm, minimum size=2.25mm}]
    \tikzstyle{level 2}=[sibling distance=3.5em]
    \tikzstyle{level 3}=[sibling distance=2em]
    \tikzstyle{level 4}=[sibling distance=1em]
    \node{}
    child{node [dummy, fill=black] {}%
      child{node [dummy] {\tiny $0$}%
        child{node [dummy, fill=black] {}%
          child{node [dummy] {\tiny $0$}}
          child{}
        }
    ...
    \node at (7.5,1.6){$\longto$};
    \tikzstyle{level 2}=[sibling distance=4em]
    \tikzstyle{level 3}=[sibling distance=8em]
    \tikzstyle{level 4}=[sibling distance=3em]
    ...
    child{node [dummy, fill=black] {}
      child{node [dummy] {\tiny $0$}
        child{node [dummy, fill=black] {}
          child[sibling distance=5em]{node [dummy] {\tiny $0$}
            child{node [dummy, fill=black] {}
              child{node [dummy] {\tiny $1$}}
              child{}
            }
          ...
          child[sibling distance=5em]{node [dummy] {\tiny $1$}
            child{node [dummy, fill=black] {}
              child{node [dummy] {\tiny $0$}
                child{node [dummy, fill=black] {}}
              }
...
\begin{remark}
  \label{OMEGA_E_REMARKS}
  Some remarks:
  \begin{enumerate}
  \item Maps in $\Omega_G^e$ have unique decompositions of the form $f = q\partial_Y\partial_\P$; in particular, for any composite $f = \partial q$, we have $f = q' \partial'$, where $\partial$ and $\partial'$ are composites of passive relabeling and active substitutions, and $q$ and $q'$ are quotients.
  \item Equivalently (and as in $\Omega_G$), for any cartesian map $q: S \to T$ and any non-cartesian map $f:T \to T'$, there must exist a factorization
    \[
    \begin{tikzcd}
      S \arrow[d, "q"'] \arrow[r, dotted, "f'"'] & S' \arrow[d, dotted, "q'"]\\
      T \arrow[r, "f"] & T'
    \end{tikzcd}
    \]
    where $q'$ is also Cartesian, and $f'$ is non-Cartesian.
  \item $\Omega_{G,-1}^{(2,1)}$ naturally embeds into $\Omega_G^e$ as a wide subcategory.
  \end{enumerate}
\end{remark}

\begin{definition}
  The nerve-evaluation map $N_{(X,Y),\P}:\Omega_{G,-1}^{(2,1)} \to \V$ extends to $\Omega_G^e$. It sends passive relabeling maps $\partial_Y^{v}$ to an application of the map $u(T_v): X(T_v) \to Y(T_v)$ in the $v$-th component of the indexed tensor product; on active substitution maps $\partial_\P^{v, S^v}$, it is given by first an application of $h(T_V): X(T_v) \to \P(T_v)$, followed by the operad structure map 'collapsing' newly adjacent active nodes.
\end{definition}

\begin{definition}
  Given $T\in \Omega_G^e$, let $V_\P(T)$, $V_X(T)$, and $V_Y(T)$ be the $G$-sets of vertices which are $\P$-, $X$-, and $Y$-labeled, respectively. Similarly, let $V_{G,\P}(T) = V_\P(T)/G$, $V_{G,X}(T) = V_X(T)/G$, and $V_{G,Y}(T)=V_Y(T)/G$ be the sets of $G$-vertices which are correctly labeled. 

Further, let $|T| = |T|_X + |T|_Y$, where $|T|_X$ is the ``number of $X$-labeled nodes in any single component of $T$'', and similarly for $|T|_Y$. Rigorously, we have
\[
|T|_X = \dfrac{|V_X(T)|}{|G.r|} = \sum_{G.v\in V_{G,X}(T)}\dfrac{|G.v|}{|G.r|}
\]
where $G.r$ is the root $G$-set of $T$; and similarly for $|T|_Y$. 
\end{definition}

\begin{definition}
  Given a $(\P; X, Y)$-alternating $G$-tree $T$, let $T^{\wedge}_{\P}$ be the alternating tree with $|T|_X=0$ (so $T^\wedge_\P$ is $(\P; Y)$-alternating) created from $T$ by
  \begin{enumerate}
  \item first relabeling all $X$-nodes $\P$ (yielding a tree in $\lambda^3\mathbb T_0$); then
  \item collapsing all connected components of $\P$-labeled nodes. 
  \end{enumerate}
There is a unique planar-tall map $\partial_\P: T^{\wedge}_\P \to T$, and in fact this factors through all maps $\partial_\P: S \to T$.
\end{definition}

\begin{example}
  We demonstrate this functor on an example element in $\lambda_1^2\mathbb T_{G}$ below. The source object $T$ is on the left; the (non-alternating) tree $T^{\wedge}_\P$ which has relabeled all $X$-nodes as $\P$-nodes is in the middle; and the right tree has collapsed all the connected active components. As before, black nodes are active, while white are passive, and we have labeled the interior of the node to denote which kind of passive it is.
\[
\begin{tikzpicture}
  [grow=up, level distance = .8cm, auto, every node/.style={font=\small}, dummy/.style={circle,draw,inner sep=0.4mm, minimum size=2.25mm}]
  ...
  child{node [dummy, fill=black] {}%
      ...
      child{node [dummy, fill=black] {}%
        child{node [dummy] {\tiny 0}%
          child{node [dummy, fill=black] {}%
            ...
            child{edge from parent node [left]{$c$}} 
          }
        }
        child{node [dummy] {\tiny 1}%
          child{node [dummy, fill=black] {}%
    ...
    child{node [dummy] {\tiny 1}%
      child{node [dummy, fill=black] {}%
        child{node [dummy] {\tiny $0$}%
          child{node [dummy, fill=black] {}%
            child{edge from parent node [swap] {$y$}}
          }
          child{node [dummy, fill=black] {}%
            child{edge from parent node {$x$}}
          }
        }
        edge from parent node {$z$}
      }
      edge from parent node {$a$}
    }
  };
  ...
  child{node [dummy,fill=black] {}%
    ...
    child[sibling distance=.5em, level distance = 1.6cm]{edge from parent node [swap, very near end]{$d$}}
    child[sibling distance=.5em, level distance = 1.6cm]{edge from parent node [very near end]{$c$}}
    child{node [dummy] {\tiny 1}%
      child{node [dummy, fill=black] {}%
        child{}
      }
...
\begin{remark}
  The categories $\Omega_G^e[k]$ and $\Omega_G^e[k,-]$ have only very limited morphisms, as there cannot be any ``active substitutions''. Thus, any map $S \to T$ in these categories just changes some $Y$-labelings into $X$-labelings, while the underlying $(1,1)$-alternating tree remains fixed (where here the one passive colour encompasses both $Y$- and $X$-labels).
\end{remark}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \end{tikzcd}
  \end{equation}
  Here, I have omitted the notation ``$val$'' from the top three rows. To understand this diagram, we first record that we have a factorization:
  \[
  q_S = \tilde q_T q_T,
  \]
  ...
  \]
  and hence, via Remark \ref{OMEGA_E_REMARKS} (2), a factorization
  \[
  \begin{tikzcd}
...
\end{lemma}
\begin{proof}
  This follows analogously to Lemma \ref{K-1_LAN_FINAL_LEMMA}, by replacing Diagram \ref{K-1_LAN_FINAL_DIAGRAM} with the diagram below:
  \[
  \begin{tikzcd}
    && S & \\[20pt]
    &&\tilde q_T^*(S) \arrow[u, "\tilde q_T"]\\
...
Note that be Lemma \ref{LAN_NATURALITY_PROP}, we have natural maps $\P_{k-1} \to \P_k$.
...
In order to state our filtration result, we will need to identify another categorical construction. This filtration will be built out of ``pushout products over trees of maps of sequences''. This subsection will be dedicated to making the components of that statement precise.

Recall the categorical wreath product, defined in Definition \ref{CAT_WREATH_DEFINITION}, and that $\mathsf F_0$ is the category of finite sets and isomorphisms.  
\begin{definition}
  Given a map $u: Y_0\to Y_1$ of sequences and $(A,D)\in\mathsf{F_0}\wr\Sigma$, we borrow notation from \cite{BM03} and define the functor $[u]^D: (0 \to 1)^A \to \V$ as the composite
\[
(0 \to 1)^A \to \mathsf{F_0} \wr V \xrightarrow{\otimes} \V
\]
where the first map is defined on $\epsilon: A \to \set{0,1}$ by 
\[
(\epsilon(a))_a \mapsto (A, (Y_{\epsilon(a)}(D(a)))_a).
\]

\end{definition}


We recall that, in a general category $\mathcal C$, a subcategory $\mathcal{C'}\subseteq \mathcal{C}$ is called \textit{convex} if whenever $c'\in \mathcal{C'}$ and $c\mapsto c'$ is an arrow in $\mathcal{C}$, then both $c'$ and the map are in $\mathcal{C'}$.


\begin{definition}
  \label{Q_DEFINITION}
 Let $\mathcal C$ be a convex subcategory of $(0 \to 1)^A$. We define $Q_{\mathcal{C}}^A[u]^D := \colim_{\mathcal{C}}[u]^D$; moreover, given nested convex subcategories $\mathcal{C'}\subseteq \mathcal{C}$, let 
\[
[u]^D\Box_{\mathcal{C'}}^{\mathcal{C}}: Q_{\mathcal{C'}}^A[u]^D \to Q_{\mathcal{C}}^A[u]^D
\]
...
\]
\[
[u]^{\Box D}:= [u]^D\Box_{\mathcal{C}}^{(0 \to 1)^A}: Q[u]^D \to \bigotimes_{a\in A}Y_1(D(a)).
\]
\end{definition}
...
\subsection{Kan Extensions - May Also be Part of the Above}
\label{KAN_EXTENSIONS_APPENDIX}
\label{LAN_APPENDIX}

We collect some technical results about the naturality of Kan extensions on their input data, and their preservation under certain categorical constructions.

\begin{remark}
  For all of the results below, their formal dual result is true of \emph{right} Kan extensions.
\end{remark}

We begin with an easy result about ``stacking'' Kan extensions.
\begin{lemma}
  \label{COMP_LAN_LEMMA}
  Suppose we have functors
\[
\begin{tikzcd}
  \mathcal C' \arrow[d, "i"'] \arrow[dr, "X"] \\
  \mathcal C \arrow[d, "j"'] \arrow[r, "Y"] & \V\\
  \mathcal D
\end{tikzcd}
\]
such that $Y = \Lan_i X$. Then $\Lan_{ji}X \simeq \Lan_j Y$. 
\end{lemma}
\begin{proof}
  This follows from the Yoneda Lemma by directly unpacking the universal properties of the two functors.
\end{proof}

\subsubsection{Naturality}
\label{CAT_DOWNARROW_APPENDIX}

We formulate precisely what data Kan extensions are natural over. 

\begin{definition}
  Let $\mathsf{WSpan}(\mathcal D, \V)$ be the following category. Objects are spans of functors
\[
 \begin{tikzcd}
   \mathcal{C}\arrow[d] \arrow[r] & \V\\
   \mathcal{D} 
 \end{tikzcd}
\]
while morphisms are pairs $(F,\Phi)$
\[
\begin{tikzcd}
  \mathcal{C}' \arrow[d,"F"'] \arrow[dd, bend right=50] \arrow[dr, bend left = 20, swap, ""{name=U}]& \\
  |[alias=V]| \mathcal{C} \arrow[r] \arrow[Rightarrow, from=U, "\Phi"'] \arrow[d] & \V\\
  \Sigma
\end{tikzcd}
\]
with $F$ a functor such that the left triangle commutes, and $\Phi$ a natural transformation.
\end{definition}

\begin{proposition}
  \label{LAN_NATURALITY_PROP}
  The left Kan extension operation is a functor $\mathsf{WSpan}(\mathcal D, \V) \to \V^{\mathcal D}$. 
\end{proposition}
\begin{proof}
  This is a straightforward diagram chase via the universal property of the left Kan extension. Indeed, suppose we are given the following data:
  \[
  \begin{tikzcd}
    \mathcal{C} \arrow[dr, "X", ""{name=U,below}] \arrow[d,"i"']& \\
    |[alias=V]| \mathcal{C'} \arrow[r, "Y"] \arrow[d, "j"'] & \V\\
    \mathcal{D} \arrow[ur, shift left, dotted] \arrow[ur, shift right, dotted] 
    \arrow[Rightarrow, from=U, to=V, "\Phi"']
  \end{tikzcd}
  \]
Then, since we always have a natural transformation $\Lan_i Y \circ i \Rightarrow Y$, we have the following chain of bijections and maps:
\begin{align*}
  \V^{\mathcal D}(\Lan_j Y, \Lan_j Y) &= \V^{\mathcal C'}(Y, \Lan_j Y \circ j) \xrightarrow{(\tilde id)^*} \V^{\mathcal C'}(\Lan_i(Y i), \Lan_j Y \circ j) = \V^{\mathcal C}(Y i, \Lan_j Y\circ ji) \\
  &\xrightarrow{\Phi^*} \V^{\mathcal C}(X, \Lan_j Y \circ ji) = \V^{\mathcal D}(\Lan_{ji}X, \Lan_j Y).
\end{align*}
The image of the identity, denoted $\Phi_*$, is the desired natural transformation. It can similarly be shown that this process preserves compositions.
\end{proof}

Diagrammatically, $\Phi_*: \Lan_{ji} X \to \Lan_j Y$ is the unique map such that the diagram below commutes:
\begin{equation}
  \label{LAN_MAPS_COROLLARY_DIAGRAM}
  \begin{tikzcd}
    X \arrow[r, "\Phi"] \arrow[dr, "\alpha_X"] & Y \circ i \arrow[r, "\alpha_Y\ \circ\ i"] & \Lan_j Y \circ j i\\
    & \Lan_{ji}X \circ j i \arrow[ur, "\Phi_*\ \circ\ j i"'] 
  \end{tikzcd}
\end{equation}

We highlight a special case:
\begin{corollary}
  \label{PRECOMP_LAN_LEMMA}
  Suppose we have functors $\mathcal{C} \xrightarrow{i} \mathcal{C'} \xrightarrow{j} \mathcal{D}$ and $X: \mathcal{C'} \to \V$. Then we have a natural transformation $\Phi: \Lan_{ji}Xi \to \Lan_j X$.
  \[
  \begin{tikzcd}
    \mathcal{C} \arrow[dr, "Xi"] \arrow[d,"i"']& \\
    \mathcal{C'} \arrow[r, "X"] \arrow[d, "j"'] & \V\\
    \mathcal{D} \arrow[ur, shift left, dotted] \arrow[ur, shift right, dotted] 
  \end{tikzcd}
  \]
\qed
...
We can also consider ``higher-dimensional'' naturality, such as the following.
\begin{lemma}
  \label{LAN_CUBE_NATURALITY_LEMMA}
  Suppose we have the following commutative diagram of natural transformations
  \[
  \begin{tikzcd}[column sep=tiny]
    & \mathcal C' \arrow[rrr, ""'{name=V}] \arrow[dd] &&&  \V\\
    \mathcal E' \arrow[dd] \arrow[ur] \arrow[rrr, crossing over, ""{name=U}] &&& \V \arrow[ur, equal]\\
    & \mathcal C \arrow[rrr] \arrow[dd] && |[alias=S]| & \V \arrow[uu, equal]\\
    \mathcal E \arrow[ur] \arrow[rrr, crossing over] & |[alias=R]| && \V \arrow[uu, crossing over, equal] \arrow[ur, equal]\\
    & \mathcal D    
    \arrow[Rightarrow, from=U, to=V]
    \arrow[Rightarrow, from=R, to=S]
%    \arrow[Rightarrow, from=V, to=B]
  \end{tikzcd}
  \]
such that the left and right faces commute, the front and back faces are some natural transformations $\Phi$ and $\Psi$, and the top and bottom faces are left Kan extension. Then the maps induced by Lemma \ref{LAN_NATURALITY_PROP} from $\Phi$ and $\Psi$ are isomorphic.
\end{lemma}
\begin{proof}
  We know the sources and targets are isomorphic by Lemma \ref{COMP_LAN_LEMMA}. The result then follows as in the proof of Lemma \ref{LAN_NATURALITY_PROP}.
\end{proof}

\subsubsection{Left Kan Extensions and Pushouts}

While dealing with general pushouts of categories requires solving a ``word problem'' on morphisms, there is a stronger notion which is much easier to understand. We recall that, given a square of categories
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
if the nerve of this square is a pushout in $\sSet$, then this is a pushout of categories (since the nerve is the inclusion of a reflective subcategory). The pushouts that most concern us in this paper will be of this form. 
\begin{definition}
