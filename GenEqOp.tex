...
\begin{definition}
      ...
      More generally, given any family $\F$ (or even collection of subgroups) of $\Sigma$,
...
In particular, when $\F$ is trivial family just containing the trivial subgroup,
...
\]
	...
	\item[(ii)] \eqref{EXTERINTADJ EQ} says that the monoidal product on $\mathcal{V}$ lifts to a left Quillen bifunctor
	\[
...
\begin{example}
	...
	$(\mathsf{sSet}_{\**},\wedge)$ have cofibrant symmetric pushout powers.
        ...
        the genuine cofibrations in $\sSet^{\Sigma_n}$ are precisely the monomorphisms (e.g. Remark \ref{ALLCOF REM}, \cite[Prop. 2.16]{Ste16}).
