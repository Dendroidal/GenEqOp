...
Denote the full subcategory spanned by the $\F$-trees by $\Omega_\F$. 
\end{definition}
...
We may now build a monad on $\Sym_{\F}(\V)$. 
...
\begin{remark}
      \label{UPGAMMA_REM}
      For any $\F$, $\upgamma^{\**}$ is easy to describe explicitly. 
      ...
      In this case, for $Y\in \Sym_\F(\V)$, we have
      \[
      \upgamma_!Y(C) =
      \begin{cases}
...
\end{cases}
\]
However, $\upgamma_{\**}$ remains mysterious. 
\end{remark}
...
It is immediate from Proposition \ref{MONADADJ1 PROP} that $\mathbb{F}_{\F}$ is a monad for any full subcategory $\Sigma_{\F} \subseteq \Sigma_G$.
However, in order to have a nice comparison with $\mathbb{F}_G$ (and later, to have nice homotopical properties), we will need to require that $\Sigma_{\F}$ has the proper closure conditions.
\begin{definition}
      Let $\Op_{\F}(\V)$ denote the category $\mathsf{Alg}_{\mathbb{F}_{\F}}(\V)$ of \textit{$\F$-genuine $G$-operads}.
\end{definition}

We have the following adjustments of Proposition \ref{MONAD_COMPARISON_PROP} and (\ref{TWOADJOINTSOP EQ}).
...
\begin{remark}
      ...
      the gynmastics with adjoints required to compare it with $\mathbb F_G$ and analyze it homotopically is required anyway, and will be cleaner with this definition. 
\end{remark}
...
\begin{remark}
      ...
      is an indexing system, and the proof of Proposition \ref{MONAD_COMPARISON_PROP} yields that $\mathbb F \simeq \mathbb F_{\F_{\text{fr}}}$.
\end{remark}

Comparison between different classes of partially-genuine operads can be difficult. 
As hinted at in Remark \ref{UPGAMMA_REM}, the adjunction maps become less tractible. 
However, combining (\ref{TWOADJOINTS EQ}) with (\ref{F_TWOADJOINTS_EQ}) as below
\[
\begin{tikzcd}[column sep =5em]
      \mathsf{Sym}_\F(\mathcal{V})
      \arrow[r, bend left, "\upgamma_{!}"]
      ...
      &
      \mathsf{Sym}^G(\mathcal{V})
      \ar[bend right]{l}[swap,midway,dashed]{\iota_{!}}
      \ar[bend left]{l}{\iota_{\**}}
\end{tikzcd}
\]
allows us to compare all partially-genuine operads with regular $G$-operads.
...
We define rooted subcategories of $\bar{\Omega}_{G}^e$:
  \begin{enumerate}
...
\begin{proposition}
        ...
        $\P_{k-1}(C) \to \P_k(C)$
        is given by a pushout in $\V^{\mathsf{Aut}(C)}$ as in (\ref{FILTRATION_LAN_LEVEL}), replacing all instances of $\Omega_G^a$ with $\Omega_{\F}^a$, 
...
\begin{proof}
        ...
        \end{tikzcd}
        \]
        Since $\upgamma^{\**}$ is a left adjoint into $\Op_{\F}(\V)$, the filtration and its pushout description from (\ref{FILTRATION_LAN_LEVEL}) induce a filtration and pushout description on the map
        \[
        ...
        and thus (\ref{F_FILTINTALT_EQ}) is isomorphic to the left Kan extension over $\Omega_{\F}^a[k]^{op}$ of the same functor, 
        yielding the desired filtration. 
\end{proof}
...
\begin{definition}
        ...
        \item \textit{$\F$-weak equivalence} (resp. \textit{$\F$-fibration}) if 
                for all $n$, $f(n): \O(n) \to \P(n)$ is one in $\V^{G \times \Sigma_n}_{\F_n}$; 
...
The key technical step in the proof follows as an easy particular case, which we now state, of the stronger Proposition \ref{AUTTCOFPUSH PROP}.
...
\begin{example}
      Our most important case is that of $\V = \sSet$, denoting $\Op^G(\sSet)$ by $\sOp^G$. By the work of \cite{Ste16}, we have that $\sSet$ is underlying strongly cellular, and thus the $\F$-model structure exists on $\sOp^G$ for any such $\F$.
\end{example}
...
Replacing (\ref{FILTRATION_LAN_LEVEL}) with (\ref{FF_LEVEL_FILTRATION}), we have that the statement analogous to Corollary \ref{CELLULAR_LEVEL_COFIB_PROP} but for free $\mathbb{F}_{\F}$-extensions remains true.
% \begin{corollary}
...
$ \mathcal{P} \to \mathcal{P}[\iota_{\**} u]$ (modified by Proposition \ref{FF_LEVEL_FILTRATION}). It thus suffices to verify by induction on $k$ that each 
