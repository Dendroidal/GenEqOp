...
(the graph subgroup $\Gamma_X$ is as discussed after 
\eqref{NORMMAPS EQ}),
...
The monadic description of genuine equivariant (colored) operads as displayed in this paper
is particularly useful for the model categorical considerations of our main results.
In \cite{Pe17,BP_edss,BP_HGOP,BP_TAS},
a combinatorial presentation of this algebraic structure is more suited for the constructions therein.
While the monadic description of genuine equivariant (colored) operads
do not explicitly appear in  \cite{Pe17,BP_edss,BP_HGOP,BP_TAS},
$G$-trees and other presentations of this algebraic structure
% the algebra and combinatorics of genuine equivariant operads
do play a prominent role.
%
In every model category encoding the homotopy theory of equivariant operads with norms,
(co)fibrations and weak equivalences are described using $G$-trees.
%
Moreover,
...
the proof of this fact will also be delegated to a sequel.
This combinatorial notion of a genuine equivariant operad appears and underlies
...
\begin{example}
      ...
      \begin{equation}
            ...
            \end{tikzpicture}
      \end{equation}
      For any $G$-operad $\O$ and $\O$-algebra $X$,
      ...
      This last $G$-tree encodes the twisted structure map
      \[
            \O(2)^{\Gamma_{G/e}} \times X \longto X^G,
      \]
      given by the composite (cf. \eqref{INTFIXPTCOMP EQ})
      \[
      ...
      \[
            \pi_0\O(2)^{\Gamma_{G/2}} \times \pi_0 X \to \pi_0 (X^G).
      \]
\end{example}
...
In \cite{BNerve}, the first-named author extends this to a functor
$N^{\otimes} \colon \sOp^G \to \sOp_G \to \Op_{\infty,G}$,
and moreover produces a functor on the categories of algebras $\Alg(\O) \to \Alg(N^{\otimes}\O)$.
When $\O$ is the genuine equivariant little disks operad,
this connection has been used by \cite{Hor} to give a characterization of equivariant factorization homology.


{\color{blue}{
    colored operad of operads, and we have colored genuine operad of genuine operads
    \begin{itemize}
    \item genuine operad for $\underline{\mathfrak C} = \iota_{\**}\mathfrak C$-colored genuine operad equals
          $\iota_{\**}$ of the operad for $\mathfrak C$-colored operads
    \end{itemize}
  }
}
...
will follow by first building $N$,
...
To build the nerve $\mathcal{N} \mathcal{P}$
...
$\Sigma_G
\leftarrow 
\Omega^n_G \xrightarrow{\mathcal{N}_n}
\mathcal{V}^{op}$
where
\[
	\mathcal{N}_n 
...
Note that the 
$\Omega^{n,o}_G$ still form a simplicial object, i.e.
one still has simplicial operators
$d_i \colon \Omega^{n+1,o}_G \to \Omega^{n,o}_G$
for $0\leq n, 0 \leq i \leq n+1$
and 
$s_j \colon \Omega^{n,o}_G \to \Omega^{n+1,o}_G$
for $0\leq n, 0 \leq j \leq n$,
though we note that one no longer has an augmentation
to $\Sigma_G$ nor extra degeneracies.
...
$\Omega_G^{n,o} \xrightarrow{\mathcal{N}_n} \mathcal{V}^{op}$.
This follows by noting that
the vertex functors
$V_G \colon \Omega^{n}_{G}
\to 
\Fin_s \wr \Omega^{n-1}_{G}$
extend to functors
$V_G \colon \Omega^{n,o}_{G}
\to 
...
\begin{tikzcd}[column sep = 18pt]
	\Omega^{n,o}_G \ar{r}{V_G} 
	\ar{d}[swap]{d_0} &
	\Fin \wr \Omega^{n-1}_G \ar{r}{\Fin \wr V_G} &
	|[alias=Vt]|
	\Fin^{\wr 2} \wr \Omega^{n-2}_G \ar{d}[swap]{\sigma^0}
&
	\Omega^{n,o}_G \ar{r}{V_G} \ar{d}[swap]{d_{i+1}} &
	\Fin \wr \Omega^{n-1}_G \ar{d}{\Fin \wr d_i}
&
	\Omega^{n,o}_G \ar{r}{V_G} \ar{d}[swap]{s_{j+1}} &
	\Fin \wr \Omega^{n-1}_G \ar{d}{\Fin \wr s_j}
\\
	|[alias=FV]|
	\Omega^{n-1,o}_G \ar{rr}[swap]{V_G} &&
	\Fin \wr \Omega^{n-2}_G  
&
	\Omega^{n-1,o}_G \ar{r}[swap]{V_G} &
	\Fin \wr \Omega^{n-2}_G
&
	\Omega^{n+1,o}_G \ar{r}[swap]{V_G} &
	\Fin \wr \Omega^{n}_G
...
Thus, by realizing
$\Omega_G^{n,o} \xrightarrow{\mathcal{N}_n} \mathcal{V}^{op}$
we obtain the desired nerve functor
$\mathcal{N} \mathcal{P}
\colon 
\Omega_G \to \mathcal{V}^{op}$.


We now turn towards the task of building a partial inverse to $\mathcal{N}$.


\subsubsection{Essential surjectivity}
...
\begin{equation}
\begin{tikzcd}
	\Omega^n_G \ar[equal]{d} \ar{r} &
	\Omega_G \ar{r}{\delta^0} &
	|[alias=Vt]|
	\Fin \wr \Omega_G \ar[equal]{d}
\\
	|[alias=FV]|
	\Omega^n_G \ar{r}[swap]{V_G} &
...
\begin{equation}
\begin{tikzcd}
	\Omega_G^1 \ar{d}[swap]{d_1} \ar{r} &
	|[alias=Vtt]|
	\Omega_G \ar{r}{\delta^0} \ar[equal]{d} &
	\Fin \wr \Omega_G \ar[equal]{d} 
&
	\Omega_G^1 \ar[equal]{d} \ar{rr} &&
	|[alias=Vtt2]|
	\Fin \wr \Omega_G \ar[equal]{d} 
\\
	|[alias=FVt]|
...
To show that the top square in 
\eqref{ALGCHECK EQ} commutes,
...
\end{tikzcd}
\end{equation}
We now consider the following, where all terms other than the top 
$\Fin \wr \mathcal{V}^{op}$
...
are obtained 
by whiskering the diagrams below with 
$\Fin \wr \Omega_G \to \Fin \wr \mathcal{V}^{op} \to 
\mathcal{V}^{op}$.
...
indeed makes $X_{-1}$ into a $N$-algebra.
...
\begin{proof}[Proof of Theorem \ref{NERVE THM}]
	...
	is  
	$\Omega^0 \xrightarrow{\mathcal{N_0}} \mathcal{V}$,
...
\begin{tikzcd}[column sep = 30pt]
	\Omega^1_G \ar{d}[swap]{d_1} \ar{r}{d_0} &
	|[alias=Vt]|
	\Omega_G^0 \ar{rd}{\mathcal{N}_0} \ar{d} &
&
	\Omega^1_G \ar{d}[swap]{d_1} \ar{rr}{d_0} 
	\ar{rrd}{\mathcal{N}_1}[near end,name=F1]{}
	[swap,near start,name=F2]{}
	&&
	|[alias=Vt2]|
	\Omega_G^0 \ar{d}{\mathcal{N}_0} &
\\
	|[alias=FV]|
	\Omega^0_G \ar{r} &
	\Omega_G \ar{r}[swap]{\mathcal{N} \mathcal{P}} &
	\mathcal{V}^{op} 
&
	|[alias=FV2]|
	\Omega^0_G \ar{rr}[swap]{\mathcal{N}_0} &&
	\mathcal{V}^{op} 
