...
In this subsection, we will explore this lattice. We begin by identifying certain useful subcategories of $\Sigma_G$.
...
\begin{remark}
      ...
      \textit{families} $\mathcal F_n$ of \textit{graph subgroups} of $G \times \Sigma_n$
      (see Lemma \ref{FAMILY_COROLLAS_LEM}).
...
\begin{remark}
      ...
      \end{cases}
      \]
      However, $\upgamma_{\**}$ remains inaccessible\footnote{
        ...
        while $\upgamma_{\**}$ is only very rarely \textit{not} inaccessible (e.g. for $G \into O_G$).}.      
...
\begin{remark}
        While this composite is always ``managable'', it can nevertheless be quite poorly behaved.
        In particular, if $\Sigma_\F$ does not contain the free $G$-tree $G \cdot C_n$ for some $n \geq 0$
        (which is certainly  possible even for indexing systems),
        ...
        Instead, we will see that it is reflective on \textit{cofibrant} objects in $\Sym^G_\F(\V)$. 
\end{remark}
...
Our next task will be that of identifying a convenient Lan-final subcategory $\widehat{\Omega}_G^{e} \hookrightarrow \Omega_G^e$.
...
\begin{remark}
	One of the key properties of the degrees $|T|$, $|T|_X$, $|T|_Y$ is that they are invariant under root pullback.
\end{remark}

{\color{red} HERE}

\begin{definition}\label{TREE_FILTRATION_PIECES_DEFINITION}
We specify some rooted subcategories of $\widehat{\Omega}_{G}^e$:
  \begin{enumerate}
  \item $\widehat{\Omega}_G^e[\leq\!\! k]$ 
  (resp. $\widehat{\Omega}_G^e[k]$) is the full subcategory of trees 
  $T \in \widehat{\Omega}_G^e$ with $|T|\leq k$ (resp. $|T| = k$);
  \item $\widehat{\Omega}_G^e[\leq\! k,-]$ (resp. $\widehat{\Omega}_G^e[k,-]$) is the full subcategory of $\widehat{\Omega}_G^e[\leq\! k]$ 
  (resp. $\widehat{\Omega}_G^e[k]$) of trees $T$ with $|T|_{Y}\neq k$;
  \item $\widehat{\Omega}_G^e[k,0]$ is the full subcategory of 
  $\widehat{\Omega}_G^e[k]$ of trees $T$ with $|T|_X = 0$ (or, equivalently, $|T|_{Y} = k$).
%  \item If $\Xi$ is any of the above categories, and $C\in \Sigma_G$, let $\Xi(C)$ denote the full subcategory of $\Xi$ spanned by those trees $T$ with $val(T) \simeq C$.
  \end{enumerate}
The above definitions still hold if we replace $\widehat{\Omega}_G^e$ with $\Omega_G^a$; in particular, we have vertical forgetful functors
\[
\begin{tikzcd}
  \widehat\Omega_G^e[k,-] \arrow[dr, "\mathsf{fgt}"'] \arrow[rr, hookrightarrow] && \widehat\Omega_G^e[k] \arrow[dl, "\mathsf{fgt}"]\\
  & \Omega_G^a[k]
\end{tikzcd}
\]
\end{definition}
...
\begin{tikzpicture}
  ...
  child{node [dummy, fill=black] {}%
    child{node [dummy, fill=white] {}%
      child{node [dummy,fill=black] {}%
        child{edge from parent node [swap,near end] {$i$}}
  ...
  \tikzstyle{level 4}=[sibling distance=1em]
	...
	child{node [dummy,fill=black] {}%
		child{edge from parent node [swap]{$i$}}
		child{edge from parent node [near end] {$h$}}
	    child[sibling distance=1em, level distance = 7.5em]{edge from parent node [swap, very near end]{$g$}}
		child[sibling distance=1em, level distance = 7.5em]{edge from parent node [very near end]{$f$}}
...
We expand on Remark \ref{FAMILY_COROLLAS_REM} here. 
\begin{lemma}
...
\begin{proof}
        ...
        and hence $\F$ is a collection of families iff $\Sigma_\F$ is a sieve of $\Sigma_G$.
...
\begin{proposition}\label{FIXPT PROP}
	...
	and that $\V$ is symmetric monoidal with diagonals.
...
\begin{proof}
        ...
        We note also that
        \begin{itemize}
        \item[(c)] the unit of the adjunction is the identity on the image of $\upgamma^{\**}\iota_{\**}$,
