...
See \cite[\S 2.2]{WY15} for a precise definition and further discussion.
...
\begin{remark}\label{FINSURJ REM}
	...
	\textit{symmetric monoidal category with diagonals}\footnote{
            These have also been called \textit{relevance} monoidal categories \cite{DP07}}.
\end{remark}
...
These are equivalent to \textit{semicartesian} symmetric monoidal categories \cite{EK66, Lei16}}.

Finally, if a symmetric monoidal category has both diagonals and projections, it must in fact be \textit{cartesian} monoidal \cite{EK66}.
\end{remark}
