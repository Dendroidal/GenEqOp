...
For much of the discussion, our base model category will need to been sufficiently nice (see Definition \ref{STRONGLY_CELLULAR}). In particular, the strongest statements all hold when $\V = \sSet$.
...
\begin{customthm}{I}\label{MAINEXIST1 THM}
        ...
        In the cases $\V = \sSet$ or $\sSet_*$, these are actual model structures.
\end{customthm}
...
\end{tikzcd}
\end{equation}
is a Quillen equivalence of semi model categories.
...
\begin{proof}[Proof of Theorem \ref{MAINEXIST1 THM}]
        ...
        are sufficient,
        as for any such $\F$, $\F$-(trivial) cofibrations are, in particular, \textit{level} (trivial) genuine cofibrations.
...
\begin{proof}[Proof of Theorem \ref{MAINEXIST2 THM}]
        All three parts follow precisely as in the proof of Theorem \ref{MAINEXIST1 THM},
...
\begin{proposition}\label{AUTTCOFPUSH PROP}
	...
	and suppose that 
	$f_i \colon A \to B$, $1\leq i \leq l$ are level $\mathcal{F}$-cofibrations (resp. trivial cofibrations)
...
\begin{proposition}\label{FIXPT PROP}
	...
	\end{equation}
            is an isomorphism in $\V^{\Omega_G^{0,\underline{l},op}}$ (with $G \times \Omega_0^{\underline{l}} \xrightarrow{\iota} \Omega_G^{0,\underline{l}}$ the inclusion).
\end{proposition}
\begin{proof}
...
\begin{proof}
        ...
        {\color{blue} 
          ...
          Indeed, this would imply that $\iota_{\**}(C^{\Sigma_\F})$ is closed under free extensions, all of which are images of free extensions in $C^{\Sigma_\F}$,
          and hence the same is true for $\iota_{\**}(C^\F)$ (where we can identify this as those objects in $\iota_{\**}(C^{\Sigma_\F})$ isomorphic to retracts of those which have a cellular decomposition beginning with $\varnothing$).
        }% COLOR: BLUE

        To begin, we first note that by Remark \ref{F_MODEL_REM}, 
        ...
        is an isomorphism of cofibrant objects.
