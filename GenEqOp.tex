...
by Gutierrez-White in \cite{GW17} 
...
We note that a similar result has also been proven by Gutierrez-White in \cite{GW17}.
...
\amalg \{U_i|U_i \hookrightarrow U_{\not \langle e}\}$. Existence of $T \to U$ now follows from the induction hypothesis. For uniqueness, the condition that no $U_i$ is a stick guarantees that $T$ possesses a single inner edge mapping to $e$, and thus admits a compatible decomposition as in (\ref{DECOMPPROOF EQ}), so that uniqueness too follows from the induction hypothesis.
...
was also obtained by Gutierrez-White in \cite{GW17}.
...
We start with an abstract argument, which has also been used by Gutierrez-White in \cite{GW17}. 
Writing 
