...
the $n$-th level $\O(n)$ has a $G \times \Sigma_n$,
...
To see why, we first recall that the class of (non-equivariant)
$E_{\infty}$-operads,
...
(\ref{NAIVEGEINFTY EQ}) does not provide the correct notion of $G$-$E_{\infty}$-operads, by which we mean 
up-to-homotopy replacements of the commutative operad 
$\mathsf{Comm}(n)=\**$ in the $G$-equivariant context. Instead, 
$G$-$E_{\infty}$-operads are characterized by the fixed point conditions
...
If $R$ is additionally a strictly commutative ring $G$-spectrum
...
described in (\ref{NAIVEGEINFTY EQ}) is then that it lacks all norms maps other than those for $H$-trivial $X$.
...
The main novelty of \cite{BH15} is the definition, for each finite group $G$, of a finite lattice of types of operads, which they dub $N_{\infty}$-operads.
...
can be thought of as encoding varying degrees of ``equivariant commutativity''.
...
(so that each $\mathcal{F}_n$ is called a \textit{family}),
...
and a key novel condition called \textit{self-induction}.
...
The main result of \cite[\S 4]{BH15} is then that whenever an operad $\O$ as in (\ref{NFINFTY EQ}) exists the associated collection $\mathcal{F}$ must be an indexing system, 
though the converse statement that given any indexing system $\mathcal{F}$ such an $\O$ can be produced is left as a conjecture.

One of the key motivating goals of the present work was to verify this conjecture of Blumberg-Hill, which we obtain 
...
To motivate our approach, we first recall the solution of a closed related but simpler problem: that of building universal spaces for families of subgroups. 
...
best understood in light Elmendorf's classical result 
...
\begin{tikzcd}[column sep =5em,row sep=0.5em]
...
\O(H_1/H_2)^{H_1} \times \O(H_2/H_3)^{H_2} \to \O(H_1/H_3)^{H_1}
\end{equation}
...
\begin{theorem}\label{MAINEXIST1 THM}
$\mathcal{F}$-model structures, 
denoted
$\mathsf{Op}^G_{\mathcal{F}}(\mathcal{V})$,
on 
the category $\mathsf{Op}^G_{\mathcal{F}}(\mathcal{V})$ exist.
\end{theorem}


\begin{theorem}\label{MAINEXIST2 THM}
Projective model structures on  
...
\begin{theorem}\label{MAINQUILLENEQUIV THM}
Let $\mathcal{V}$ be $\mathsf{sSet}$ or, more generally,
...
\begin{corollary}\label{NINFTY_REAL_COR_MAIN}
For $\V = \sSet$ and 
$\mathcal{F} = \{\mathcal{F}_n\}_{n \geq 0}$
any weak indexing system,
$N \mathcal{F}$-operads exist. I.e. there exist operads $\O$
such that
\begin{equation}\label{NFINFTY EQ}
	\O(n)^{\Gamma} \sim 
\begin{cases}
...
\label{GENUINE_OP_MONAD_SECTION}
...
\begin{definition}
  ...
  \begin{enumerate} 
  \item $\V$ is a strongly cofibrantly-generated symmetric monoidal model category, and 
...
\begin{remark}\label{GENUINE FGTRIGHT REMARK} 
  ...
  \[ 
  \bar\Pi\cdot_\Pi(-): \V^\Pi_{gen} \to \V^{\bar \Pi}_{gen}
  \] 
...
The key technical result follows as the easy cases of stronger results from Section \ref{COFIB SEC}.
...
\end{proposition}
\begin{proof}(c.f. the proof of Proposition \ref{AUTTCOFPUSH PROP})
...
\end{corollary}  
\begin{proof} 
  By using the levelwise filtration from (\ref{FILTRATION_LAN_LEVEL}), this follows immediately from Proposition \ref{GENUINE_TREE_BOX_COFIBRANT_PROP} and Remark \ref{GENUINE FGTRIGHT REMARK}. 
\end{proof} 
...
\begin{corollary} 
  \label{GENUINE_OP_F_SEMI_MODEL_THM}
...
\begin{theorem} 
  ...
  If $\V$ is Cartesian strongly cellular, and moreover
  \begin{itemize}
  ...
  \end{itemize}
  then the $\F$-model structure on $\Op_G(\V)$ exists for any $\Sigma_\F$.
\end{theorem} 
\begin{proof} 
...
We now turn our consideration back to regular $G$-operads $\O \in \Op^G(\V)$. In this setting, let $\F = \set{\F_n}$ denote any collection of sets $\F_n$ of subgroups, closed under conjugation, of $G\times \Sigma_n$.
...
\begin{proof}[Proof of Theorem \ref{GEN_SEMI_GOP_THM}]
  Follows exactly as \ref{GENUINE_SEMI_VOP_G_THM}, replacing \ref{CELLULAR_LEVEL_COFIB_PROP} with \ref{CELLULAR_LEVEL_COFIB_PROP2}.  
\end{proof}
...
\begin{proof}
  ...
  \end{tikzcd}
  \]
  in $\Op^{G}_\F(\sSet)$ of the unique map into a cofibration and trivial fibration. Since the initial operad is cofibrant, Theorem \ref{COFIBRANT_G_OPS_THEOREM} implies that $\varnothing \to N\F$ is a level $\F$-cofibration, and hence each $N\F(n)$ is cofibrant in $\sSet^{G\times \Sigma_n}_{\F_n}$; thus, for all $\Gamma \not\in\F_n$, $N\F(n)^\Gamma = \varnothing$. Further, since $N\F$ is $\F$-equivalent to $*$, $N\F(n)^\Gamma \simeq *$ for all $\Gamma \in \F_n$. Hence, each $N\F(n)$ is a universal space for $\F_n$, as desired.
\end{proof}
...
\todo[inline]{come back: restriction to categories? Is this interesting?}
\todo[inline]{come back: any examples of genuine $G$-operads?}
...
A surprising feature of topological algebra is that the category of topological commutative monoids is quite small, and only consists of products of Eilenburg-MacLane spaces \todo{cite Moore}. Instead, the most interesting cases are monoids which are commutative and associative up to homotopy, or more so up to ``coherent'' homotopies. To capture these more complex algebraic notions, Boardman-Vogt \cite{BV73} and May \cite{May72} developed the notions of \textit{operads}. Operad encode ``generalized multiplication'', and consists of a sequences $\O = \set{\O(n)}$ of $\Sigma_n$-spaces $\O(n)$ recording ``$n$-ary operations''. The structures of (commutative) monoids, Lie algebras, algebras with a module, and other algebraic structure can be all captured using this structure. Moreover, in \cite{May72}, May introduced $E_\infty$-operads, ``homotopical replacements'' for the commutative operad which encode the above ``monoids up to higher homotopies''. In particular, an $E_\infty$-algebra structure on $X$ doesn't give unique maps $X^n \to X$, but instead ``homotopy unique'' maps.

Moreover, he showed that these operads also encoded infinite loop spaces (and hence connective spectra). Further, $E_\infty$-operads are characterized by the homotopy type of their levels $\O(n)$: $\O$ is $E_\infty$ iff each $\O(n)$ is $\Sigma_n$-free and contractible; that is,
\[
\O(n)^\Gamma \sim
\begin{cases}
...
Now, given a finite group, we can consider operads with a $G$-action that commutes with all of the algebraic structure. In this case, the homotopical replacements form $\mathsf{Comm}$ are harder to pin down. In particular, commutative $G$-monoids $X$ have additional unique multiplicative \textit{norm maps} $N^A X \to X$, where $A$ is a finite $H$-set with $H\leq G$, and $N^A X$ is the Hill-Hopkins-Ravenel Norm; in particular, these norm maps were instrumental in the proof of the Kervaire Invariant One problem \cite{HHR, BH15}.
...
\end{definition*}
There is an immediate example of such objects: equipping any non-equivariant $E_\infty$-operad $\O$ with the trivial $G$-action yields such an object, with the following fixed points:
\[
\O(n)^\Gamma \sim
\begin{cases}
        * \qquad & \Gamma \leq G\\
...
\end{cases}
\]
However, Constenoble-Waner \cite{CW91} showed that algebras over this operad only have trivial norm maps $X^n \to X$; that is, these encode $E_\infty$-algebras (or infinite loop spaces) that happen to have a $G$-action, completely missing this more interesting structure.

The missing ingredient is that the $N_\infty$ condition does not specify a unique $G\times\Sigma_n$-homotopy type for $\O(n)$. Indeed, it only says that $\O(n)^\Gamma = \varnothing$ for $\Gamma \cap \Sigma_n \neq *$. If $\Gamma \leq G$ were the only subgroups of $G\times \Sigma_n$ for which this was not true, we wouldn't have an issue. However, it is easy to verify that $\Gamma \cap \Sigma_n = *$ iff $\Gamma = \Gamma(\phi)$ is the graph of a partial homomorphism 
\[
G \leq H \xrightarrow{\phi} \Sigma_n.
\]
Thus, in the above example, all fixed point spaces for non-trivial homomorphisms are empty. Instead, Constenoble-Waner showed that the 'correct' notion of equivariant $E_\infty$ requires that each $\O(n)$ is ``as contractible as possible while still being $\Sigma_n$-free'': $\O$ is \textit{$G$-$E_\infty$} iff
\[
\O(n)^\Gamma \sim
\begin{cases}
...
These variations are based on which norm maps are specified: the na\'ive choice has only trivial norms, while the genuine option of \cite{CW91} has all norms. Blumberg-Hill \cite{BH15} characterized all possible options, using the following definition of $N_\infty$ (a term they coined):
\begin{definition}
...
\end{definition}
As each homomophism $H \to \Sigma_n$ encodes an $H$-set structure on $\underline{n} = \set{1,2,\ldots, n}$, a collection of families $\F = \set{\F_n}$ is equivalent to a collection of $H$-sets $\F(G/H)\subseteq \Fin^H$ for each $H\leq G$. Indeed, $A\in \F(\O)(G/H)$ iff $\O$ algebras $X$ have norm maps $N^A X \to X$. 

Moreover, if $\F$ came from an $N_\infty$-operad $\O$, these $\F(\O)_n$, and hence the $\F(\O)(G/H)$, are subject to compatibility conditions coming from the operadic structure. 
\begin{definition}\cite{BH15}
        Let $\underline{\Fin}$ denote the functor $O_G^{op} \to \mathsf{SymMon}$ sending $G/H$ to the symmetric monoidal category $(\Fin^H, \amalg)$. A system $\underline{\F} = \set{\F(G/H)}$ is called an \textit{indexing system} if $\underline{\F}$ is a subsystem of $\underline{\Fin}$ which contains all trivial sets and is closed under finite limits and self-induction.
\end{definition}
...
In order to arrive at this result, we need a better understanding of the interplay between the subtle equivariance structures we just discussed, and the combinatorics which underly operads. Indeed, we will need to develop the proper notion of ``coefficient system'' in the context of operads over some ``nice'' base category $\V$

To that end, we exploit the equivariant generalization $\Omega_G$ of the dendroidal category $\Omega$ discussed in \cite{Pe17}. First, we repackage the description of the free operad monad $\mathbb F$ on the category of symmetric $G$-sequences $\Sym^G(\V) = \V^{G\times\Sigma}$ as a left Kan extension out of a span 
\[
\begin{tikzcd}
        |[alias=U]| \Omega \arrow[d, "\mathsf{lr}"'] \arrow[r, "N_X"] & \V\\
        \Sigma \arrow[ur, "\mathbb F X"', ""{name=V}]
        \arrow[Rightarrow, from=U, to=V] 
\end{tikzcd}
\]
where $\mathsf{lr}$ is the ``leaf-root'' or ``valence'' functor, and $N_X$ sends $T$ to the ``product of the evaluations at the vertices of $T$ via $X$''.

Generalizing, we define the \textit{free genuine $G$-operad} monad $\mathbb F_G$ as a similarly-built left Kan extension over $\mathsf{lr}: \Omega_G \to \Sigma_G$, where $\Sigma_G$ is the category of $G$-corollas. This is a monad on the category of \textit{$G$-symmetric sequences} $\Sym_G(\V) = \V^{\Sigma_G^{op}}$.  Intuitively speaking, $G$-symetric sequences $Y\in \Sym_G(\V)$ can be evaluated at any finite $H$-set for $H\leq G$ (more accurately, at any $G$-corolla), and genuine $G$-operads $Y\in \Op_G(\V)$ have composition maps, e.g. of the form
\[
Y(G/H_1 \amalg G/H_2) \times Y(H_1/K_{1,1} \amalg H_1/K_{1,2}) \times Y(H_2/K_2) \to Y(G/K_{1,1} \amalg G/K_{1,2} \amalg G/K_2). 
\]

There is a natural inclusion $\Op^G(\V) \into \Op_G(\V)$ with essential image those $\O$ such that $\O(A) \simeq \O(|A|)^{\Gamma_A}$, where $A\in \Fin^H$ for some $H\leq G$, and $\Gamma_A = \Gamma(\phi_A)$ for any $\phi_A: H \to \Sigma_{|A|}$. That is, the image is those ``systems'' which satisfy a strong fixed-point condition relating the different evaluations. In this way, $\Sym_G$ plays the role of ``coefficient systems''.

We would like to show that this comparison map is a Quillen equivalence. Moreover, from the discussion of $N_\infty$-operads above, we would further like to homotopically restrict our consideration to any subsystem $\underline{\F}$ of $\underline{\Fin}$.
Operadically extending the work of \cite{Pia91, Ste16}, we prove a stronger result (holding for more general $\underline{\F}$):
...
Furthermore, the construction of $\mathbb F_G$ naturally inspires a grafting closure condition with respect to the equivariant ``valence'' (``leaf-root'') functor we could ask our $\Sigma_\F$ to satisfy, that of being a \textit{weak indexing system}. With this setup in place, we come to our main result:
...
Part (i) is sufficient to resolve in the affirmative the conjecutre of Blumberg-Hill, using a cofibrant replacement argument (\ref{N_INFINITY_REALIZATION_COROLLARY}). However, as throughout the paper we use techniques analogous to the framework Elmendorf \cite{Elm83} (by way of \cite{Pia91,Ste16}), we also build an explicit cofibrant replacement in $\Op_G(\V)$ using a bar construction (\ref{N_INFINITY_BAR}). 
...
\[
\begin{tikzcd}
        \Op^G_\F(\V) \arrow[r, "i_*"', "\simeq_Q"] \arrow[d, "hcN^G"', "\simeq ?"] & \Op_G^\F(\V) \arrow[d, "hcN_G", "\simeq ?"]\\
        \mathsf{dSet}^G_\F \arrow[r, "i_*"', "\simeq_Q ?"] & \mathsf{dSet}_G^\F
\end{tikzcd}
\]
We expect to make a full comparison between these notions in sequels.

% In order to compare $\Op^G$ and $\Op_G$, we extend the framework defining $\mathbb F_G$ in order to build, in particular, a filtration of free extensions of genuine operads. We then spend the rest of the paper investigating this filtration, and in particular how cofibrancy interacts with the equivariance restrictions. 
...
\subsection{Tools and Techniques / Outline}
\todo[inline]{come back}
