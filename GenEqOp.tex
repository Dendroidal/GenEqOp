...
\end{equation}
where the inputs $\O(3), \O(2), \O(3), \O(0)$ correspond to the nodes (i.e. circles) in the tree $T$, with arity given by number of incoming edges (i.e. edges immediately above)
...
can have multiple inputs but a single input.}
$\Omega(T)$,
as follows.
...
which is labeled by the corresponding edge set of orbits in the expanded representation
...
of a node does not just count incoming edge orbits,
