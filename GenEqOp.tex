...
\begin{remark}\label{FILTCOMP REM}
	...
	The frameworks in \cite{WY18},\cite{BB17}
	are general enough to cover the usual category $\mathsf{Op}(\V)$ of operads,
	so one might hope they would suffice for our purposes.
%	
	However, one runs into two main issues:
\begin{inparaenum}
\item[(i)]
	by Remark \ref{NEED_WREATH_REMARK},
	defining genuine operads $\mathsf{Op}_G(\V)$
	\emph{requires} using diagonal maps in $\V$,
	and since diagonals play no role in the monads in \cite{WY18},\cite{BB17},
	genuine operads can not covered by those frameworks;
\item[(ii)]
	\cite{WY18},\cite{BB17}
	...
	\eqref{GENEOPEQMT EQ}.
	Consequently, writing $\widetilde{\mathbb F}$ for the monad that \cite{WY18},\cite{BB17} use to describe
	$\mathsf{Op}^G(\V) = \mathsf{Op}(\V^G)$,
	one has that not all the generating (trivial) cofibrations needed for 
	Theorem \ref{MAINEXIST1 THM}
	(see \eqref{ANOPUCH EQ})
...
As such, rather than attempting the hefty task of modifying \cite{WY18},\cite{BB17} to address the two issues above, 
our approach adapts the key ideas in
\cite{WY18},\cite{BB17} while focusing on $G$-trees for intuition.
Ultimately, the description of our filtration 
\eqref{FILTRATION_LAN_LEVEL}
is reminiscent of the description of free extensions of operads in \cite[\S 5.11]{BM03}\footnote{We caution that
\cite[\S 3]{BM09}
corrects an issue in \cite{BM03} concerning the effect of operadic units.},
though we note that our workflow is very different from that in 
\cite{BM03}.
Namely, 
\cite{BM03} describes an analogue of 
\eqref{FILTRATION_LAN_LEVEL}
directly via an ``educated guess'' so as to build an object $F_{\infty}$
which must a posteriori be checked to be both an operad and the desired pushout $P[u]$ in \cite[Prop. 5.1]{BM03}.
On the other hand, 
we start with the pushout $\mathcal{P}[u]$ itself,
abstractly defined, 
and gradually work our way down to 
\eqref{FILTRATION_LAN_LEVEL},
similarly to \cite{WY18},\cite{BB17}.
\end{remark}
