...
A surprising feature of topological algebra is that the category of (connected) topological commutative monoids is quite small, consisting only of products of Eilenberg-MacLane spaces (e.g \cite[4K.6]{Hatcher}). Instead, the more interesting structures are those monoids which are commutative and associative only up to homotopy, and moreover up to ``all higher homotopies''. To capture these more subtle algebraic notions, Boardman-Vogt \cite{BV73} and May \cite{May72} developed the theory of \textit{operads}. Informally, an operad $\O$ 
consists of sets/spaces $\O(n)$ of ``$n$-ary operations'' carrying a $\Sigma_n$-action recording ``reordering the inputs of the operations'', and a suitable notion of ``composition of operations''.
The purpose of the theory is then the study of 
``objects $X$ with operations indexed by $\O$'',
refereed to as \textit{algebras}, with the notions of monoid, commutative monoid, Lie algebra, algebra with a module, and more,
...
$E_\infty$-operads are characterized by the homotopy type of their levels $\O(n)$: $\O$ is $E_\infty$ iff each $\O(n)$ is $\Sigma_n$-free and contractible. That is, for each subgroup $\Gamma \leq \Sigma_n$ one has
\[
...
upon forgetting the $\Sigma_n$-actions.
In this context, $E_\infty$-operads are then equivalent to the commutative operad $\mathsf{Comm}$ and, moreover, 
any cofibrant replacement of $\mathsf{Comm}$
is $E_{\infty}$.
...
the correct replacement of $\mathsf{Comm}$
 in the $G$-equivariant context. 
 %Instead, %notion of $G$-$E_{\infty}$-operads, by which we mean 
%up-to-homotopy replacements of $\mathsf{Comm}$ in the $G$-equivariant context. 
...
Moreover, for any $\mathsf{Comm}$-algebra $R$, i.e. strictly commutative $G$-ring spectrum, 
...
described in (\ref{NAIVEGEINFTY EQ}) is then that it lacks all norms maps other than those for $H$-trivial $X$, thus lacking 
some of the data encoded by $\mathsf{Comm}$.
...
though our analysis goes significantly further, again guided by Blumberg-Hill's work in \cite{BH15}.
...
state that each $\mathcal{F}_n$ is a \textit{family}, i.e. closed under subgroups and conjugation. These first conditions, which are common in equivariant homotopy theory,
...
and an entirely novel key condition called \textit{self-induction}.
...
The main result of \cite[\S 4]{BH15} is then that whenever a $N\mathcal F$-operad $\O$ as in (\ref{NFINFTY EQ}) exists, the associated collection $\mathcal{F}$ must be an indexing system.
...
best understood in light of Piacenza's classical result
from \cite{Pia91}
...
in the same way as in (\ref{GENEOPEQ EQ}) along with obvious restriction maps and, more importantly, 
...
We can now finally give a full informal description of the category $\mathsf{Op}_G$ featured in 
...
and composition maps e.g. %, for example,
\[
...
In the cases $\V = \sSet$ or $\sSet_*$, these are both actual model structures.
\end{customthm}
...
If $\V = \sSet$ or $\sSet_*$, then the above are equivalences of Quillen model categories.
...
In (\ref{COFADJINT EQ}), universal spaces for families $E\F$ are constructed by
cofibrantly replacing the coefficient system $\delta_\F$ in $\V^{O_G^{op}}$ (where, to construct $EG$, $\delta_\F$ is in fact the terminal object).
Equivalently, we could have cofibrantly replaced the terminal object in the category of \textit{partial} coefficient systems $\V^{O_\F^{op}}$, 
...
In the parallel operadic story, 
$N_\infty$-operads replace the $E\F$, 
and the category $\Op_G$ just built plays the role of $\V^{O_G^{op}}$.
To complete our investigation, we need to characterize those ``$\delta_\F$'' which \textit{can} exist in $\Op_G$,
and moreover develop the notion of \textit{partial} genuine operads,
to provide a natural setting for the construction of the $N\F$.
...
\begin{definition}
      For any category $\mathcal D$, a subcategory $\widehat{\mathcal D}$ is called a \textit{sieve} if 
      for all maps $f: d \to \widehat d$, knowing $\widehat d \in \widehat{\mathcal D}$ implies both $d$ and the map $f$ are also in $\widehat{\mathcal D}$.
...
\begin{remark}
      ...
      Due to this, we will abuse notation and abbreviate either set of data simply as $\F$. 
\end{remark}
...
$\Sigma_{\mathcal{F}}$,
and we denote the full subcategory spanned by the $\F$-trees $\Omega_\F \hookrightarrow \Omega_G$. 
\end{definition}
...
$\mathsf{lr}(T) \to T$ is sufficient to show that the condition in 
...
\begin{remark}
      ...
      However, $\upgamma_{\**}$ remains hard to unpack in general\footnote{
        In fact, this formula holds for any $\upgamma_!: \V^{\widehat{\mathcal D}^{op}} \to \V^{\mathcal D^{op}}$ when $\widehat{\mathcal D}$ is a sieve of $\mathcal D$.
...
Finally, in certain special cases,
%if our base category $\V$ is even nicer, 
these semi model structures are in fact Quillen.
...
\begin{remark}\label{SIEVE REM}
	Any collection of subgroups $\F$ determines a full subcategory 
	$\mathsf{O}_{\mathcal{F}} \hookrightarrow \mathsf{O}_G$
	of those orbital $G$-sets $G/H$ for 
	$H \in \mathcal{F}$.	
	Moreover, it is easy to check that $\F$ is a family iff $\mathsf{O}_{\mathcal{F}}$ is a sieve.
...
\begin{lemma}
        ...
        The set of families of corollas $\Sigma_{\mathcal F}$ 
        is in bijection with 
        the set of collections $\mathcal F = \set{\mathcal F_n}_{n \geq 0}$ of $G$-graph families $\mathcal F_n$ of $G\times\Sigma_n$.
