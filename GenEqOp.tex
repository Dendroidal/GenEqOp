...
so that $\O \to \O'$ considered a weak equivalence only if
\begin{equation}\label{NAIVEOPEQ EQ}
	\O(n)^H \xrightarrow{\sim} \O'(n)^{\Gamma}, \qquad \Gamma \leq G\times \Sigma_n, \Gamma \cap \Sigma_n = \{\**\}.
\end{equation}
are weak equivalences.

{\color{red} HERE}

We now recall why it is that (\ref{GENGEINFTY EQ}), rather than 
(\ref{NAIVEGEINFTY EQ}), provides the best up-to-homotopy replacement for $\mathsf{Comm}$ in the equivariant context.
The original perspective in \cite{CW91} is focused on equivariant loop spaces, 
but for our purposes we will the perspective
of Blumberg-Hill in \cite{BH15},
which focuses on the Hill-Hopkins-Ravenel norm maps used in the solution of the Kervaire invariant problem.
...
\begin{equation}
	N^X R \to R
\end{equation}
satisfying natural equivariance and associativity conditions.
