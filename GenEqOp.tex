...
While we note that in the result above, the sets $\F_n$ can contain \textit{general} subgroups of $G\times \Sigma_n$, this flexibility does not exist (or make sense) when considering our genuine $G$-operads. Instead, we must restrict to collections ${\mathcal F} = \set{{\mathcal F}_n}$ of sets of \textit{graph} subgroups, or equivalently full subcategories $\Sigma_{\mathcal F}$ of $\Sigma_G$. Letting $\Op_{\mathcal F}(\V)$ denote the category of $\mathbb F_G$-algebras in $\Sym_{\mathcal F}(\V)$, we have the following, proved in Section \ref{OP_G_EXISTS_SECTION}.
...
Oone important remark to make after these results is that these model structures need not be ``well-behaved'', in particular with respect to cofibrancy, unless the collections ${\mathcal F}$ are highly structured.
...
While this paper focuses soley on \textit{single coloured} operads, we expect the theory to extend immediately to the coloured setting. More than just for completeness, such an extension will allow for more complete comparisons with the other models for $G$-operads.
...
\item The rightmost diagrams in (\ref{PIIDEFDI EQ}) are pullbacks diagrams in $\mathsf{Cat}$.
...
Lastly, (d) is argued by induction on $k$ and $n$. The case $k=0$ follows by the diagram on the left below, combined with
...
\begin{tikzcd}[row sep=1.7em,column sep = 2.5em]
	...
	\Fin_s^{\wr 2} \wr \Omega_G^{n-k-2} \ar{r}{\sigma_0} \ar{d}[swap]{\Fin_s^{\wr 2}d_{i-1}}  &
	\Fin_s \wr \Omega_G^{n-k-2} \ar{d}[swap]{\Fin_s \wr d_{i-1}}
\\
	\Omega_{G}^{n-1} \ar{d}[swap]{d_{1,\cdots,n}} \ar{r}[swap]{V_G} &
	\Fin_s \wr \Omega_{G}^{n-2}
	\ar{d}{d_{0,\cdots,n-1}}  &
	\Omega_G^{n-1} \ar{r}[swap]{V_G} &
	\Fin_s \wr \Omega_G^{n-3} \ar{r}[swap]{V_G^k} &
	\Fin_s^{\wr 2} \wr \Omega_G^{n-k-3} \ar{r}[swap]{\sigma_0} &
...
\begin{remark}
	Generalizing the notation $\Omega_{G,-1} = \Sigma_G$, we will also write $\Omega^{(A)}_{G,-1} = A$, in which case
	$V_{G,-1}^{(A)} \colon \Omega^{(A)}_{G,-1} \to \Fin_s \wr A$
...
\begin{remark}
The identifications 
$\Omega_{G,0}^{\left(\Omega_{G,n}^{(A)}\right)} \simeq
 \Omega_{G,n+1}^{(A)}$
canonically induce more general identifications 
$\Omega_{G,k}^{\left(\Omega_{G,l}^{(A)}\right)} \simeq
 \Omega_{G,k+l+1}^{(A)}$
 which we will need to detail then $k=1$.
\end{remark}


\begin{remark}
For any $A \to \Sigma_G$ there are natural isomorphisms as on the leftmost diagram below, while the rightmost diagram is strictly commutative.
\begin{equation}\label{PIIDEFA EQ}
\begin{tikzcd}[row sep=0.9em,column sep = 6em]
	\Omega_{G,n}^{(A)} \ar{dr}[name=F1]{V^{(A)}_{G,n}} \ar{dd}[swap]{d_i} &
&
	\Omega_{G,n}^{(A)} \ar{dr}{V_{G,n}^{(A)}} \ar{dd}[swap]{s_j} &
\\
	& \Fin \wr A &
	& \Fin \wr A 
\\
	|[alias=G2]|
	\Omega_{G,n-1}^{(A)}  \ar{ur}[swap]{V^{(A)}_{G,n-1}} & &
	\Omega_{G,n+1}^{(A)}  \ar{ur}[swap]{V^{(A)}_{G,n+1}} & 
\arrow[Leftrightarrow, from=F1, to=G2,shorten >=0.25cm,shorten <=0.25cm,"\pi_i^{(A)}"]
\end{tikzcd}
\end{equation}
In fact, the constituent arrows of $\pi_i^{(A)}$ simply the pullbacks over $\Fin$, i.e. permutations,
lifting the pullbacks $\pi_i$ of (\ref{PIIDEF EQ}).
Therefore, the $\pi_i^{(A)}$ are natural in $A$ in the sense that for any functor $H \colon A \to B$ over $\Sigma_G$
there is an equality of natural transformations
\begin{equation}\label{PIANAT EQ}
	(\Fin \wr H)\pi_i^{(A)} = \pi_i^{(B)}\Omega_{G,n}^{(H)},
\end{equation}
and the $\Omega^{(A)}_{G,n}$ variants of all three parts of 
Proposition \ref{PIIPROP PROP} also hold.
%are functors 
...
\begin{equation}\label{MULTDEFSPAN EQ}
	\begin{tikzcd}
	\Sigma_G \ar[equal]{d}&
	\Omega_{G,1}^{(A)} \ar{r}{V_G^{(A)}} \ar{d}[swap]{d_{0}^{(A)}} \ar{l}&
	\Fin \wr \Omega_{G,0}^{(A)} \ar{r}{\Fin \wr V_G^{(A)}} &
...
\\
	\Sigma_G &
	|[alias=Omega]|\Omega_{G,0}^{(A)} \ar{rr}[swap]{V_G^{(A)}} \ar{l}&&
	\Fin \wr A \ar{r} &
	|[alias=cat]|
	\Fin \wr \mathcal{V}^{op} \ar{rr}[swap]{\Pi^{op}} &&
	\mathcal{V}^{op}
	\arrow[Leftrightarrow, from=FFOmega, to=Omega,shorten <=0.15cm,,shorten >=0.15cm,"\pi_0^{(A)}"]
...
$\pi_0^{(A)}$ square is indeed $V_{G,1}^{(A)}$
(using the inductive description given by the top row in (\ref{ALLSQUARES EQ})), and $\alpha$ is the associativity isomorphism for $\Pi$ described in (\ref{COHER EQ}).
Note that naturality of $\mu$ 
follows from the commutativity of (\ref{PIANAT EQ}).
...
\\
	\Sigma_G &
	\Omega_{G,0}^{(A)} \ar{l} \ar{r}[swap]{V_G^{(A)}}&
	\Fin \wr A \ar{r} &
...
\\
	...
	\arrow[Leftrightarrow, from=FFOmega, to=Omega,shorten <=0.15cm,shorten >=0.15cm,"\pi_0^{(A)}"]
	\arrow[Leftrightarrow, from=dog, to=cat,shorten <=0.15cm,shorten >=0.15cm,"\alpha"]
	\arrow[Leftrightarrow, from=dog2, to=cat2,shorten <=0.15cm,shorten >=0.15cm,"\Fin \wr \pi_0^{(A)}"]
...
\begin{equation}
	...
	\Omega_{G}^2 \wr A \ar{d}[swap]{d_1^{(A)}} \ar{r} &
	\Fin \wr \Omega_{G}^1 \wr A \ar{d} \ar{r}&
	...
	\mathcal{V}^{op} \ar[equal]{d}
\\
	\Omega_{G}^1 \wr A \ar{r} \ar{d}[swap]{d_{0}^{(A)}}&
	|[alias=cat2]|
