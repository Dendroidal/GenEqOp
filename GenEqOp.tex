...
provides the best up to homotopy replacement for $\mathsf{Comm}$ in the equivariant context comes from the study of equivariant infinite loop spaces. %(mimicking the original source of $E_\infty$-algebras). 
...
Moreover, for any $\mathsf{Comm}$-algebra $R$, i.e.
...
a $\mathcal{F}$ satisfying all of those conditions.
...
by Gutierrez-White in announced work 
and by Rubin in \cite{Rub17}, 
with each of their approaches having different advantages:
the Gutierrez-White's
model for $N \mathcal{F}$
is cofibrant 
while Rubin's model is explicit.
Our model, which emerges from a more conceptual approach, satisfies both of these desiderata.

%This conjecutre has been independently confirmed in \cite{Rub17} and announced by Gutierrez-White \cite{GW},
%both by different methods.
%Consequently, Rubin obtains models of $N\mathcal F$-operads which are explicit, while the models of Gutierrez-White are cofibrant;
...
also called an $E \mathcal{F}$-space,
...
\O(H/L \amalg H/K)^{H}
\end{equation}
where $\O(X)$ for a $H$-set (resp. $K$-set) $X$ denotes $\O(|X|)$ together with a suitably mixed $H$-action ($K$-action).
...
strengthens the (not so basic) observation that for a simplicial operad $\O$ the associated homotopy operad 
$\text{ho}(\mathcal{O})$
...
which relaxes the model structure axioms by requiring that some of the axioms need only apply
...
%Independently, using different techniques, Rubin \cite{Rub17} and Gutierrez-White \cite{GW} have also announced confirmations of this conjecture for (regular) indexing systems.

\subsection{Future Work}

In order to simplify our discussion this
paper focuses exclusively on the theory of 
...
We note, however, that the colored setting comes with an important new subtlety: 
...
Of particular importance are the notion of 
...
\S \ref{GENUINE_OP_MONAD_SECTION} by discussing the concept of node substitution (which is at the core of the definition of free operads)
 in the context of equivariant trees. The key idea, which is captured in diagram (\ref{SUBSDATUMTREES EQ}), 
...
the key to this section is a careful analysis of  the free extensions in diagram
(\ref{FREE_FG_EXT_EQ}),
with 
\S \ref{LABELSTRI SEC}, 
\S \ref{EXTTREE SEC},
...
a Grothendieck fibration together with 
...
\mathcal{B}^{op} \to \mathsf{Cat}$,
the contravariant Grothendieck construction is
$(\mathcal{B}^{op} \ltimes 
\mathcal{E}_{\bullet})^{op}$.
\end{definition}
...
\end{tikzcd}
\end{equation}
Similarly, $\alpha$ is unital in the sense that both of the following diagrams strictly commute or, more precisely, if the composite natural transformation in either diagram is the identity for the functor 
...
Finally, if a symmetric monoidal category has both diagonals and projections, it must in fact be \textit{cartesian} monoidal \cite[IV.2]{EK66}.
\end{remark}
...
Such understanding can be obtained by 
reading \cite[Example 5.10]{Pe17}
...
Similarly, any finite set or orbital finite $G$-set together with a total order is implicitly reinterpreted as an object of
$\Fin$, $\mathsf{O}_G^p$.
\end{convention}
...
\begin{remark}
If one forgoes the requirement that $T_{\underline{e} \leq e}$ be equipped with the pulled back planar structure, the inclusion $T_{\underline{e} \leq e} \hookrightarrow T$ is usually called simply an \textit{outer face}.
\end{remark}
...
is then that substitution data as in (\ref{SUBSDATUMTREES2 EQ}) can equivalently be 
...
Further, the $T$-substitution datum is planar iff so are the maps $T_{v_{Ge}} \to U^{\mathsf{r}}_{v_{G e}}$.
\end{remark}
...
\begin{equation}\label{STRINGOBJ EQ}
	...
	\end{tikzcd}	
\end{equation}
	where $T_i \in \Omega_G$ and the $\varphi_i$ are planar tall maps, while arrows are commutative diagrams 
...
As such, we will in \S \ref{MONSPAN SEC} first build a monad 
...
\begin{proof}
While not hard to explicitly write formulas for $V_G^k$, $d_i$, $s_j$, $\pi_i$ 
...
(by arguing by induction on $k$ as in (\ref{PROOFD EQ})), there is a unique choice for $d_i$. 
...
\textit{does} become an isomorphism when restricted to  cofibrant objects is one of the key ingredients of our proof of the Quillen equivalence between 
...
$Y=\** \in \mathsf{Sym}^G(\mathcal{V})$ be the simpleton.
	...
	When evaluating $\mathbb{F}_G Y$ at the $G$-fixed stump corolla $G/G \cdot C_0$ 
	(where $C_0 \in \Sigma$ denotes the $0$-th corolla),
	 the two $G$-trees $T_1$ and $T_2$ below encode two distinct points (since $T_1$, $T_2$ are not isomorphic as objects under $G/G\cdot T_0$).
\[
...
Here $\mathsf{O}_\mathcal{F} \hookrightarrow \mathsf{O}_G$ consists of those $G/H$ such that $H \in \mathcal{F}$ so that 
$\mathsf{Top}^{\mathsf{O}_{\mathcal{F}}^{op}}$
consists of partial coefficient systems.
...
\begin{definition}
	...
	for any arrow $f \colon d \to s$ of $\mathcal{D}$ with
...
we thus have a pair of standard adjunctions
\begin{equation}
...
preserves this subcategory and, as it turns out,
...
\begin{remark}\label{VACUOUSNESS REM}
	...
	$G \cdot_H \eta \simeq (\eta)_{G/H}$ are always $\mathcal{F}$-trees.
\end{remark}
...
$(G/H)\cdot C_n \simeq (C_n)_{G/H}$ are in $\Sigma_{\mathcal{F}}$.
\end{definition}


\begin{remark}
	In light of Remark \ref{VACUOUSNESS REM} any weak indexing system must contain the $1$-corollas $(G/H) \cdot C_1 \simeq (C_1)_{G/H}$.
\end{remark}
...
was presented by the second author in \cite[\S 9]{Pe17}. The equivalence between the definitions in \cite{BH15} and \cite{Pe17} was addressed in 
...
planar tall strings $\Omega_{\mathcal{F}}^1$ between $\mathcal{F}$-trees match the pullback 
...
\begin{remark}\label{COMPADJ REM}
	...
	$\Sigma_G^{\text{fr}} \nsubset \Sigma_{\mathcal{F}}$ (the most interesting case being that of $\Sigma_{G}^{\geq 1} \hookrightarrow \Sigma_G$ the corollas of arity $\geq 1$, which models non-unital operads),
	in which case (and compatibly with the 
...
\end{tikzcd}
\end{equation}
We note that the right adjoint 
$\gamma^{\**} \iota_{\**}$
...
\todo[inline]{We delay the techincal proof of the following statement to the Appendix.}
...
Recall that given a group $\Sigma$, the genuine model structure (if it exists)
on $\mathcal{V}^{\Sigma}$,
...
Our main proof will require some auxiliary results concerning the above genuine model structures.
...
More generally, given a family $\mathcal{F}$ (or even collection of subgroups) of $\Sigma$, there then exists a model structure 
...
\begin{lemma}\label{EXMAINLEM LEM}
	...
	Suppose that $\mathcal{P} \in \mathsf{Sym}_G(\mathcal{V})$
	is level genuine cofibrant
	and that 
	$u: X \to Y$ in $\Sym_G(\V)$ is a level genuine cofibration. 
...
\begin{proof}
	...
	then yield that it suffices to check that
\[
\bigotimes\limits_{v \in V_{G}^{ac}(T)}\P(T_v)
=
...
$\mathcal{P}$ is cofibrant in $\mathsf{Sym}^G(\V)$,
and
$u \colon X \to Y$ is a (trivial) cofibration in $\mathsf{Sym}^G(\V)$,
...
(\ref{EXMAINLEM EQ})
is a $\mathsf{Aut}(G \cdot C_n) 
\simeq G 
\times \mathsf{Aut}(C_n)=
G \times \Sigma_n$ genuine cofibration,
...
and using either 
\cite[Thm. 11.3.2]{Hi03} (or equivalently \cite[Thm A.1]{Ste16})
...
with their naive model structure, where weak equivalences and fibrations are defined by forgetting the $\mathsf{Aut}(C)$-action,
and ignoring fixed point conditions.
The desired projective model structure thus
has both less generating (trivial) cofibrations
...
holds,
establishing the existence of the projective model structure% \footnote{
...
We now turn for Theorem \ref{MAINEXIST1 THM}.
...
(as in this case, we may use the filtration (\ref{FILTRATION_LAN_LEVEL}) with $G = \**$ and $\V = \V^G$ by Remarks \ref{NEED_WREATH_REMARK} and \ref{SIGMA_WR_REM}).
\end{proof}
...
In this final section we prove our main result Theorem \ref{MAINQUILLENEQUIV THM}, i.e. we show that there are Quillen equivalences
\[
\begin{tikzcd}[column sep =4.5em]
	\mathsf{Op}_{G}(\mathcal{V}) 
	\arrow[r, shift left=1.5,swap,"\iota^{\**}"']
...
Throughout 
$\mathcal{F}$ denotes a family of subgroups of a finite group $G$, i.e. a collection of subgroups closed under conjugation and inclusion or, equivalently
...
whenever $\mathcal{V}$ is cofibrantly generated and has cellular fixed points,
\cite[Prop. 2.6]{Ste16} shows the existence of a model structure
$\mathcal{V}^{G}_{\mathcal{F}}$
on the $G$-object category
$\mathcal{V}^G$
whose fibrations and weak equivalences are determined by fixed points $(\minus)^H$ for $H \in \mathcal{F}$.
...
%We end this section by cataloging some straightforward interactions of $\mathcal{F}$-model structures with regards to change of group and pushout products.
...
Let $\mathcal{F}$ and $\bar{\mathcal{F}}$ be families of $G$ and $G$, respectively.
...
each concerning cofibrancy and fixed point properties. We start by discussing the cofibrancy axiom.
...
\begin{equation}\label{COMPNFOLDFACT EQ}
	...
	\bullet
\end{equation}
where each $k_i$, $0 \leq i \leq n$, fits into a pushout diagram
...
Remark \ref{FINSURJ REM}.
More explicitly, one has compatible
$\Sigma_n$-equivariant
maps $X \to X^{\otimes n}$.
...
concerns the fixed points of what is more often called the norm object
...
\end{equation}
If $f$ is a genuine $G$-cofibration between cofibrant objects then 
...
(\ref{FIXEDPUSH EQ})
should be an immediate consequence of (\ref{CARTFIX DEF}). However, the real content here is that the two pushout products in 
...
Moreover, it is clear from (\ref{CARTFIX DEF}) that (\ref{FIXEDPUSH EQ}), which is a map of arrows, is an isomorphism on the target objects, hence the real claim is that this map is also an isomorphism on sources.
...
We now convert the results in the previous sections to the context we are trully interested in:
graph families. 
...
a cofibration (trivial cofibration) in 
...
will encode the families of $G$-corollas 
 $\Sigma_{\mathcal{F}}$
...
$(g_i C_n)_{G/H}$ in $\Sigma_G$. The following is then elementary.
...
(for any arbitrary choice of order of the $G/H$ and coset representatives)
...
for some arbitrary (and inconsequential for the remaining discussion) choice of order on $G/H$ and coset representatives.
...
can be translated in terms of families as saying that for any tree $T \in \Omega$ and 
...
are the $G$-graph subgroup $K \leq G \times \Sigma_U$
...
Propositions \ref{AUTTCOFPUSH PROP} and \ref{FIXPT PROP} will now allow us to prove Lemma \ref{MAINLEM LEM}, which provides a characterization of cofibrancy in 
$\mathsf{Op}_{\mathcal{F}}(\mathcal{V})$,
...
and it suffices to verify by induction on $k$
that each 
$\mathcal{P}_k$ is both in the essential image of
...
%``essential image characterization'' of cofibrant objects in (\ref{COFADJ2 EQ}).
%
%
%
%
...
Blumberg-Hill can be built from the theory of genuine equivariant operads.

We start with an abstract argument. 
Writing 
...
consider any Quillen small object argument ``cofibration followed by trivial fibration factorization''
...
$\mathcal{O}_{\mathcal{F}} \xrightarrow{\sim} \mathsf{Comm}$
...
\mathsf{Op}_{\mathcal{F}}(\mathsf{Set})^{\Delta^{op}}
	\hookrightarrow
\in \mathsf{Op}_G(\mathsf{Set})^{\Delta^{op}}
...
The proof of Proposition \ref{BARCOF PROP}
...
\[
\colim_{V \subsetneq U} X_V = L_U X
\xrightarrow{l_U X} 
X_U
\]
...
are the cofibrant objects for the projective model structure on $n$-cubes. As such, they are characterized as the $n$-cubes with the left lifting property against maps of $n$-cubes $Y_{(\minus)} \to Z_{(\minus)}$ that are levelwise trivial fibrations.
\end{remark}
...
\begin{proof}
	...
	$\mathbb{F}_G$ in (\ref{FGXDEF EQ})
	with Proposition \ref{FIBERKANMAP PROP}
	and the fact that 
	the rooted under categories
	$C \downarrow_{\mathsf{r}} \Omega_G^0$
	are groupoids 
	(compare with (\ref{FILTRATION_LAN_LEVEL}))
	yields the formula
...
\end{equation}
	where we note that the innermost expression
	is the top left functor in (\ref{MAINPFADJVAR EQ}).
	...
	$(Y \times Z)_{(U,V)} = Y_U \times Z_V$.
	It is straightforward to check that this $2n$-cube has latching maps
	$l_{(U,V)} Y \times Z = l_U Y \square l_V Z$,
	and is thus a monormorphism $2n$-cube.
	It remains to check that
	the diagonal $n$-cube $\Delta^{\**} (Y \times Z)$
...
$l_{\underline{n}} X^n$
are monomorphisms. This will follow from the stronger claim that the $X^n$
are monomorphim $n$-cubes, which we argue by induction on $n$.
When $n=0$ there is nothing to show.
Otherwise, for any 
$U \subsetneq \{1,\cdots,n,n+1\}$
the restriction of $X^{n+1}$ to subsets of
$U$ is isomorphic to the cube $X^{|U|}$,
so that we need only analyze the top
latching map $l_{\underline{n_+1}} X^{n+1}$.
We now write
$X^{n+1} = (X^n \to 
\widetilde{\mathbb{F}}_G X^n)$, regarding the $(n+1)$-cube as a map of $n$-cubes.
The top latching map $l_{\underline{n_+1}} X^{n+1}$
is then the latching map of the composite square
\begin{equation}\label{BARCOFSQ EQ}
\begin{tikzcd}
 	L_{\underline{n}} X^n \ar{d} \ar[equal]{r}
&
	L_{\underline{n}} X^n \ar{d}
	\ar{r}
&
	X^n_{\underline{n}}
	\ar{d}
\\
	L_{\underline{n}} (\widetilde{\mathbb{F}}_G X^n)
	\ar{r}
&
	\widetilde{\mathbb{F}}_G(L_{\underline{n}}  X^n)
	\ar{r}
&
	\widetilde{\mathbb{F}}_G X^n_{\underline{n}}
\end{tikzcd}
\end{equation}
...
applied to the map
$l_{\underline{n}} X^n \colon 
L_{\underline{n}} X^n \to X^n_{\underline{n}}$, which is a monomorphism by the induction hypothesis.
On the other hand, the left bottom horizontal map in 
(\ref{BARCOFSQ EQ})
is a monomorphism
by applying Lemma \ref{MONOCUBE LEM}(a)
to the cube $\tilde{X}^n$ obtained from 
$X^n$ by replacing the top level
$X^n_{\underline{n}}$ with
$L_{\underline{n}} X^n$.
Hence the latching maps in both squares
in (\ref{BARCOFSQ EQ}) are monomorphisms, 
and thus so is the monomorphism of the composite, showing that $l_{\underline{n_+1}} X^{n+1}$
...
since the maps 
$l_{\underline{n}} X^n \colon 
L_{\underline{n}} X^n \to X^n_{\underline{n}}$
are monomorphisms.
\[
\begin{tikzcd}
	\widetilde{\mathbb{F}}_G 
	(L_{\underline{n}} X^n \times \Delta^n) 
	\ar{r} \ar{d} &
	\mathsf{sk}_{n-1} B_{\bullet} \ar{d}
\\
	\widetilde{\mathbb{F}}_G 
	(X^n_{\underline{n}} \times \Delta^n) 
