...
\abstract{% We build new algebraic structures, which we call genuine equivariant operads
  We define and build the new algebraic structure of genuine equivariant operads,
  which can be thought of as a hybrid between equivariant operads and coefficient systems.
  We then prove an Elmendorf-Piacenza type theorem stating that
  % equivariant operads, with their graph model structure, are equivalent to genuine equivariant operads, with their projective model structure.
  the projective model structure on genuine equivariant operads is Quillen equivalent to
  the graph model structure on equivariant operads. 
...
with an operad $\O$ being called an
...
upcoming work of Bergner-Gutierrez), 
the fact that 
the levels of each 
...
We note that a similar result has also been proven by Gutierrez-White in \cite{GW}.
...
For further details, we recommend the discussiona in \cite[\S 2.2]{WY15} and \cite[\S 12.1]{Fre09}.
...
in \cite[Cor. 5.6]{BH15}
 ...
 (\ref{NFINFTY EQ}) for some collection of graph subgroups 
...
$f^{\**} e \to e$ lifting $f$, meaning that for any choice of horizontal arrows 
\[
...
the data of Grothendieck fibration together with 
...
(note that if we're not in the split case, pullbacks may be chosen arbitrarily).
\end{proof}
...
This $\alpha$ is then associative in the sense that the composite natural isomorphisms between the two functors
...
Suppose that $\mathcal{V}$ is a bicomplete category with fold maps such that
the monoidal product %coproducts
commute with limits in each variable. If the leftmost diagram
...
Dually, if in $\mathcal{E}$
the monoidal product %products
commute with colimits in each variable, and the leftmost diagram
...
\begin{proof}
	Unpacking definitions using the pointwise formula for Kan extensions (\cite[X.3.1]{McL}), the claim concerning (\ref{WRRAN EQ}) amounts to showing that for each $(d_i) \in \Fin_s \wr \mathcal{D}$ one has natural isomorphisms
...
\begin{remark}
      The above results also work if we replace $\Fin_s$ with $\Fin$,$\Fin_i$, or $\Sigma$, and appropriately update the requirements on $(\mathcal V, \otimes)$.
\end{remark}
...
{\color{blue} %%%%% COLOR: BLUE
  In this section, we will investigate the concepts of node substitution, mediated by the notions of planar tall maps.
We will see that node substitution provides access to the structure maps of the free operad monad.
Informally, the free operad on a sequence $X = \set{X_n}$, denoted $\mathbb F X$,
consists of trees with nodes labeled by $X$,
while $\mathbb F \mathbb F X$ has trees with nodes labeled by trees labeled by $X$,
or precisely a ``substitution datum'' labeled by $X$.
Moreover, the monad multiplcation is given, intuitively, by
collapsing the distinctions between the multiple layers of trees,
and this operation will be captured by performing node substitution on this datum. 
This story will be made rigorous in this section and the next.
}%%% COLOR: BLUE
...
(as $\Omega$ has no coproducts, the free cocartesian monoidal category is the coproduct completion).
...
\begin{tikzcd}
	\Omega_G \ar{r} \ar{d} & \Phi^G \arrow[d, "r^G"]
\\
	\mathsf{O}_G \ar{r} & \Fin^G
\end{tikzcd}
\end{equation}
(where $r: \Phi \to \Fin$ is the root functor),
...
is the closure of the relations in $\leq_d$
and those in $\leq_p$ internal to every
$a^{\uparrow}$
...
Further, unpacking the construction in (\ref{OGDEF EQ}), one sees that the pullback of the $G$-tree (or $G$-forest)
...
with planar structure pulled back from $T$ (in the sense of Remark \ref{PULLPLANAR REM}).
\end{definition}
...
We now label another class of important maps in $\Omega$. 
\begin{definition}
...
\todo[inline]{is it important to relate node substitution to iterated grafting?}
...
is \textit{equivalent} to the data of said planar tall maps.
% can equivalently be repackaged using planar tall maps. 
We begin by formalizing the above process.
...
This section builds off the previous, introduces two functors that are central to our definition of the category $\mathsf{Op}_G$ of
...
\begin{remark}\label{NEED_WREATH_REMARK}
      ...
      is equal on objects to the map from Remark \ref{VERTEXDECOMPG REM}.
      However, the variance here is reversed. 
      Indeed, in the non-equivariant case the vertex functor can be defined to land instead in $\Sigma \wr \Sigma$,
      where the variance is irrelevant.
	...
	For a simple example of this observation and to demonstrate the functoriality and variance of $V_G$,
...
To describe the monad structure on $\mathbb{F}_G$, however, we will find it preferable to separate the left Kan extension step in (\ref{FGXDEF EQ})
from the remaining construction.
As such, we will 
build a monad $N$ (Proposition \ref{MONSPAN PROP})
...
\]
where both $V_G(T_k)$ and $T_{n,v_{G f}}$ are ordered lexicographically according to the associated strings of maps out of (subtrees of) $T_0$.
...
and $\tilde{T} =(T,(A_{v_{Ge}})_{V_G(T)}) \in \Omega_G^0 \wr A$ over $X$,
...
$\bar{\psi} \colon \psi^{\**}T \to T$
and $\bar{\psi}_{G e}$ denotes the restrictions
...
By Proposition \ref{MONADADJ PROP}, we thus have the following.
...
While showing (iv), we will slightly abuse notation by writing 
...
\end{tikzpicture}
\]
However, when pulling these points back to the $G$-free stump corolla $G \cdot C_0$ one obtains the same point
in $\mathbb F_G \iota_{\**} Y(G \cdot C_0)^{\Gamma} = \iota_{\**} \iota^{\**} \mathbb F_G \iota_{\**} Y(G/G \cdot C_0)$,
...
\begin{definition}
	...
	if an arrow $f \colon d \to s$ in $\mathcal{D}$ has
...
For technical reasons, we prefer a ``doubled'' bar construction
...
\Lan N^{(\mathcal P, X, Y)}_{n,l} =: \mathsf{Lan} N (N^{n} \iota \mathcal{P} 
...
In this section, we build towards Remark \ref{NPXY_REM}, where we explicitly identify the categories underlying the left Kan extensions in (\ref{LANLEVELFOR EQ}).
...
a partition of $\underline{l} = \{1,2,\cdots,l\}$.
...
\begin{equation}
      ...
      \end{tikzcd}
\end{equation}
% We denote the top horizontal composite as the morphism $N^{(A_i)}_{n,l}$
% (and note that this agrees with the notation from (\ref{LANLEVELFOR EQ})).
...
\begin{definition}
      Let $\tilde N^{(\mathcal P,X,Y)}$ denote the (a priori, set) map
      \[
            \Omega_G^e \to \mathsf F \wr \Sigma_G^{\amalg\set{\mathcal P, X,Y}} \xrightarrow{\mathcal P, X, Y}
            \mathsf F \wr \V^{\amalg 3} \xrightarrow{\amalg}
            \mathsf F \wr \V \to \V.
      \]      
\end{definition}
...
which identifies $\tilde N^{(\mathcal P,X,Y)}_{\bullet,\bullet}$ with $\tilde N^{(\mathcal P,X,Y)}$.
\end{proposition}
...
\begin{corollary}
      $\mathcal P \limits\coprod_{\mathbb F X} \mathbb F Y \simeq \Lan_{(\Omega_G^e \to \Sigma_G)^{op}}\tilde N^{(\mathcal P, X,Y)}$. \qed
\end{corollary}
...
where the $U_i^X$ and $U_j^Y$ are simply the $X$- and $Y$-labeled nodes,
...
$\mathcal{P}$ level genuine cofibrant in $\mathsf{Sym}^G(\V)$,
and
$u \colon X \to Y$ a level genuine (trivial) cofibration in $\mathsf{Sym}^G(\V)$,
...
$(G \times \phi)(\Gamma) \in \mathcal{F}_n$
...
was also obtained by Gutierrez-White in \cite{GW}.
...
We start with an abstract argument, which has also been used by Gutierrez-White in \cite{GW}. 
Writing 
...
and thus so is the latching map of the composite square, showing that $l_{\underline{n_+1}} X^{n+1}$
...
one has $\pi_0 (\iota_{\**} \mathcal{O})\in \mathsf{Op}_G(\mathsf{Set})$. Therefore, if $\mathcal{O}$ has fixed points as in (\ref{NFINFTY EQ}) then 
