...
%%%%%%%%%%%%%%%%%%%%%%% ALL THE PROJECTIVE MODEL STRUCTURES
% %%%%%%%%%%%%%%%%%%%%%%%

% \begin{definition}
%   \label{SUBD_PROJ_F_MODEL_STRUCTURE}
%   Suppose we are given a small category $\mathcal D$, a subcategory ${\bar{\mathcal D}^{op}} \subseteq \mathcal D$, and a collection $\F_d$ of subgroups of $\mathrm{Aut}(d)$ for each $d\in \bar{\mathcal D}^{op}$.
...
% ---- $Sym_G^F$
% ---- $Op_G^F$
% ---- All that stuff about barD-projective F bullshit, sans the stuff I need about genuine thingies
...
\todo[inline]{come back: combine with the above. Refine. Bring back the Quillen equivalence from before.}
...
\begin{definition}
        [cf. \ref{FGXDEF EQ}, \ref{THEMONAD DEF}]
...
\begin{definition}
        ...
        \item \textit{projective weak equivalence} (resp. \textit{projective fibration})
                ...
                % \item \textit{projective cofibration} iff it has the left lifting property against all maps which are both projective weak equivalences and projective fibrations.
...
\todo[inline]{come back: can also define the $\bar\F$-model structure on $\Op_\F$ for any $\bar \F \subseteq \F$, exactly analogously.}
...
\todo[inline]{come back: move to before Lemma \ref{MAINLEM LEM}}
