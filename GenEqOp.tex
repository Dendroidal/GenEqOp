...
\end{equation}
with generating broad relations the relations $f^{\uparrow} \leq f$ in $T$ for $f$ satisfying (\ref{OUTERFACE EQ}) but $\forall_i f\neq e_i$,
...
\item[(d)] any pair of broad relations $\underline{g} \leq v$, $\underline{f}v \leq e$ induces a \emph{grafting pushout diagram}
...
are planar tall.
But it now follows that (\ref{ASSEMBLYGRAFT EQ}) is a grafting pushout diagram (\ref{GRAPTPUSH EQ}), so that the pushout indeed exists. The conditions that
...
\begin{proof}
	...
	\left(F(c_i)\right).
	\end{equation}
As the canonical factorizations of each $(\varphi,(f_i))\colon (d_i)_{i \in I} \to (k c_j)_{j \in J}$ as
...
\begin{remark}\label{PULLCOMP REM}
	...
	are compatible with \textit{root pullbacks} of trees. More concretely, any pullback $\pi \colon S=\varphi^{\**} T \to T$ induces pullbacks 
...
$\varphi^{\**}C \to C$
under precomposition (where we note that $\phi^*C \in \Sigma_G^{\text{fr}}$).
\end{remark}
...
\end{tikzcd}
\end{equation}
{\color{blue} for any collection $\set{A_j}_{j\in\underline{l}} \subset \mathcal C$, }
...
Arrows in $\Omega_{G,n,s}^{J}$ are strings of quotients
...
\[
N \circ \amalg \circ (N^{\times J})^{\circ \bullet} \underline{A} = 
\coprod_J^a N^{\bullet +1} A_j \amalg^a
\coprod_{\underline{l}-J}^a N A_j
\]
{\color{blue} for a tuple of $N$-alegebras $\underline(A)$.}
...
\begin{equation}\label{TARGETDEFINITON EQ}
	...
	[0,i'_m].
\end{equation}
To check compatibility with composition, % associativity, 
...
also preserves cofibrant objects follows 
