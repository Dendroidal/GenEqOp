...
Informally, an operad $\O$ consists of a sequence of
sets/spaces $\O(n)$ of ``$n$-ary operations'' carrying a $\Sigma_n$-action
(recording ``reordering of the inputs of the operations''),
...
$E_\infty$-operads are characterized by the homotopy type of their levels $\O(n)$: $\O$ is $E_\infty$ if and only if each $\O(n)$ is $\Sigma_n$-free and contractible. That is, for each subgroup $\Gamma \leq \Sigma_n$ one has
\[
...
where a map of $G$-spaces $X \to Y$ is deemed a $G$-equivalence only if
...
is obtained via the identification
$H \simeq \Gamma_X$.
%
...
$\Omega(C) \to \Omega(T)$
so that,
writing $\mathsf{Op}_{\bullet}$ for the category of colored operads,
\eqref{COMPEX EQ}
...
a forest with edges labels that indicate the $G$-action.
Note that all edges are conjugate to one of the 
edges $a,b,c,d$ which have, respectively, stabilizers $L, K, K, H$.
For example, the labels of $T$ imply that $\pm i d = \pm k d$ and $\pm j d = \pm d = d$.
...
for $\O \in \mathsf{Op}^G$ a $G$-operad,
the composition 
...
We recall the precise connection between $T$ and \eqref{INTFIXPTCOMP EQ}. 
Let $\mathsf{Op}_{\bullet}^G$
be the category of 
$G$-objects in colored operads.
...
\index{categories!SigmaF@$\Sigma_{\mathcal F} \hookrightarrow \Sigma_G$}
and partial $G$-symmetric sequences
$\mathsf{Sym_{\mathcal{F}}}(\mathcal{F})
=
\mathcal{V}^{\Sigma_{\mathcal{F}}^{op}}$
\index{categories!SymF@$\mathsf{Sym_{\mathcal{F}}}(\mathcal{F})=\mathcal{V}^{\Sigma_{\mathcal{F}}^{op}}$}
which are suitably compatible with the monad
$\mathbb{F}_G$,
thus giving rise to categories
$\mathsf{Op}_{\mathcal{F}}(\mathcal{V})$
of \textit{partial genuine equivariant operads}
\index{categories!OpF@$\mathsf{Op}_{\mathcal{F}}(\mathcal{V})$}.
...
This article is part of a series of papers by the authors \cite{Pe17,BP_edss,BP_HGOP,BP_TAS}
producing and analyzing different models for the homotopy theory of equivariant operads with norm maps.
These papers generalize the non-equivariant program of Cisinski, Moerdijk, and Weiss \cite{MW07,MW08,CM11,CM13a,CM13b}.
A major result is the existence of a Quillen equivalence
\begin{equation}
      \label{DSETSOP_EQ}
      \begin{tikzcd}[column sep =4em]
...
model category of equivariant dendroidal sets encoding $G$-$\infty$-operads, i.e. ``colored $G$-operads with norms up to homotopy'', from \cite[Theorem 2.1]{Pe17}, and 
$\mathsf{sOp}^G$ is the category of equivariant many-colored simplicial operads equipped with the many-colored variant of the model structure in Theorem \ref{MAINEXIST1 THM} from \cite[Theorem III]{BP_HGOP}.
In particular, this is a generalization of the equivalence between the homotopy theories of simplicial categories and quasicategories.
More details can be found in the introduction to \cite{BP_HGOP}.
...
As mentioned above, \cite[Theorem III]{BP_HGOP} is an extension of Theorem \ref{MAINEXIST1 THM} to the colored setting.
Moreover,
% Nonetheless,
we conjecture 
the many-colored variants of Theorems \ref{MAINEXIST2 THM} and \ref{MAINQUILLENEQUIV THM}
also hold,
...
equivariant setting:
while usual equivariant colored operads have $G$-sets of objects,
genuine equivariant colored operads will instead have \textit{coefficient systems} of objects.
Furthermore, one can show that
there is a genuine equivariant colored operad encoding single-colored genuine equivariant operads,
analogous to the colored operad encoding single-colored operads from e.g. \cite{GV12}. 

Genuine equivarinat operads and $G$-trees play a prominent role
throughout the authors' program describing the homotopy theory of equivariant operads with norms.
In every model category from \cite{Pe17,BP_edss,BP_HGOP,BP_TAS},
(co)fibrations and weak equivalences are described explicitly using $G$-trees.
Moreover, 
while the monadic description of genuine equivariant (colored) operads used here
is particularly useful for the model categorical considerations of our main results,
a combinatorial presentation of this algebraic structure is more suited in \textit{loc cite}.
Recalling that a category (resp. operad) is equivalent, under a nerve functor, to a strict inner Kan complex in $\sSet$ \cite[Prop. 1.1.2.2]{Lu09} (resp. $\dSet$ \cite[Prop. 5.3, Thm. 6.1]{MW08}),
in Theorem \ref{NERVE THM} we show that
a genuine equivariant operad is equivalent, under a nerve functor $\Op_G \to \dSet_G = \Set^{\Omega_G^{op}}$,
to a strict inner Kan complex (cf. \cite[Remark 3.42]{BP_edss}).
The combinatorial structure underlies
key steps in the proofs of the main results from each of \cite{Pe17,BP_edss,BP_HGOP,BP_TAS},
for example \cite[Def. 5.8]{BP_edss} and \cite[Prop. 4.47]{BP_TAS}.
% a combinatorial variant of a genuine equivariant operads
% arises in each of \textit{loc cite}, encoding revelant and necessary structure for the proofs.
% ---------- As in \cite{BP_edss}, we delay the proof of this nerve theorem to a sequel.
% Similarly, $G$-operads are strict inner Kan complexes in $\dSet^G$ \cite[Remark 3.42]{BP_edss}. 
% However, as elaborated on in \cite[Remark 3.42]{BP_edss},
% this is \textit{not} an appropriate strictification of $G$-$\infty$-operads in $\dSet^G$:
% $G$-$\infty$-operads are defined as those presheaves in $\dSet^G$ with the right lifting property against
% inner horn inclusions for all $G$-trees,
% while for strict inner Kan complexes on $\dSet^G$, the non-free $G$-trees are all superfluous.

Additionally, we expect there to be a ``genuine'' analogue
\[
	\begin{tikzcd}[column sep =4em]
		\mathsf{dSet}_G \ar[shift left=1.5]{r} 
	&
		\mathsf{sOp}_G,
            \ar[shift left=1.5]{l}
      \end{tikzcd}
\]
of the Quillen equivalence from \eqref{DSETSOP_EQ}.\\
...
Let $\mathcal{T}$
denote the colored operad whose algebras are operads, 
as defined in {\color{red} Gutierrez-Vogt}.

In terms of our language, 
the colors of $\mathcal{T}$
are the arities $C \in \Sigma$,
and an operation with signature
$(C_1,\cdots,C_n;C_0)$
consists of a tree $T$, a permutation 
$\sigma \in \Sigma_n$
such that $V(T) = (C_{\sigma(i)})$
and a tall map $C_0 \to T$.


Their construction can be modified to construct a 
$G$-equivariant colored operad $\mathcal{T}_G^{fr}$,
as follows.
Operations are now the $G$-free corollas 
$C \in \Sigma_G^{fr}$
and operations with signature $(C_1,\cdots,C_n;C_0)$
are encoded by $G$-free trees $T$,
a permutation $\sigma \in \Sigma_n$ such that
such that $V_G(T) = (C_{\sigma(i)})$
and a tall rooted map $C_0 \to T$.
Moreover, the $G$-action on 
$\mathcal{T}_G^{fr}$
is described on objects by
$gC = g (C_i)_{i \in I} = (C_{g i})_{i \in I}$
and similar by
$gT = g (T_i)_{i \in I} = (T_{g i})_{i \in I}$
on operations
(some caution is needed concerning the permutation
$\sigma$, since while the vertices of $gT$ are conjugate to the vertices of $T$, they do not share the same order).

{\color{red} Note:} The operad $\mathcal{T}_G^{fr}$
does \emph{not} include ``$G$-action operations''. 
Instead, both $\mathcal{T}_G^{fr}$ and its algebras 
come with prescribed $G$-actions, 
which then impose equivariance conditions on the algebra structure maps.


One then has a map of $G$-colored operads
(where $G$ acts trivially on $\mathcal{T}$)
\[
	\mathcal{T} \to \mathcal{T}_G^{fr}
\]
Moreover, upon forgetting the $G$-action,
this map is fully faithful and essentially surjective.
However, this map is not $G$-essentially surjective.
More precisely, 
for any $\** \neq H \leq G$
one has that the induced map on $H$-fixed points
...
$\mathcal{H}_n(X) = \mathcal{C}(X^{\otimes n}, X)$


For coefficient system consisting of fixed points,
an algebra structure is a map
$\mathcal{P} \to \iota_{\**} \mathcal{H}_{\bullet}(X)$.
...
The techniques and machinery developed in this paper have
applications and extensions in a number of topics
outside the homotopy theory of equivariant operads.

First, this work identifies additional structure on the homotopy groups of an $\O$-algebra $X$
for any $G$-operad $\O$ and $G$-space $X$:
While it is clear that $X^H$ is an algebra over $\O^H$, and hence
$\pi_{\**}(X^H)$ is an algebra over $\pi_{\**}(\O^H)$,
the genuine equivariant operad paradigm tells us that in fact there are ``twisted structure maps'',
as
the coefficient system $\pi_{\**}(\iota_{\**}X)$ is an algebra over the genuine equivariant operad $\pi_{\**}(\iota_{\**}\O)$.

\begin{example}
      Let $G = \mathbb Z/2 = \set{-1,1}$ be the cyclic group of order 2,
      and consider the three $G$-trees below;
      the orbital representations are displayed above the extended representations. 
      \begin{equation}
            \begin{tikzpicture}
                  ...
                  child{node [dummy] {}
                    child{node [dummy,fill=black] {}
                      edge from parent node [swap, near end] {$G/e$}
                    }
                    child{node [dummy,fill=black] {}
                      edge from parent node [near end] {$G/e$}
                    }
                    edge from parent node [swap] {$G/e$}
                  };
                  % ----------
                  \node at (5,0) {$G/G \cdot C_2$}%
                  child{node [dummy] {}
                    child{node [dummy,fill=black] {}
                      edge from parent node [swap, near end] {$G/G$}
                    }
                    child{node [dummy,fill=black] {}
                      edge from parent node [near end] {$G/G$}
                    }
                  ...
                  child{node [dummy] {}
                    ...
                    edge from parent node [swap] {$t$}
                  };
            \end{tikzpicture}
      \end{equation}
      For any simplicial $G$-operad $\O \in \sOp^G$ and $\O$-algebra $X$,
      these $G$-tree diagrams encode structure maps,
      by placing elements of $\O(2)$ (resp. $X$) of appropriate isotropy in the root vertices (resp. stumps).
      The first two trees encode the algebra structure maps
      \[
            \O(2) \times X^{\times 2} \to X,
            \qquad
            \O(2)^{\Gamma_{G/G \amalg G/G}} \times (X^G)^{\times 2} \to X^G
      \]
      (where we note that $\Gamma_{G/G \amalg G/G} = G \leq G \times \mathbb Z/2$, and hence
      the right equation is the $G$-fixed points of the left).
      This last $G$-tree encodes the twisted structure map (cf. \eqref{INTFIXPTCOMP EQ})
      \[
            \O(2)^{\Gamma_{G/e}} \times X \longto X^G,
            \qquad
            (\phi,x) \longmapsto \phi(x, -x).
      \]
      % given by the composite
      % \[
      %       \O(2)^{\Gamma_{G/e}} \times X \xrightarrow{\mathrm{id} \times \Delta_{\mathbb Z/2}}
      %       \O(2)^{\Gamma_{G/2}} \times (X \times X)^{\Gamma_{G/2}} \to
      %       X^G
      % \]
      % where $\Delta_{\mathbb Z/2}$ sends $x$ to $(x, -x)$.
      This induces a twisted action map on homotopy groups, for example
      \[
            \pi_0\left(\O(2)^{\Gamma_{G/e}}\right) \times \pi_0(X) \to \pi_0 \left(X^G \right).
      \]
\end{example}

A full discussion of algebras over genuine equivariant operads is forthcoming.

{\color{OliveGreen}
  Let $\Omega_{G, \mathrm{Cl}}^{0,{\P,Y}}$ denote the full subcategory of closed (no leaves) 2-labeled $G$-trees, with $\lambda_a = \set{\P}$ and $\lambda_i = \set{Y}$, such that all inert vertices are stumps.
  Then, for $\P$ a genuine $G$-operad and $Y$ a coefficient system,
  the \textit{free $\P$-algebra on $Y$}, $\mathbb F_\P(Y)$, is given by the opposite of the right Kan extension
  \[
        \begin{tikzcd}
              \Omega_{G, \mathrm{Cl}}^{0,\set{\P,Y}} \arrow[r, "\boldsymbol{V}_G^{\set{\P,Y}}"] \arrow[d, "\mathsf{r}"]
              &
              \mathsf F_s \wr \Sigma_G \times \mathsf F_s \wr \mathsf O_G \arrow[r, "{(\P,Y)}"]
              &
              \mathsf F_s \wr \V^{op} \times \mathsf F_s \wr \V^{op} \arrow[r, "\otimes"]
              &
              \V^{op}
              \\
              \mathsf O_G \arrow[urrr, "\mathbb F_\P(Y)^{op} = \Ran"']
        \end{tikzcd}
  \]
}
...
Second, much of the machinery in this paper, presenting the free operad monad and its equivariant variants,
can be applied in different contexts to define new equivariant algebraic notions.
It is used in \cite{BP_HGOP} to construct a many-color variant of Theorem \ref{MAINEXIST1 THM}.
Moreover, as previewed in \cite{BNerve},
there is a subcategory $\Sym_G \to \Op_G$ of ``genuine symmetric monoidal categories'', constructed by
blending the description of $\mathbb F_G$ used here {\color{red} reference} with
the monad $\Sigma \wr (-)$ encoding symmetric monoidal categories (cf. Remark \ref{SIGMA_WR_REM}).
Our formal investigation of $\Sym_G$ is in progress.
Furthermore, we conjecture that this bookkeeping can be applied to produce
genuine equivariant analogues of properads and other algebraic theories.


\todo[inline]{transition}

Finally, the comparison between simplicial $G$-operads $\sOp^G$ and the parametrized $G$-$\infty$-operads of \cite{BDGNS}
...
$\O^\otimes \to \Fin_{\**}$
equipped with a functor to the category of pointed finite sets, which acts like a fibration over a certain wide subcategory,
...
In \cite{BNerve}, the first-named author generalizes this process,
by first building from a genuine equivariant operad $\P$ a
simplicial category
...
%
This has been applied by \cite{Hor} in the case where $\O = \mathcal D_V$ is the equivariant little disks operad over a $G$-representation $V$.
Specifically, Horev shows \cite[\S 3.9]{Hor} that $N^\otimes(\mathcal D_V)$ is equivalent to the
...
%    Applications:
%   \begin{enumerate}
%   \item \sout{algebraically: if $X$ is an $\O$-algebra in $G$-spaces, then $\pi_{\**}X$ is an $\pi_{\**}\O$-algebra;
%         but we have more structure: $\pi_{\**}\iota_{\**}X$ is a $\pi_{\**}\iota_{\**}\O$ algebra}
%   \item \sout{any time $G$-operads with norms appear, $G$-trees appear:
%         e.g. each model for $G$-$\infty$-operads, as in \cite{BP_HGOP,BP_TAS,Pe17,BP_edss},
%         have weak equivalences defined and applied using $G$-tree manipulations}
%   \item \sout{the analogue combinatorial notion of a strict genuine equivariant inner Kan complex in $\sdSet_G$
%         appears in every paper}
%   \item \sout{$\Op_G(\sSet)$ are the natural source for the genuine equivariant generalization of the operadic nerve $\Op(\sSet) \to \Op_\infty$, used in {\color{red} Horev} for equivariant factorization homology}
%   \item \sout{$\Op_G(\Set)$ has a good notion of symmetric monoidal category}
%   \end{enumerate}

%   Context:
%   \begin{enumerate}
%   \item \sout{$hcN \colon \sOp^G \to \dSet^G$ is a Quillen equivalence.}
%   \end{enumerate}

  
%   Future Work:
%   \begin{enumerate}
%   \item \sout{formally defining $\P$-algebras}
%   \item \sout{formally defining many-colored variants}
%   \item \sout{$hcN \colon \sOp_G \to \dSet_G$ is a Quillen equivalence.}
%   \item \sout{more work with $\Sym_G$.}
%   \end{enumerate}

%   Machine applications:
%   \begin{enumerate}
%   \item \sout{This machine is applied in \cite{BP_HGOP}}
%   \item \sout{This machine/presentation of the free operad monad can be used in great generality to define algebraic notions,
%         including properads, cyclic operads, modular operads, symmetric monoidal categories, etc., 
%         simply by substituting the category $\Omega$ for the category which encodes the combinatorics of the desired multiplication,
%         and checking that the technical results (regarding grafting, substitution data, planar strings, etc) hold to
%         give a closed-form categorical description of the iterations
%         % describe iteration as strings of (well-behaved) maps in the original category.
%         - WARNING ABOUT CYCLIC OPERADS: the 'operads' generated by trees are no longer finite, and/or the inclusion of ``cyclic trees'' into cyclic operads is not fully-faithful}
%   \item \sout{Similarly, we expect the bookkeeping used here will also be helpful to produce appropriately genuine equivariant models for these algebraic structures.}
%   \item \sout{EXAMPLE: $\Sym_G$}
%   \end{enumerate}
% }
...
In this section we assume that the monoidal structure 
on $\mathcal{V}$
...
\begin{theorem}\label{NERVE THM}
	...
	i.e. the $X \in \mathcal{V}^{\Omega_G^{op}}$
...
{\color{red} note that can't do this the usual way since we have no colored genuine operads}


\begin{remark}
	\eqref{SEGCOND EQ} implies
	that
	$X(G/H \cdot \eta) = \**$ for all $H \leq G$,
	since
	$V_G(G/H \cdot \eta) = ()$ is the empty tuple,
	motivating the term ``pointed''.
	If one allows for non-pointed $X$
	(i.e. for $X(G/H \cdot \eta) \neq \**$),
	the Segal condition \eqref{SEGCOND EQ} needs to be modified.
\end{remark}


The proof of Theorem \ref{NERVE THM}
will follow by first building $\mathcal{N}$,
and then describing a partial inverse
(when restricted to pointed Segal objects).



To build the nerve 
$\mathcal{N} \mathcal{P} \in \mathcal{V}^{\Omega_G^{op}}$
for $\mathcal{P} \in \mathsf{Op}_G(\mathcal{V})$,
recall first that 
$\iota \mathcal{P}$
is a $N$-algebra
and consider the bar construction
$B_{\bullet}=B_{\bullet}(N,N,\iota \mathcal{P})
=N^{n+1} \iota \mathcal{P}$,
whose levels are spans of the form
$\Sigma_G
\leftarrow 
\Omega^n_G \xrightarrow{N^{\mathcal{P}}_n}
\mathcal{V}^{op}$
where
\[
	N_n^{\mathcal{P}} 
	\left(T_0 \to \cdots \to T_n\right)
	\simeq
	\prod_{v \in V_G(T_n)} \mathcal{P}(T_{n,v})
\]
Moreover, the face maps $d_i$ of $B_{\bullet}$
for $i < n$ are induced by the multiplication 
$NN \Rightarrow N$,
and thus their natural transformation component is an isomorphism.
Hence, by Proposition \eqref{RANTRANS PROP},
and noting that
$|\Omega^n_G| = \Omega^{\mathsf{t}}_G$
is the category of $G$-trees and tall maps,
one obtains upon realization
a functor
$\mathcal{N}^{\mathsf{t}} \colon 
\Omega_G^{\mathsf{t}} \to \mathcal{V}^{op}$.
This construction almost defines the desired
nerve functor in Theorem \eqref{NERVE THM},
except we have a functor out of 
the subcategory $\Omega_G^{\mathsf{t}}$ 
rather than the entire category $\Omega_G$.
To address this, 
we first enlarge the string categories
$\Omega^n_G$
to categories
$\overline{\Omega}^{n}_G, n \geq 0$
whose objects are still strings but whose maps between strings are 
the the maps which are component wise outer maps
(note that quotients are componentwise isomorphisms).
Note that the 
$\overline{\Omega}^{n}_G$ still form a simplicial object, i.e.
one still has simplicial operators
$d_i \colon \overline{\Omega}^{n,o}_G \to \overline{\Omega}^{n,o}_G$
for $0\leq n, 0 \leq i \leq n+1$
and 
$s_j \colon \Omega^{n,o}_G \to 
\overline{\Omega}^{n+1}_G$
for $0\leq n, 0 \leq j \leq n$,
though we note that one no longer has an augmentation
to $\Sigma_G$ nor extra degeneracies $s_{-1}$.
It is clear that one now has
$|\overline{\Omega}_G^{n}| = \Omega_G$
so that, to build
$\mathcal{N} \colon \Omega_G \to \mathcal{V}^{op}$,
it suffices to show that
$B_{\bullet}$
can be extended to a simplicial object
$\overline{B}_{\bullet}$
whose levels are functors 
$\overline{\Omega}_G^{n} \xrightarrow{N_n^{\mathcal{P}}} \mathcal{V}^{op}$.
This follows by noting that
the vertex functors
$V_G \colon \overline{\Omega}^{n}_{G}
\to 
\Fin_s \wr \Omega^{n-1}_{G}$
extend to functors
$V_G \colon \overline{\Omega}^{n,o}_{G}
\to 
\Fin \wr \Omega^{n-1}_{G}$.
This now follows from the observation that the following diagrams
commute
(note that all the simplicial operators 
of $B_{\bullet}$
are naturally factored
so that the leftmost
is extended by one of the diagrams below).
\begin{equation}
...
\begin{definition}
	Let $\mathcal{P} \in \mathsf{Op}_G(\mathcal{V})$.
	The \emph{nerve}
	$\Omega_G \xrightarrow{\mathcal{N} \mathcal{P}} \mathcal{V}^{op}$
	is the realization of the simplicial object
	$\overline{\Omega}_G^{n} \xrightarrow{N_n^{\mathcal{P}}} \mathcal{V}^{op}$.
\end{definition}
...
We now turn towards the task of building a partial inverse to $\mathcal{N}$.

Suppose now one is given
$X \colon \Omega_G \to \mathcal{V}^{op}$
whose opposite satisfies the pointed 
strict Segal condition \eqref{SEGCOND EQ}.
Letting $\mathcal{P}$ be the composite
$\Sigma_G \to \Omega_G \to \mathcal{V}^{op}$,
we will show that $\mathcal{P}$
admits a genuine operad structure for which
$X \simeq \mathcal{N}\mathcal{P}$.
Moreover, 
noting that $\mathcal{N}$ is actually built from 
the span $\iota \mathcal{P}$,
we will instead show that 
$\iota \mathcal{P}$ admits a $N$ algebra structure.

Throughout, 
we will make repeated use of the functors 
$\overline{\Omega}^n_G \to \Omega_G$
and
$\Omega^n_G \to \Omega_G$
given by
$(T_0 \to \cdots \to T_n) \mapsto T_n$,
and will leave these functors implicit.
Note that the maps $T_{n-1} \to T_n$ induce transformations
...
\[
	...
	\right).
\]
Note that $X_{-1} = \iota \mathcal{P}$.
Moreover, note that the transformations \eqref{SIMP EQ}
make $X_n$ into a simplicial object in spans.

Next, note that one has natural transformations $\epsilon_n$
\begin{equation}
\begin{tikzcd}
...
\\
	...
	\Fin \wr \Omega_G 
\arrow[Leftarrow, from=Vt, to=FV, shorten <=0.10cm,shorten >=0.10cm,"\epsilon_n"]
\end{tikzcd}
\end{equation}
which,
...
$\epsilon_n$
determines a map of spans
$X_n \xrightarrow{\epsilon_n} N X_{n-1}$.

Moreover,
the claim that $X$ satisfies the 
strict Segal condition \eqref{SEGCOND EQ}
is equivalent to 
the maps of spans
$X_n \xrightarrow{\epsilon_n} N X_{n-1}$
being isomorphisms (of spans).
The key to proving essential surjectivity 
is now the claim that 
$X_{-1}$ becomes a $N$-algebra with multiplication given by
\begin{equation}\label{XMINUSMULT EQ}
\begin{tikzcd}
	N X_{-1} &
	X_0 \ar{l}[swap]{\epsilon}{\simeq} \ar{r}{d_0} &
	X_{-1} 
\end{tikzcd}
\end{equation}
The claim that this defines 
an algebra structure is the claim that the outer paths
from $NN X_{-1}$ to $X_{-1}$ below coincide.
\begin{equation}\label{ALGCHECK EQ}
\begin{tikzcd}
	NNX_{-1} \ar{rr}{\mu}&&
	NX_{-1}
\\
	NX_0 \ar{u}{N \epsilon}[swap]{\simeq} \ar{d}[swap]{Nd_0} &
	X_1 \ar{l}[swap]{\epsilon}{\simeq}  \ar{d}{d_1} \ar{r}{d_0} &
	X_0 \ar{d}[swap]{d_0} \ar{u}{\epsilon}[swap]{\simeq}
\\
	N X_{-1} &
	X_0 \ar{r}{d_0} \ar{l}[swap]{\epsilon}{\simeq} &
	X_{-1} 
\end{tikzcd}
\end{equation}
It now remains to check that all squares in \eqref{ALGCHECK EQ}
commute, with commutativity of the bottom right square being tautological.
For the bottom left square in \eqref{ALGCHECK EQ},
note that, up to whiskering with
$\Fin \wr \Omega_G \to \mathcal{V}^{op}$,
the composites 
$X_1 
\xrightarrow{d_1} X_0 
\xrightarrow{\epsilon} N X_{-1}$
and
$X_1  
\xrightarrow{\epsilon} N X_0 
\xrightarrow{N d_0} N X_{-1}$
are described by the diagrams below
\begin{equation}
\begin{tikzcd}
...
&
	...
	\Fin \wr \Sigma_G \ar{r} &
	\Fin \wr \Omega_G 
\arrow[Leftarrow, from=Vt, to=FV,shorten <=0.10cm,shorten >=0.10cm,"\epsilon"]
\arrow[Leftarrow, from=Vtt, to=FVt,shorten <=0.10cm,shorten >=0.10cm,"m"]
\arrow[Leftarrow, from=Vt2, to=FV2,shorten <=0.10cm,shorten >=0.10cm,"m"]
\arrow[Leftarrow, from=Vtt2, to=FFV, shorten <=0.10cm,shorten >=0.10cm,"\epsilon"]
\end{tikzcd}
\end{equation}
...
\arrow[Leftarrow, from=dog2, to=cat2,shorten <=0.15cm,,shorten >=0.15cm,"\epsilon"]
\end{tikzcd}
\end{equation}
We now consider the following, where all terms other than the 
new $\Fin \wr \mathcal{V}^{op}$ term on the top row
retain their relative positions from \eqref{INTERM EQ}.
\begin{equation}
\begin{tikzcd}[column sep=1.05em]
...
\eqref{INTERM2 EQ})
with 
$\Fin \wr \Omega_G \to \Fin \wr \mathcal{V}^{op} \to 
\mathcal{V}^{op}$.
It now follows that 
$X_1 \xrightarrow{\epsilon} 
N X_0 \xrightarrow{N \epsilon}
NN X_{-1} \xrightarrow{\mu} N X_{-1}$
and
$X_1 \xrightarrow{d_0} X_0 \xrightarrow{\epsilon} N X_{-1}$
...
\arrow[Leftrightarrow, from=FFOmega, to=Omega,shorten <=0.15cm,,shorten >=0.15cm,"\pi_0"]
\arrow[Leftarrow, from=dog2, to=cat2,shorten <=0.15cm,,shorten >=0.15cm,"\epsilon"]
\arrow[Leftarrow, from=dog, to=cat,shorten <=0.15cm,,shorten >=0.15cm,"\epsilon"]
\arrow[Leftarrow, from=Vt, to=FV, shorten <=0.10cm,shorten >=0.10cm,"\epsilon"]
\end{tikzcd}
\end{equation}
The fact that the composites in 
...
We have now shown that \eqref{ALGCHECK EQ} commutes,
i.e. that \eqref{XMINUSMULT EQ}
indeed makes $X_{-1} = \iota \mathcal{P}$ into a $N$-algebra.

{\color{red} HERE}

\begin{proof}[Proof of Theorem \ref{NERVE THM}]
	Let $\mathcal{P} \in \mathsf{Op}_G(\mathcal{V})$,
	and set $X = \mathcal{N} \mathcal{P}$.
	By construction of the realization process,
	one has that the composite
	$\Omega^0 \to \Omega 
	\xrightarrow{\mathcal{N} \mathcal{P}} \mathcal{V}$
	is  
	$\Omega^0 \xrightarrow{N_0^{\mathcal{P}}} \mathcal{V}$,
	from which it follows that
	$X_{-1} = \iota \mathcal{P}$
	and that 
	$X_0 \xrightarrow{\epsilon} NX_{-1}$
	in \eqref{XMINUSMULT EQ}
	is the identity.
	Moreover, again by construction of the realization process,
...
&
	...
	\mathcal{V}^{op} 
\arrow[Leftarrow, from=Vt, to=FV, shorten <=0.10cm,shorten >=0.10cm,"m"]
\arrow[Leftrightarrow, from=Vt2, to=F1, shorten <=0.10cm,shorten >=0.10cm,"\mu"]
\arrow[Leftarrow, from=F2, to=FV2, shorten <=0.10cm,shorten >=0.10cm,"\varphi_1"]
\end{tikzcd}
\end{equation}
Hence, the natural transformation
$X_0 \xrightarrow{d_0} X_{-1}$
is given by the composite natural transformation
\begin{equation}
	\begin{tikzcd}[column sep = 30pt]
...
\end{tikzcd}
\end{equation}
which, by the simplical relations of $\mathcal{N}_{\bullet}$,
is simply the operator 
$\mathcal{N}_0 \xrightarrow{d_0} \mathcal{N}_{-1}$,
i.e. the multiplication in $\iota \mathcal{P}$.
In other words, we have shown that 
\eqref{XMINUSMULT EQ}
indeed recovers $\mathcal{P}$.

{\color{red} HERE}

Conversely, 
suppose $X \in \mathcal{V}^{\Omega_G^{op}}$ satisfies the Segal condition and consider $X_{-1}$ with its operad structure.
We need to show that $X$ and $\mathcal{N} X_{-1}$
are naturally isomorphic.
Namely, we claim that
$\epsilon_0 \colon X \Rightarrow \mathcal{N} X_{-1}$
is such a natural transformation.
Since $\Omega^0 \to \Omega 
\xrightarrow{\mathcal{N} \mathcal{P}} \mathcal{V}$
is  
$\Omega^0 \xrightarrow{\mathcal{N}_0} \mathcal{V}$,
naturality holds on arrows 
of $\Omega$ that are in the image of $\Omega^0$
it thus suffices to check naturality on arrows 
in the image of $m$ in \eqref{EXPREAL EQ}.
But now note that the right side of \eqref{EXPREAL EQ}
is computing
$NX_{-1} \overset{\mu}{\leftarrow}
NNX_{-1} \overset{\epsilon}{\leftarrow}
NX_0 \xrightarrow{Nd_0}
NX_{-1}$,
which by 
\eqref{ALGCHECK EQ}
coincides with
$NX_{-1} \overset{\epsilon}{\leftarrow}
X_0 \overset{d_0}{\leftarrow}
X_1 \xrightarrow{d_1}
X_0 \xrightarrow{\epsilon}
N X_{-1}$.
Noting that the natural transformation underlying $d_0$ is the identity, the result follows.
\end{proof}
...
\index{GrothendieckFibration@$\pi \colon \mathcal{E} \to \mathcal{B}$ (Grothendieck fibration)}
if for every arrow 
$f \colon b' \to b$ in $\mathcal{B}$
and $e \in \mathcal{E}$ such that $\pi(e)=b$,
there exists a \emph{cartesian arrow} 
$f^{\**} e \to e$ lifting $f$,
\index{Grothendieck FibrationCartesian@$f^{\**} e \to e$}
...
(over $\mathcal B$)
\index{categories!GrothendieckConstruction@$\mathcal{B} \ltimes \mathcal{E}_{\bullet}$}
...
\end{equation}
\index{Ran@$\mathsf{Ran}_{\pi}F$ (right Kan extension)}  
...
\index{categories!undercategory@$\bar{e} \downarrow_{\mathcal{B}} \mathcal{E} \hookrightarrow \bar{e} \downarrow \mathcal{E}$}
...
\index{categories!Fin@$\Fin$}denote the usual skeleton of the category of (ordered) finite sets and all set maps. Explicitly, its objects are the finite sets $\{1,2,\cdots,n\}$ for $n\geq 0$.


\begin{definition}
	For a category $\C$, we write 
	$\Fin \wr \C := (\Fin^{op} \ltimes (\C^{op})^{\times \bullet})^{op}$
        \index{categories!FinWrC@$\Fin \wr \C$}
...
\Sigma_{\lambda_k}$
\index{SigmaLambda@$\Sigma_{\lambda}$}
...
\begin{remark}
      If $\mathcal{V}$ has all finite coproducts, then
      injections and fold maps assemble into a functor as on the left below.
      Dually, if $\mathcal{V}$ has all finite products,
      then projections and diagonals assemble into a functor as on the right.
...
\begin{remark}\label{FINSURJ REM}
	...
	$\Fin_s \wr \mathcal{V} \xrightarrow{\otimes} \mathcal{V}$
        \index{$\Fin_s \wr \mathcal{V} \xrightarrow{\otimes} \mathcal{V}$}
	satisfying the analogues of  
	\eqref{COHER EQ}, \eqref{COHER2 EQ}, \eqref{COHER3 EQ}.
...
This has the effect of providing $\mathsf F^G = \mathsf F \wr \mathsf O_G$ with canonical choices for many constructions
(for example, $\mathsf F^G$ is a \textit{bipermutative} monoidal category).

We will find it convenient to use a model of $\Omega$ with similar extra structure, given by planar structures on trees.
As desired, this provides us with canonical choices for many constructions
(e.g. Example \ref{ROOTPULL EX}, Proposition \ref{BUILDABLE PROP}, Remark \ref{LEAFROOTEXAMP REM}, Definition \ref{VG DEF}, Propositions \ref{SUBSASPULL PROP}and \ref{PIIPROP PROP}, \dots).
...
$e^{\uparrow} \leq e$\index{Broadvert@$e^{\uparrow} \leq e$}.
...
$\underline{e} \leq e$\index{Broadvert@$\underline{e} \leq e$} holds 
...
\label{PLANARPULL PROP}
	Let $F \xrightarrow{\varphi} F'$ be an independent map in $\Phi$ (or $\Omega$, $\Omega_G$, $\Phi^G$). Then there is a unique factorization 
...
\begin{notation}\label{UEUPE NOT}
	...
	as simply
	$U_{e^{\uparrow} \leq e}$\index{UEUPE@$U_{e^{\uparrow} \leq e}$}.
\end{notation}
...
\begin{remark}\label{WHYALT REM}
	...
	then the maps $S_{v_{Ge}} \to S$ need to be planar on each tree component of $S_{v_{Ge}}$ but need not respect the order of the roots of $S_{v_{Ge}}$.
	This is because the tuple
	\eqref{UEUPEG EQ} 
	...
	reorded according to $G \varphi(e)$.
...
%$S_{(\minus)}^{\mathsf{r}}$ and $S'_{(\minus)}$ notations
%(cf. Remark \ref{WHYALT REM})).
...
%\[
%\begin{tikzcd}
%	(G^{op} \times \Omega_1)^{(\iota^{\**} A)}	\ar{ddd} \ar{rd} \ar{rrr} &&&
%	\Fin \wr (G^{op} \times \Omega_0)^{(\iota^{\**} A)} \ar{rd} \ar[dashed]{ddd} 
%\\
...
%\\
%	G^{op} \times \Omega_0	\ar{dd} \ar{rd} \ar[dashed]{rrr}&&&
%	\Fin \wr (G^{op} \times \Sigma) \ar[dashed]{rd}
%\\
...
%	one has that the monads
%	$\mathbb{F}_G$ and $\mathbb{F}$ are identified
%	over the essential image of $\iota_!$.
\end{remark}
...
For now, we end this section with a minimal counterexample to the  claim that \eqref{KEYNONISO EQ} 
is an isomorphism.
...
%This filtration will allow for a homotopical study of these free extensions, which combined with the transfer principle of Kan \cite[Thm. 11.3.2]{Hi03} will allow us to prove Theorems \ref{MAINEXIST1 THM} and \ref{MAINEXIST2 THM}, in \S \ref{MAINEXIST SEC}.
...
We note that, as genuine operads are hard to describe explicitly,
...
% $\mathcal{V}^{G}_{\text{gen}}$
% and the $\F$-model structure for a family $\V^G_\F$
...
% More generally, given a family $\mathcal{F}$ (or even collection of subgroups) of $G$, there then also exists a model structure 
% $\mathcal{V}^{G}_{\mathcal{F}}$ with weak equivalences, fibrations and generating (trivial) cofibrations all described by restricting $H$ to 
% $\mathcal{F}$.
% \end{remark}
...
\begin{proof}
      ...
      %of $\mathsf{res}^{\bar G}_{\phi(G)}(\bar G/H)$ as
      % $= \mathsf{Res}^{\bar G}_{\phi(G)}\mathsf{Ind}_{H}^{\bar G}(H/H)$.
...
\begin{remark}\label{CSPP_REM}
	...
	(e.g. 
	%the \textit{commutative monoid axiom} of
	\cite[Def. 3.1]{Wh17},
	% $(\clubsuit)$ of
	\cite[Thm. 6.1.1]{WY18},
	% and
	% \textit{symmetric $h$-monoidal} of
	\cite[Def. 2.1]{PS18b}).
	
	% along with the \textit{monoid axiom} of \cite{SS00} and some small technical conditions,
	% It is quite restrictive, and several more general conditions have been developed
	% (see e.g. \cite[Thm. 6.1.1 $(\clubsuit)$]{WY18}, \cite[Defn. 2.1 and Thm. 1.1]{DS18})
	% % ---------- these are both versions of the power cofibration axiom AND the monoid axiom 
	% which accomplish these tasks.
	...
	such as the ones in Theorem \ref{MAINEXIST1 THM}.
      ...
      % We replace the power cofibration axiom % and the monoid axiom
      % with
      % cofibrant symmetric pushout powers, % and the \textit{global monoid axiom} of \cite[Defn. 3.6]{BP_HGOP}, respectively, both
      % designed explicitly to work in the context of genuine equviariant homotopy theory.

      % In the semi-model structure case,
      % the monoid axiom is no longer necessary,
      % as we can reduce to Proposition \eqref{BIQUILLENG PROP} instead.
      % This is reflected by
      % the difference in the axioms $(\clubsuit)$ verses $(\spadesuit)$ in \cite{WY18},
      % and addressed in \cite[Remark 1.16]{BP_HGOP}.
      ...
      % or the more relaxed \textit{commutative monoid axiom} of \cite[Def. 3.1]{Wh17}.
      ...
      % % analogues of the results from, for example, \cite{Wh17,WY18}.
...
% ----- proof for reduced operads and genuine symmetric sequences -----
%when $G = G \times \Sigma_T$ and $\mathcal{F} = \mathcal{F}_T$ 
%\[
%\phi_! \iota_{\**}(G \cdot_H C)
%=
%\coprod_{[H\text{-maps } C \to T]/\sim}
%{X(T)}^H
%\]
%\[
%\iota_{\**} \phi_! (G \cdot_H C)
%=
%\left(\coprod_{[\text{maps } C \to T]/\sim}
%{X(T)}\right)^H
%\]
%here both $\sim$ are isomorphism classes determined along isomorphisms $T\simeq T'$. The reason why coproducts, rather than limits, can be employed is because the resulting undercategory is a a groupoid with contractible components (provided trees have no stumps).
%The crux is that in the fixed points only the summands corresponding to $H$-maps survive (in fact, in the reduced case, the $H$-structure on $C$ and the map $C\to T$ determine at most one $H$-structure on $T$)
