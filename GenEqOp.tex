...
Further, pullbacks with respect to $\mathsf{r}$ are called
...
\begin{definition}\label{SUBSTITUTIONDATUM}
	...
	by $\mathsf{Sub}(T)$ (resp. $\mathsf{Sub}_p(T)$).
\end{definition}

\begin{definition}
	Let $T \in \Omega$ be a tree. 
	The \textit{Segal core poset $\mathsf{Sc}(T)$} is the poset with objects the single edge subtrees $\eta_e$ and vertex substrees $T_{e^{\uparrow} \leq e}$, ordered by inclusion.
\end{definition}
...
\item[(ii)] for the datum $\{T_{e^{\uparrow}\leq e}\}$, it is $T = \colim_{\mathsf{Sc}(T)} T_{(\minus)}$;
...
\begin{definition}\label{VG DEF}
	Given $T = (T_x)_{x \in X} \in \Omega_G$ we define the set $V_G(T)$ of 
	\textit{$G$-vertices} of $T$ to be the orbit set $V(T)/G$, i.e. the quotient of the vertex set 
	$V(T) = \coprod_{x \in X} V(T_x)$ by its $G$-action.
	
Furthermore, we will regard $V_G(T)$ as an object of $\Fin$ by equipping it with its lexicographic order: i.e. vertex equivalence classes $[e^{\uparrow} \leq e]$ are ordered according to the planar order $\leq_p$ of the smallest representative $ge$, $g \in G$.
\end{definition}
