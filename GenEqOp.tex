...
\todo[inline]{LUIS: these definition of ``$G$-graph subgroup'' and ``$G$-vertex family'' probably belong earlier in the paper, possibly the introduction. Though it could also make sense to mention them in the intro, and formally define them here} 
...
We will define $\F$-model structures on $\V\Sym_G$ and $\V\Op_G$ for any $G$-vertex family $\F$, given the following assumptions about our base category $\V$: 
...
\item \textit{level $\F$-cofibration} if $f(C)$ is a cofibration in $\V^{\mathrm{Aut}(C)}_{gen}$ for all $C\in \Sigma_\F$. 
\end{enumerate} 
...
\end{tikzcd} 
\] 
where $u: X \to Y$ is a genuine level trivial cofibration and $\P$ is genuine level cofibrant in $\V^{\Sigma_G^{op}}_{gen}$, $\P \to \P[u]$ is a level cofibration, trivial if $u$ is. 
\end{corollary} 
...
\begin{proof}[Proof of Theorem \ref{GENUINE_SEMI_VOP_G_THM}] 
 This follows by general arguments of Kan \cite[11.6.1]{Hi03} from Proposition \ref{CELLULAR_LEVEL_COFIB_PROP}, as transfinite compositions are constructed levelwise, and all genuine (trivial) cofibrations in $\V^{\Sigma_G^{op}}$ and $\V\Op_G$ are so levelwise. 
\end{proof} 
...
\begin{proof} 
  This follows from Corollary \ref{CELLULAR_LEVEL_COFIB_PROP}. Indeed, it suffices to show that if $u: X\to Y$ is a level trivial $\F$-cofibration and $\P$ is level $\F$-cofibrant in $\V^{\Sigma_G^{op}}$, then the cellular extension $\P \to \P[u]$ is a level trivial $\F$-cofibration. However, such $u$ are level genuine cofibrations, and such $\P$ are level genuine cofibrant, and hence $\P \to \P[u]$ is a level trivial genuine cofibration by said corollary, and thus is a level trivial $\F$-cofibration. 
\end{proof} 
...
\begin{theorem} 
 If genuine cofibrations in $\V^\Pi$ are underlying cofibrations in $\V$ for all finite groups $\Pi$, and every object in $\V$ is cofibrant, then the $\F$-model structure on $\V\Op_G$ exists. 
\end{theorem} 


\begin{proof} 
 In these cases, the technical hypotheses on $\P$ in Corollary \ref{CELLULAR_LEVEL_COFIB_PROP} are always satisfied. 
\end{proof} 
...
\section{Cofibrancy}\label{COFIB SEC}
...
\begin{remark}
	...
	$\mathsf{O}_{\mathcal{F}} \subset \mathsf{O}_G$
	consisting of the orbital $G$-sets $G/H$ for 
	$H \in \mathcal{F}$.
	
	Furthermore, $\mathsf{O}_{\mathcal{F}}$ has the property of being a sieve of $\mathsf{O}_G$, i.e., for any map $G/K \to G/H$ in $\mathsf{O}_G$ such that $G/H \in \mathsf{O}_{\mathcal{F}}$ it is also $G/K \in \mathsf{O}_{\mathcal{F}}$. Moreover, it is straightforward to show that families as in Definition \ref{FAMILY DEF} are in fact in bijection with such sieves.
\end{remark}

\begin{remark}
	We note that for any fixed group $G$, families form a lattice, with the order given by inclusion, and meet and join given by intersection and union, respectively.
\end{remark}

We now recall the following fundamental notion and result.

{\color{blue} add Stephan reference}

\begin{definition}\label{CELL DEF}
	$\mathcal{V}$ is said to have \textit{cellular fixed points} if:
\begin{itemize}
	\item[(i)] fixed points $(\minus)^H$ preserve direct colimits;
	\item[(ii)] fixed points $(\minus)^H$ preserve pushouts where one of the legs is $(G/K)\cdot f$, for $f$ a cofibration;
	\item[(iii)] for each object $A \in \mathcal{V}$, the natural map 
	$(G/K)^H \cdot A \to ((G/K) \cdot A)^H$
	is an isomorphism.
\end{itemize}
\end{definition}
...
\begin{theorem}
	...
	\textbf{$\mathcal{F}$-model structure}
	such that both weak equivalences and fibrations are determined by the fixed points $(\minus)^H$ for 
	$H \in \mathcal{F}$.
\end{theorem}

We will also make use of the following strengthenings of
conditions (ii) (iii) in Definition \ref{CELL DEF}.

\begin{remark}\label{GENPIXEDPUSH REM}
If $\mathcal{V}$ is cofibrantly generated, then
(i) and (ii) in Definition \ref{CELL DEF} imply that fixed points $(\minus)^H$ preserve pushouts where one of the legs is a $G$-genuine cofibration.
\end{remark}


\begin{lemma}
	If $\mathcal{V}$ is cofibrantly generated and has cellular fixed points then fixed points $(\minus)^H$
	preserve any pushout diagram where one leg is a $G$-genuine cofibration.
\end{lemma}

\begin{proof}
	This is an immediate consequence of (i) and (ii) by writing
	the cofibration as a retract of a transfinite composition of pushouts of generating cofibrations. 
\end{proof}


\begin{lemma}\label{FIXEDDIAG LEM}
	Suppose $\mathcal{V}$ is strongly cofibrantly generated, 
	has cellular fixed points and is a closed monoidal model category.
	
	Then for $C,D \in \mathcal{V}^G$ genuinely cofibrant the canonical map
\begin{equation}\label{FIXEDTENSORISO EQ}
	C^H \otimes D^H \to (C \otimes D)^H	
\end{equation}
is an isomorphism.
\end{lemma}
...
\begin{proof}
	Considering first the case $C = G/H \cdot A$, where $A$ is cofibrant in $\mathcal{V}$,
	the properties in Definition \ref{CELL DEF} ensure that the $D$ 
	satisfying the condition that (\ref{FIXEDTENSORISO EQ}) is an isomorphism are closed under cellular extension,
	and thus include all cofibrant objects.
	But now the same argument shows that the $C$ satisfying that
	(\ref{FIXEDTENSORISO EQ}) is an isomorphism for each fixed cofibrant $D$ are themselves closed under cellular extension, and the result follows.  
\end{proof}


\begin{lemma}\label{FIXCOF LEM}
	If $\mathcal{V}$ is cofibrantly generated and has cellular fixed points then each of the fixed point functors $(\minus)^H$
	preserve cofibrations and pushouts where one of the legs is a cofibration.
\end{lemma}

\begin{proof}
	This follows by Definition \ref{CELL DEF}: the case of generating cofibrations follows by (iii), their pushouts follow by (ii) and transfinite compositions by (i).
\end{proof}


\begin{lemma}\label{PUSHFIX LEM}
	Suppose $\mathcal{V}$ is strongly cofibrantly generated, 
	has cellular fixed points and is a closed monoidal model category.
	
	If $f,g$ are $G$-genuine cofibrations between $G$-genuine cofibrant objects, then the canonical map
\[
	f^H \square g^H \to (f \square g)^{H}
\]
is an isomorphism.
\end{lemma}
...
\begin{proof}
	...
	Lemmas \ref{FIXEDDIAG LEM} and \ref{FIXCOF LEM}.
\end{proof}
...
\begin{lemma}\label{STROIII LEM}
	Suppose that $\mathcal{V}$ is cofibrantly generated and has cellular fixed points and that $A \in \mathcal{V}^{H}$ is genuinely cofibrant. Then the natural map
\begin{equation}\label{STROIII EQ}
	(G/H)^K \cdot A^K
		\to
	(G \cdot_H A)^K
\end{equation}
...
\begin{proof}
	The isomorphism (\ref{STROIII EQ}) is obvious in the case of $A$ a set, and thus follows from (iii) in Definition \ref{CELL DEF} for $A$ either the domain or codomain of a generating cofibration $(H/L)\cdot f$. (i) and (ii) in Definition \ref{CELL DEF} now show that the property that (\ref{STROIII EQ}) is an isomorphism is preserved by both pushouts of generating cofibrations and transfinite composition.
	The result follows by writing $\emptyset \to A$ as a retract of a transfinite composition of pushouts of generating cofibrations.
\end{proof}

We now list some key properties of $\mathcal{F}$-model structure that we will use throughout.




\begin{proposition}\label{FGTRIGHT PROP}
	Let $\phi \colon G \to \bar{G}$ be a homomorphism and $\mathcal{V}$ as above.
...
\begin{equation}
	\mathsf{fgt}
	\colon
	\mathcal{V}^{\bar{G}}_{\mathcal{\bar{F}}}
		\rightleftarrows
	\mathcal{V}^{G}_{\mathcal{F}}
	\colon
...
\[
	...
	\right)	\cdot f
\]
it is immediate that the left adjoint preserves cofibrations and trivial cofibrations.
\end{proof}
...
\[
	\phi^{\**} (\bar{\mathcal{F}}) \subset \mathcal{F}
\Leftrightarrow
	\left( \underset{H \leq G,\phi(H) \in \bar{\mathcal{F}} }{\forall} 
	H \in \mathcal{F} \right)
\Leftrightarrow
	\left( \underset{\bar{H} \in \bar{\mathcal{F}}}{\forall}
	\phi^{-1}(\bar{H}) \in \mathcal{F}
	\right)
\Leftrightarrow
	\left( \underset{\bar{H} \in \bar{\mathcal{F}},\bar{g} \in \bar{G}}{\forall}
	\phi^{-1}(\bar{H}^{\bar{g}}) \in \mathcal{F}
	\right)
\Leftrightarrow
	\phi^{\**} (\bar{\mathcal{F}}) \subset \mathcal{F}
...
\begin{proposition}\label{BIQUILLENG PROP}
	Suppose that $\mathcal{V}$ is as above and also a closed monoidal model category.
...
\begin{equation}
	(G/H \cdot f) \square (G/\bar{H} \cdot g)
		\simeq
	(G/H \times G/\bar{H}) \cdot (f \square g)
		\simeq
	\left(
...
We defined their \textit{external intersection} to be the 
...
That (\ref{EXTERINTADJ EQ}) is a left Quillen bifunctor (and its obvious higher order analogues) is one of the key properties of pushout products of $\mathcal{F}$ cofibrations when those cofibrations (and well as the group) are allowed to change. However, when those cofibrations (and hence also $G$) coincide there is an additional symmetric group action that must be considered.

To handle such actions we introduce the following axiom

\begin{definition}
	We say $\mathcal{V}$ has 
	\textit{cofibrant symmetric pushout powers}
	if for each cofibration $f$ the pushout product 
	$f^{\square n}$ is a genuine $\Sigma_n$-cofibration.
\end{definition}
...
We define the \textit{$n$-th semidirect power of $\mathcal{F}$} to the family of $\Sigma_E \wr G = \Sigma_E \ltimes G^{\times E}$ given by
\begin{equation}\label{FLTIMESN EQ}
	\mathcal{F}^{\ltimes E}
		=
	\left(
	\iota_{ \Sigma_{\{e\}} \times G \times \Sigma_{E-e}\wr G}
	\right)_{\**}
	\left(
		\left(
...
More explicitly, noting that in (\ref{PHISTARDEF3 EQ}) one needs only consider conjugates by coset representatives of $\bar{G}/\phi(G)$, it follows that when computing $\iota$ one needs only consider conjugates by permutations of $E$ not fixing $e$, so that one has that
\begin{equation}\label{FLTIMESN2 EQ}
	K \in \mathcal{F}^{\ltimes E} 
	\text{ iff }
	\underset{e \in E}{\forall} \pi_{G}
	\left(
		K \cap \Sigma_{\{e\}} \times G \times \Sigma_{E-e} \wr G
	\right)
	\in \mathcal{F},
\end{equation}
showing that in particular the definition in (\ref{FLTIMESN EQ})
...
\begin{proposition}
	...
	\mathcal{F}^{\ltimes \bar{E}}
		\subset
	\iota^{\**}(\mathcal{F}^{\ltimes E \amalg \bar{E}}).
	\end{equation}
...
\pi_G
\left(
	K \cap \Sigma_{\{e\}} \times G
		\times
	\Sigma_{E \amalg \bar{E} - e} \wr G
\right)
	=
\pi_G
\left(
...
$\mathcal{F}^{\ltimes E}$. The case of $e \in \bar{E}$ is identical.

(\ref{LTIMESPRODQUI EQ}) simply combines 
(\ref{EXTERINTADJ EQ}) and 
...
\begin{proposition}\label{POWERF PROP}
	Suppose that $\mathcal{V}$ has cellular fixed points and is a closed monoidal model category.
...
Our proof of Proposition \ref{POWERF PROP} will be essentially a repetition of the main argument in the proof of
{\color{green} Pe16 Thm1.2}.
However, both for the sake of completeness and to stress that the argument is independent of the (fairly technical) model structures in {\color{green}, Pe14}, we include an abridged version of the proof below, the key ingredient 
of which is that (\ref{LTIMESPRODQUI EQ}) is a left Quillen bifunctor. 

\begin{proof}
	We first note that in the case of 
	$i = (G/H) \cdot \bar{i}$, $H\in \mathcal{F}$, a generating (trivial) cofibration it is 
	$i^{\square n} = (G/H)^{\times n} \cdot \bar{i}^{\square n}$, and thus this case follows since the $\Sigma_n \wr G$-orbits of $(G/H)^{\times n}$ are in $\mathcal{F}^{\ltimes n}$.
...
Additional details concerning this filtration appear in the proof of {\color{green} Pe16 Lemma 4.8}.

The general proof now follows by writing $f$ as a retract of a transfinite composition of pushouts of generating (trivial) cofibrations. As usual, retracts can be ignored, and we can hence assume that there is an ordinal $\kappa$
...
\[
\begin{tikzcd}
		Q^n(\bar{f}_{1}) \ar{d}[swap]{\bar{f}_0^{\square n}} \ar{r}
	&
		Q^n(\bar{f}_{2}) \ar{d}[swap]{\bar{f}_1^{\square n}} \ar{r}
	&
		Q^n(\bar{f}_{3}) \ar{d}[swap]{\bar{f}_2^{\square n}} \ar{r}
	&
		Q^n(\bar{f}_{4}) \ar{d}[swap]{\bar{f}_3^{\square n}} \ar{r}
	&
		\cdots \ar{r}
	&
...
Condition (ii) above implies that this map is an isomorphism for $\beta$ a limit ordinal and the map
$Q^n(\bar{f}_{\beta+1})
\amalg_{Q^n(\bar{f}_{\beta}) }
X_{\beta}^{\otimes n} 
	\to
X_{\beta+1}^{\otimes n} 
$ for $\beta+1$ a successor. But since 
...
(cf. {\color{green} Pe16 Lemma 4.11}) and the cofibrancy of $i_{\beta}^{\square k}$ proven at the beginning.
\end{proof}
...
Y^{\otimes n}.
\]
It then follows that $K$ acts on the diagram category 
$(0 \to 1)^{\times n}$ itself (via the composite
$K \to \Sigma_n \wr G \to \Sigma_n$).
The fixed diagram subcategory 
$\left((0 \to 1)^{\times n}\right)^K$
consists of those tuples in $\{0,1\}^{n}$
...
\begin{example}
When $n=3$ and $n/K = \left\{\{1,2\},\{3\}\right\}$
$(0 \to 1)^{n/K}$ is identified with the subposet on the right below.
\[
...
and then computing a pushout over that smaller cube (either before or after taking fixed points of each level).
...
\begin{proposition}
	Suppose that $\mathcal{V}$ has cellular fixed points and is a closed monoidal model category with symmetric pushout powers.
...
all maps in (\ref{FIXEDPUSH EQ}) are isomorphisms.
\end{proposition}
...
one thus obtains isomorphisms
\begin{align*}
	\left(
	\Sigma_n \underset{\Sigma_{n-i} \times \Sigma_i}
	{\cdot}\left( X^{\otimes n-i} \otimes f^{\square i} \right)
	\right)^K
\simeq &
...
\otimes 
	...
	\right)
\end{align*}
where the first step is an instance of Lemma \ref{STROIII LEM}, 
the second step an instance of
Lemma \ref{PUSHFIX LEM},
and the last step follows by combining Lemma \ref{FIXEDDIAG LEM}
with the induction hypothesis (since $|B|\leq i <n$).

We have thus shown that the leftmost map in the pushouts (\ref{COMPNFOLDFACTPUSH EQ}) for 
$\left( f^{\square n} \right)^K$
is isomorphic to the leftmost map in the corresponding pushout for 	
$\underset{[i] \in n/K}{\mathlarger{\mathlarger{\square}}}
\left( f^{\otimes [i]} \right)^K$,
...
\subsection{$G$-graph families and trees}

We note that in this section we use $\Sigma$ to denote a general group.

\begin{definition}
A subgroup $\Gamma \leq G \times \Sigma$ is called a
\textit{$G$-graph subgroup} if $\Gamma \cap \Sigma = \**$. 

Further, a family $\mathcal{F}$ of $G \times \Sigma$ is called a \textit{$G$-graph family} if it consists only of $G$-graph subgroups.
\end{definition}

\begin{remark}\label{GRAPH REM}
One can show that $\Gamma$ is a $G$-graph subgroup iff it can be written as
\[
	\Gamma = 
	\{
	...
	\}
\]
for some partial homomorphism $G \geq K \xrightarrow{\varphi} \Sigma$.
\end{remark}
...
\cap
\left(
\Sigma_{\{e\}}\times (G \times \Sigma) \times
 \Sigma_{E-\{e\}} \wr (G \times \Sigma)
\right)
=
G \times \Sigma_{\{e\}} \times \Sigma \times
 \Sigma_{E-\{e\}} \wr \Sigma
 \]
 one has  
\begin{equation}\label{FLTIMESN2G EQ}
	...
	\left(
		K \cap 
		G \times \Sigma_{\{e\}} \times \Sigma \times \Sigma_{E-e} \wr \Sigma
	\right)
	\in \mathcal{F}.
\end{equation}
\end{remark}

Combining either of (\ref{EXTERINTADJ EQ}) or 
...
\begin{remark}
	...
	$\mathcal{F} \sqcap_G \bar{\mathcal{F}}$
	is indeed a $G$-graph family of $G \times \Sigma \times \bar{\Sigma}$, note that it is \textit{not} the case that
	$\mathcal{F}^{\ltimes_G n}$ is a $G$-graph family of
	$G \times \Sigma_n \wr \Sigma$, due to the need to consider the power $\Sigma_n$-action.
\end{remark}



The $G$-graph families we will be interested in will encode certain families of $G$-trees. We start by with the case of corollas.



\begin{definition}
	A \textit{family} $\Sigma_{\mathcal{F}}$ of $G$-corollas is a sieve 
	$\Sigma_{\mathcal{F}} \subset \Sigma_G$, i.e., a full subcategory such that for any morphism $T \to T'$ with $T' \in \Sigma_{\mathcal{F}}$ it is also $T \in \Sigma_{\mathcal{F}}$.
	
	Equivalenty, the data of a family of corollas $\Sigma_{\mathcal{F}}$ consists of
	$G$-graph families $\mathcal{F}_n$ of $G \times \Sigma_n$ for each $n \geq 0$ with $C \in \Sigma_{\mathcal{F}}$ precisely if $C \simeq G \cdot_H C_H$ for $C_H \in \Sigma_n^H$ an $H$-equivariant corolla encoded by a partial homomorphism $G \geq H \to \Sigma_n$ encoding a subgroup in $\mathcal{F}_n$ 
...
The notion of indexing system in Definition \ref{INDEXSYS DEF} matches the eponymous notion first introduced in {\color{green} BH}, though packaged quite differently.
...
\begin{remark}
	In the language of {\color{green} BH}, the isomorphism classes $\Sigma_{\mathcal{F}}$ are instead encoded by 
...
coincides with Lemma 5.9 of {\color{green} BerMoer}. Moreover, it is not hard to leverage that non-equivariant result to provide a proof of Proposition \ref{AUTTCOFPUSH PROP} for the universal family $\Sigma_G$ of all $G$-corollas. However, the reader of {\color{green} BerMoer} may note that the proof therein is technically easier, using no analogue of the rather subtle $\mathcal{F}^{\ltimes_G n}$ families. Indeed, this is reflected 
...
that would suffice for the direct generalization of {\color{green} BerMoer} suggested above.
...
(described in the $H$-set language of {\color{green} BH}) by the $G$-set $G/\Sigma_3$.
...
\begin{theorem}
  \label{GENUINE_SEMI_VOP_G_THM_REP}
  The genuine semi-model structure on $\V\Op_G$ exists for all $\V$ satisfying \textsc{Assumption 1}.
\end{theorem}
\begin{proof}
...
We can now prove a proposition, modeled on the proof of \cite[Theorem 2.10]{Ste16}.
\begin{proposition}
