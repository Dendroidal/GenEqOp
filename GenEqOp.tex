...
Our first result, proved in Section \ref{G_OP_EXISTS_SECTION}, shows that $G$-operads can be endowed with a (semi) model structure encoding the theory of operads with norm maps. In fact, it shows more, in particular that the partially-genuine (semi) model structures also exist.
...
The necessary condition, found in Section \ref{MAINTHM_PROOF_SECTION} that $\mathcal \F$ is a \textit{weak} indexing system,
is also precisely the necessary condition from Section \ref{F_GEN_OP_SECTION} to build the new algebraic structures of genuine equivariant operads.
...
In Section \ref{OP_G_EXISTS_SECTION}, we show these categories of algebras have a projective model structure.
...
there exists a projective semi model structure on $\mathsf{Op}_{\mathcal F}(\V)$, determined by the forgetful functor
\[
...
Our main result, proved in Section \ref{MAINTHM_PROOF_SECTION}, says both notions of equivariant operads are Quillen equivalent.
...
\subsection{Indexing systems and $\F$-genuine $G$-operads}
\label{F_GEN_OP_SECTION}
\label{INDEXING_SECTION}

Recalling $N_\infty$-operads from the introduction, we observe that they arose from an insight, 
that the fully genuine $G$-$E_\infty$-operads of \cite{CW91} and the $G$-trivial $E_\infty$-operads
were not the only options for equivariant commutativity,
but instead they were the maximum and minimum respectively of an entire lattice of ``partially genuine'' options.

Similarly, the genuine $G$-operad monad $\mathbb{F}_G$ and the regular $G$-operad monad $\mathbb{F} \simeq \iota^{\**} \mathbb{F}_G \iota_{!}$ are not the only options for equivariant operads, 
but instead there is an additional lattice of ``partially genuine operads'',
with each choice having a description as some ``restriction'' of (\ref{FGXDEF EQ}), as already seen in (\ref{IFGI EQ}). 

In this subsection, we will explore this second lattice. We begin by identifying certain useful subcategories of $\Sigma_G$.

\begin{definition}
      \label{COROLLA_FAMILY_DEF}
	A \textit{family} $\Sigma_{\mathcal{F}}$ of $G$-corollas is a \textit{sieve} subcategory
...
\begin{remark}
	...
	\[C \simeq G \cdot_H C_e\]
        ...
        \todo[inline]{deal with reference to graph subgroups}
	(cf. Remark \ref{GRAPH REM}). 
\end{remark}

\todo[inline]{come back: the following paragraph - where does it belong?}
Since $\Sigma_{\mathcal{F}}$ is determined by the families $\{\mathcal{F}_n\}_{n \geq 0}$, 
we will abuse notation and abbreviate either set of data simply as $\mathcal{F}$ 
...
\begin{remark}
      The definition below will actually not immeditely be of a form analogous to (\ref{FGXDEF EQ}). 
      This is in part because the \textit{existence} of such a monad is in fact more general, and because the given definition will make cleaner both the gynmastics with adjoints required to compare it with $\mathbb F_G$ and the homotopical analysis.
      However, we can only actually understand $\mathbb F_\F$ and compare it to $\mathbb F_G$ when $\F$ is a weak indexing system, and in this case, the monad is indeed (isomophic to one) given by an analogous left Kan extension (see (\ref{F_F_LAN_EQ})).
\end{remark}

For any family of corollas $\F$, let $\upgamma: \Sigma_{\F} \into \Sigma_G$ denote the inclusion.
...
\begin{remark}
      \label{UPGAMMA_REM}
      For any $\F$, the forgetful map $\upgamma^{\**}$ is easy to describe explicitly. 
      ...
      \end{cases}
      \]
      However, $\upgamma_{\**}$ remains mysterious\footnote{
        ...
        while $\upgamma_{\**}$ is only very rarely \textit{not} mysterious (e.g. for $G \into O_G$).}.      
...
If $\F$ is not a weak indexing system, our discussion must end here.
However, 
...
%       While we could have defined it as this endofunctor, and just repeated Section \ref{FMON SEC} mutatis mutandis, 
...
\begin{remark}
      ...
      is an indexing system, and the proof of Proposition \ref{MONAD_COMPARISON_PROP}(i) yields that $\mathbb F \simeq \mathbb F_{\F_{\text{fr}}}$.
\end{remark}
...
\begin{equation}\label{EXTTREEFOR EQ}
	\mathcal{P} \coprod_{\mathbb{F} X} \mathbb{F} Y 
\simeq 
...
In particular, we identify a smaller
subcategory 
$\bar{\Omega}_G^e \hookrightarrow \Omega_G^e$
that is $\mathsf{Lan}$-final, 
so that $\Omega_G^e$ can be replaced with $\bar{\Omega}_G^e$
...
\begin{definition}
	$\bar{\Omega}_G^e \hookrightarrow \Omega_G^e$ is the full subcategory of $(\mathcal{P},X,Y)$-labeled trees
...
imply that for any map in $\bar{\Omega}_G^e$
...
\begin{proposition}\label{LXP PROP}
	...
	$\mathsf{lr}_{\mathcal{P}} (U) \in \bar{\Omega}_G^e$ together with a unique planar label map in $\Omega_G^e$
	\[\mathsf{lr}_{\mathcal{P}} (U) \to U.\]
	Furthermore, $\mathsf{lr}_{\mathcal{P}}$ extends to a right retraction 
	$\mathsf{lr}_{\mathcal{P}} \colon \Omega_G^e \to \bar{\Omega}_G^e$.
\end{proposition}
...
(\ref{LRXDEF EQ}) into a label map, but we must still check $T \in \bar{\Omega}^{e}_G$, i.e. that 
...
$S$, $T$ are thus both trees in $\bar{\Omega}_G^e$ obtained from $F$ by adding degenerate $\mathcal{P}$ vertices, 
...
in (\ref{LRPFUN EQ}), where we want $S \in \bar{\Omega}_G^e$
...
\begin{example}
The following illustrates the $\mathsf{lr}_{\mathcal{P}}$ construction when applied to the map $\varphi$ in (\ref{REGALTERNMAP EQ}).
\begin{equation}
\begin{tikzpicture}[grow=up,auto,level distance=2.3em,
every node/.style = {font=\footnotesize},
dummy/.style={circle,draw,inner sep=0pt,minimum size=2.25mm}]
...
In order to encode the homotopical information inspired by $N_\infty$-operads dicussed in the introduction, 
we will introduce aand transfer several associated (semi) model structures on the categories $\Op_G(\V)$ and $\Op^G(\V)$ for a wide range of $\V$. 
As identified in, for example, \cite{SS00}, analysis of free extensions is paramount when transferring model structures.
Summarizing the results in the previous section, we have that the free $\mathbb{F}_G$-extension $\P[u]$ defined by the pushout
...
$(\bar{\Omega}_{G}^e)^{op} \xrightarrow{\mathsf{lr}} \Sigma_G^{op}$. 
Now,
so as to study the homotopical properties of the map $\P \to \P[u]$, we follow standard practice 
(e.g \cite{SS00, Spitz01, BM03, Har09, Whi14, WY15, HP15, Pe16}) 
and identity a suitable filtration of this map, which will in turn be induced by a suitable filtration of the extension tree category $\bar{\Omega}_{G}^e$.
...
Intuitively, $|T|_X$ counts the number of $X$-labeled vertices in a single tree component of $T$.


\begin{remark}
	One of the key properties of the degrees $|T|$, $|T|_X$, $|T|_Y$ just defined is that they are invariant under root pullback.
\end{remark}


\begin{definition}\label{TREE_FILTRATION_PIECES_DEFINITION}
We specify some rooted subcategories of $\bar{\Omega}_{G}^e$:
  \begin{enumerate}
...
\begin{remark}\label{LIMMOR REM}
  The categories $\bar{\Omega}_G^e[k]$ and $\bar{\Omega}_G^e[k,-]$ have rather limited morphisms.
  ...
  Moreover, we have a clear rooted isomorphism of categories $\bar\Omega_G^e[k,0] \simeq \Omega_G^a[k]$. 
\end{remark}
...
\begin{tikzpicture}
  ...
  child{node [dummy,fill=black] {}%
    child{edge from parent node [swap]{$i$}}
    child{edge from parent node [near end] {$h$}}
...
Now, suppose we are given any rooted tall map $T \to S$ with 
...
\end{tikzcd}
\end{equation}
where $T \to T' \to S$ is the natural factorization such that the second map is $Y$-inert, 
i.e., $T'$ is obtained from $T$ by simply relabeling to $X$ those $Y$-labeled vertices of $T$ that become $X$-vertices in $S$.
Note that the existence of the right square in (\ref{K-1LANFINAL EQ}) follows from the map $T' \to S$ being $Y$-inert together with Remark \ref{YINERT REM}.
Since (\ref{K-1LANFINAL EQ}) becomes a diagram of rooted  isomorphism on $\mathsf{lr}$, it produces the necessary zigzag connecting the objects $T \to S$ and 
$\mathsf{lr}_{\mathcal{P}}(S_{\mathcal{P}}) \to S$
...
\begin{lemma}
  ...
  $\bar{\Omega}_G^{e}[k,0]$ is $\mathsf{Ran}$-initial in $\bar{\Omega}_G^e[k]$ over $\Sigma_G$. \qed
\end{lemma}
...
\end{corollary}
\begin{proof}
        Any span defining the pushout in (\ref{FF_FREE_EXTENSION}) is equivalent to the data of the solid span in the following pushout diagram in $\Op_G(\V)$.
        \begin{equation}
        ...
        and thus the filtration and pushout description for (\ref{F_FG_FREE_EXTENSION}) from (\ref{FILTRATION_LAN_LEVEL}) induce a filtration and pushout description on the map
        \[
...
\begin{definition}[\cite{Ste16}]
      ...
      \begin{enumerate}
      \item \textit{$\F$-weak equivalence} (resp. \textit{$\F$-fibration}) if $f^H$ is so in $\V$ for all $H\in \F$.
      \item \textit{$\F$-cofibration} if it has lifts against all maps which are both $\F$-fibrations and $\F$-weak equivalences. 
      \end{enumerate}
\end{definition}
\begin{definition}
      The \textit{$\F$-model structure} on $\V^\Pi$, if it exists, is the unique model structure with $\F$-fibrations, $\F$-weak equivalences, and $\F$-cofibrations.
\end{definition}

If $\F$ is the set of all subgroups of $\Pi$, this is called the \textit{genuine} model structure, and denoted $\V^{\Pi}_{\text{gen}}$. 
...
\begin{definition}
      ...
      \[
      \begin{tikzcd}[column sep = 5em]
            \V^{\mathcal D}_{\F} \arrow[r, shift right] & \V^{\text{Ob}(\mathcal D)}_{\F} = \prod_{d\in \mathcal D}\V^{\mathsf{Aut}(d)}_{\mathcal F_d}. \arrow[l, shift right]
      \end{tikzcd}
      \]
...
The proof of Theorem \ref{MAINEXIST1 THM} will be split into two parts, Corollary \ref{F_OP_SEMI_MODEL_THM} and Theorem \ref{F_OP_Q_MODEL_THM}.
...
\begin{definition}
        The \textit{$\F$ (semi) model structure} is the unique (semi) model structure on $\Op^G(\V)$, if it exists, with $\F$-fibrations, $\F$-cofibrations, and $\F$-weak equivalences, and will be denoted $\Op^G_{\F}(\V)$. 
...
\begin{proposition}
        ...
        $T\in \Omega^a$, and 
        we are given level genuine cofibrations $f_{ac}: Q \to \P$ and $f_{in}: X \to Y$ in  $\Sym^G(\V)$. 
...
\end{corollary}  
\begin{proof} 
        As $\Op(\V^G) = \Op^G(V)$, we consider the filtration in (\ref{FILTRATION_LAN_LEVEL}) with $G = *$ and $\V = \V^G$ . Then the result follows from Corollary \ref{AUTTCOFPUSH_GEN_PROP} and Remark \ref{GEN_FGTRIGHT_REMARK}(i).
\end{proof} 

\begin{proof}[Proof of Theorem \ref{G_OP_SEMI_MODEL_THM}]
        Using \cite[Lemma 2.3]{SS00}, the fact that weak equivalences and transfinite compositions are levelwise implies that Proposition \ref{CELLULAR_LEVEL_COFIB_PROP2} and Remark \ref{GEN_FGTRIGHT_REMARK}(i) are sufficient.  
\end{proof}
...
\begin{remark}
        ...
        Indeed, when restricting the machinery of Section \ref{GENUINE_OP_MONAD_SECTION} to $G = *$ and $\V = \V^G$, Remark \ref{NEED_WREATH_REMARK} no longer applies, and hence 
        we may reformulate the monad on spans from Definition \ref{WSPAN_MONAD_DEFINITION} by replacing $\Fin_s$ with $\Sigma$ and using the natural map $\Sigma \wr \V^{op} \xrightarrow{\otimes} \V^{op}$ which exists for \textit{any} symmetric monoidal product $\otimes$ on $\V$.
\end{remark}
...
\subsection{Genuine $\F$-operads}
\label{OP_G_EXISTS_SECTION}
% \subsubsection{The $\F$-model structures}

We now turn our consideration to the homotopy theory of genuine $G$-operads from Section \ref{GENUINE_OP_MONAD_SECTION}.
As above, we prove the main result Theorem \ref{MAINEXIST2 THM} in two parts: Corollary \ref{OP_F_SEMI_MODEL_THM} and Theorem \ref{OP_F_Q_MODEL_THM}. 

Throughout this subsection, fix a weak indexing system $\Sigma_{\F} \subseteq \Sigma_G$ (see Definition \ref{INDEXSYS DEF}).


COME BACK HERE

\begin{notation}
        In this subsection, we will let $\Sigma_{\F}$ denote a fixed \textit{family of corollas}, i.e. a \textit{sieve} subcategory of $\Sigma_G$
        
        and we let $\Sym_{\F}(\V)$ denote the model category 
        $\V^{\Sigma_{\F}^{op}}_{\text{gen}}$, if it exists. 

        Additionally, we write $\Sym_{G / \F}(\V)$ for the category $\Sym_G(\V)$ endowed with the $\Sigma_{\F}$-projective genuine model structure from Definition \ref{SUBD_PROJ_F_MODEL_STRUCTURE}. 
        Equivalently, if we let $\upgamma: \Sigma_{\F} \into \Sigma_G$ denote the inclusion, this is the model structure transfered from the adjunction
        \begin{equation}
                \label{SYM_F_ADJ_EQ1}
                \begin{tikzcd}
                        \Sym_{G / \F}(\V)
                        \arrow[r, shift right, "\upgamma^{\**}"']
                        &
                        \Sym_{\F}(\V)
                        \arrow[l, shift right, "\upgamma_!"'].
                \end{tikzcd}
        \end{equation}
\end{notation}

As $\Sym_{\F}(\V) \simeq \prod_{n \geq 0} \V^{O_{\F_n}^{op}}$,
Proposition \ref{DBAR_PROJ_MODEL_PROP} immediately implies the following.
\begin{corollary}
        \label{SYM_F_Q_COR}
        If $\V$ is admissible for finite groups, then 
        both $\Sym_{\F}(\V)$ and $\Sym_{G / \F}(\V)$ exist and are Quillen equivalent. \qed
\end{corollary}

\begin{remark}
      More generally, the above holds for any two nested sieve subcategories $\Sigma_{\bar \F}\subseteq \Sigma_{\F}$ of  $\Sigma_G$, not just $\Sigma_{\F}$ with $\Sigma_G$ itself.
\end{remark}

We identify the following maps in $\Op_G(\V)$ (cf. Definitions \ref{F_MODEL_DEFN}, \ref{F_MAPS_DEFN}).
\begin{definition} 
        We say a map $f: \O \to \P$ in $\Op_G(\V)$ is an  
        \begin{itemize} 
        \item[(i)] \textit{$\F$-weak equivalence} (resp. \textit{$\F$-fibration}) if 
                $f(C):\O(C) \to \P(C)$ is one in $\V^{\mathrm{Aut}(C)}_{gen}$ for all $\F$-corollas $C\in\Sigma_{\F}$. 
        \item[(ii)] \textit{$\F$-cofibration} if 
                it has the left lifting property against all maps which are both $\F$-fibrations and $\F$-weak equivalences. 
        ...
        \item[(iv)] \textit{level genuine cofibration} if 
                $f(C)$ is a cofibration in $\V^{\mathrm{Aut}(C)}_{gen}$ for all $C\in \Sigma_\G$. 
        \end{itemize} 
\end{definition} 
...
\begin{definition} 
        The \textit{$\F$ (semi) model structure} on $\Op_G(\V)$, if it exists, 
        is the unique (semi) model structure with $\F$-fibrations, $\F$-weak equivalences, and $\F$-cofibrations,
        denoted $\Op_{G / \F}(\V)$ . 
        Equivalently, it is the (semi) model structure transferred along the adjunction below.
        \[ 
        \begin{tikzcd} 
                \Op_G(\V) 
...
\begin{notation} 
      If $\Sigma_{\F} = \Sigma_G$, we refer to the $\F$ model structure as the \textit{genuine} model structure, and denote it simply $\Op_G(\V)$. 
\end{notation} 

We can now state the first part of our main result for this section.
\begin{theorem}\label{OP_G_SEMI_MODEL_THM} 
      For $\V$ strongly cellular with diagonals, the genuine semi model structure on $\Op_G(\V)$ exists.
\end{theorem} 

Again, the key technical step written out below follows from the simple case of a result from Section \ref{COFIB SEC}.
\begin{proposition}
        \label{GENUINE_TREE_BOX_COFIBRANT_PROP} 
        Suppose $\V$ is strongly cellular with diagonals, 
        $T\in \Omega_G^a$, and 
        we are given level genuine cofibrations $f_{ac}: \mathcal Q \to \P$ and $f_{in}: X \to Y$ in $\Sym_G(\V)$. 
        Then the iterated box product
        \[ 
...
\begin{proof}
        ...
        Thus, it suffices to show that $f^{\Box V_G(T)}$ is cofibrant on the left hand side. 
...
\begin{corollary}\label{CELLULAR_LEVEL_COFIB_PROP} 
        For $\V$ strongly cellular with diagonals, and for any free $\mathbb{F}_G$-extension  
        \[ 
        \begin{tikzcd} 
                \mathbb{F}_G X \arrow[d, "u"'] \arrow[r] & \P \arrow[d]\\ 
                \mathbb{F}_G Y \arrow[r] & \P[u] 
...
\end{corollary}  
\begin{proof} 
        By using the levelwise filtration from (\ref{FILTRATION_LAN_LEVEL}), this follows immediately from Proposition \ref{GENUINE_TREE_BOX_COFIBRANT_PROP} and Remark \ref{GEN_FGTRIGHT_REMARK}(i). 
\end{proof} 


\begin{proof}[Proof of Theorem \ref{OP_G_SEMI_MODEL_THM}] 
        This follows precisely as the proof of Theorem \ref{G_OP_SEMI_MODEL_THM}, 
...
Again, we find that the existence of the $\F$ semi model structures follows identically as the genuine. 
Indeed, \ref{LEVEL_COFIB_REMARK} also implies that cofibrations in $\Sym_{G / \F}(\V)$ are in fact level genuine cofibrations, which suffices.

\begin{corollary} 
        \label{OP_F_SEMI_MODEL_THM}
        If $\V$ is strongly cellular with diagonals, 
        the $\F$ semi model structure on $\Op_G(\V)$ exists
        for any sieve subcategory $\Sigma_{\F} \subseteq \Sigma_G$. \qed
\end{corollary} 

Precisely as for Theorem \ref{F_OP_Q_MODEL_THM}, if our base category $\V$ is even nicer, these semi model structures are in fact Quillen model structures.
\begin{theorem} 
        \label{OP_F_Q_MODEL_THM}
        For $\V$ underlying strongly cellular with diagonals, % and moreover
        % \begin{itemize}
        % \item genuine cofibrations in $\V^\Pi$ are underlying cofibrations in $\V$ for all finite groups $\Pi$, and
        % \item every object in $\V$ is cofibrant,
        % \end{itemize}
        then the $\F$-model structure $\Op_{G / \F}(\V)$ exists 
        for any sieve subcategory $\Sigma_{\F} \subseteq \Sigma_G$. \qed
\end{theorem} 
% \begin{proof} 
...
\begin{example} 
      The $\F$-model structure exists on $\sOp_G := \Op_G(\sSet)$ for any sieve subcategory $\Sigma_{\F} \subseteq \Sigma_G$.
\end{example} 
...
The $G$-graph families we will be interested in encode certain families of $G$-trees. We start with the case of families of corollas (see Definition \ref{COROLLA_FAMILY_DEF}).
...
%%%%%% Model structures on Op_F

We can now build model structures onto this new category of algebras.
...
\begin{definition}
        Let $\Sigma_{\bar\F} \subseteq \Sigma_{\F}$ be a sieve subcategory, i.e. $\bar\F\subseteq \F$ a subfamily of corollas.
        We say a map $f: \O \to \P$ in $\Op_{\F}(\V)$ is a 
        \begin{enumerate}
        \item \textit{$\bar\F$-weak equivalence} (resp. \textit{$\bar\F$-fibration}) if $f(C): \O(C) \to \P(C)$ is one in $\V^{\mathsf{Aut}(C)}_{\text{gen}}$ for all $\bar\F$-corollas $C \in \Sigma_{\bar \F}$.
                % \item \textit{$\Sigma_{\bar \F}$-cofibration} iff it is so after forgetting to $\Sym_{\bar \F}(\V) \subseteq \Sym_{\F}(\V)$.
        \item \textit{$\bar\F$-level genuine cofibration} iff $f(C)$ is a cofibration in $\V^{\mathsf{Aut}(C)}_{\text{gen}}$ for all $C\in \Sigma_{\bar \F}$. 
        \end{enumerate}
        The \textit{$\bar\F$ (semi) model structure} on $\Op_{\F}(\V)$, if it exists, is the (semi) model structure denoted $\Op_{\F/\bar\F}(\V)$ with the above $\bar\F$-weak equivalences and $\bar\F$-fibrations. 
        Equivalently, this is the model structure transferred across either composition of free-forgetful adjunctions
        \[
        \begin{tikzcd}
                \Op_{\F/\bar \F}(\V) 
                \arrow[r, shift right, "\nu^{\**}"'] 
                ...
                \\
                \Sym_{\F/\bar\F}(\V) 
                \arrow[u, shift right, "\mathbb{F}_{\F}"'] 
                \arrow[r, shift right, "\nu^{\**}"'] 
                & 
                \Sym_{\bar \F}(\V) 
                \arrow[l, shift right, "\nu_!"']
                \arrow[u, shift right, "\mathbb{F}_{\bar F}"']
        \end{tikzcd}
        \]
\end{definition}
...
% \begin{align*} 
%         I_{\F/ \bar \F} &= \sets{\mathbb{F}_{\F}(\Sigma_{\F}(-,C)/H \cdot i)}{C \in \Sigma_{\bar \F},\ H\leq \mathsf{Aut}(C),\ i\in I}\\
%         J_{\F/ \bar \F} &= \sets{\mathbb{F}_{\F}(\Sigma_{\F}(-,C)/H \cdot j)}{C \in \Sigma_{\bar \F},\ H\leq \mathsf{Aut}(C),\ j\in J}.
% \end{align*} 

If $\bar \F = \F$, this is referred to as the \textit{projective} (semi) model structure on $\Op_{\F}(\V)$. 

Replacing Proposition (\ref{FILTRATION_LAN_LEVEL}) with Corollary (\ref{FF_LEVEL_FILTRATION}), we have that the statement analogous to Corollary \ref{CELLULAR_LEVEL_COFIB_PROP} but for free $\mathbb{F}_{\F}$-extensions remains true.
% \begin{corollary}
%         For $\V$ strongly cellular with diagonals, and for any free $\mathbb{F}_{\F}$-extension (\ref{FF_FREE_EXTENSION})
%         where $u: X\to Y$ is a level genuine cofibration and
%         $\P$ is level genuine cofibrant in $\Sym_{\F}(\V)$,
%         the map $\P \to \P[u]$ is a level genuine cofibration as well, trivial if $u$ is so.
% \end{corollary}
Thus, mimicking the proofs of Theorem \ref{OP_G_SEMI_MODEL_THM}, Corollary \ref{OP_F_SEMI_MODEL_THM}, and Theorem \ref{OP_F_Q_MODEL_THM} together yields the following result.
\begin{theorem}
        For $\V$ strongly cellular with diagonals, the $\bar\F$ semi model category $\Op_{\F / \bar\F}(\V)$ exists for any subfamily $\bar\F\subseteq \F$.
        
        If $\V$ is additionally underlying strongly cellular, this is an actual model structure. \qed
\end{theorem}
...
is not really new. Indeed, as there are equivalences of categories
$
\mathsf{Sym}_{\mathcal{F}}(\mathcal{V})
\simeq \prod_{n \geq 0}
...
$\mathbb{F}_{\F} (\emptyset) \simeq \iota_{\**} \mathbb{F} (\emptyset)$, which is readily checked directly, 
...
Noting that $\gamma \iota = \iota_G$
(where we now let $\iota_G$ denote the composite injection $G \times \Sigma \to \Sigma_{\F} \to \Sigma_{G}$)
and recalling that there are natural isomorphisms 
\[
(\iota_G)^{\**} \mathbb{F}_{G} (\iota_{G})_{\**}
\simeq  \mathbb{F} (\iota_G)^{\**} (\iota_{G})_{\**} \simeq \mathbb{F}
\qquad
...
\upgamma^{\**} \mathbb{F}_G \upgamma_* \iota_{\**} Z \simeq 
\iota_{\**} \iota^{\**} \upgamma^{\**} \mathbb{F}_G \upgamma_{\**} \iota_{\**} Z \simeq
