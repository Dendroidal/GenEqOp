...
Then the projective semi model structure on $\mathsf{Op}_{G}(\V)$ exists, and is determined by the forgetful functor
\[
...
the projective semi model structure on $\mathsf{Op}_{\mathcal F}(\V)$ exists, and is determined by the forgetful functor
\[
...
\begin{definition}
      ...
      for all maps $f: d \to \bar d$, $\bar d \in \bar{\mathcal D}$ implies both $d$ and the map $f$ are also in $\bar{\mathcal D}$.
...
\begin{remark}
      ...
      families $\mathcal F_n$ of \textit{graph subgroups} of $G \times \Sigma_n$
...
\begin{notation}
        Abusing notation, we will denote the composite adjunction as simply
        \[
...
\begin{proof} 
        ...
        as a map $(-)\otimes \P(T_v)$ is equivalently given by $(-) \Box (\varnothing \to \P(T_v))$.
\end{proof} 
...
\begin{definition} 
      ...
      \begin{itemize} 
      \item[(i)] \textit{projective weak equivalence} (resp. \textit{projective fibration}) if 
            $f(C):\O(C) \to \P(C)$ is a weak equivalence (fibration) in $\V^{\mathrm{Aut}(C)}_{gen}$ for all $\F$-corollas $C\in\Sigma_{\F}$. 
      \item[(ii)] \textit{projective cofibration} if 
...
\begin{definition} 
      The \textit{projective (semi) model structure} on $\Op_\F(\V)$, if it exists, 
      is the unique (semi) model structure with projective weak equivalences, fibrations, and cofibrations.        
      ...
      \begin{tikzcd} 
            ...
            & 
            \Sym_{\F}(\V) 
            \arrow[l, shift right, "\mathbb{F}_\F"'] 
      \end{tikzcd} 
      \] 
\end{definition} 
...
\begin{example} 
      The projective model structure exists on $\sOp_\F := \Op_\F(\sSet)$ for any weak indexing system $\F$.
\end{example} 
...
\begin{lemma}
        ...
        Families of corollas $\Sigma_{\mathcal F}$ are equivalent to collections $\mathcal F = \set{\mathcal F_n}_{n \geq 0}$ of $G$-graph families $\mathcal F_n$ of $G\times\Sigma_n$.
\end{lemma}
\begin{proof}
        ...
        conversely, any $\mathcal F$ dually defines a full subcategory of $\Sigma_G$, 
