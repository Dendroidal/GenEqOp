...
is proven in {\color{red} \S \ref{MAINEXIST SEC},
where the additional conditions are also discussed.
} 
A similar result announced by Gutierrez-White \cite{GW} also establishes the existence of 
$\Op^G_{\mathcal F}(\Top)$ for $\mathcal F$ a (regular) indexing system.
...
generalizating the work of, e.g. Joyal, Lurie, and Bergner on $\infty$-categories.
\begin{equation}
  \label{CM_EQ}
  \tag{$\filledstar$}
  \begin{tikzcd}
...
Sequals will extend both of these to the $G$-equivariant context. 
In particular, we expect the categories $\Op^G_\F(\sSet)$ and $\mathsf{dSet}^G_\F$ to be Quillen equivalent.
