...
The diagrams (with back and lower slanted faces instances of (\ref{PTPULL EQ}))
	...
	\begin{tikzcd}[column sep = 0.1em]
		\Omega_{G,{n+2}} \ar{rr} \ar{rd}{d_{i+1}} \ar{dd}
		 & & \Fin_s \wr \Omega_{G,{n+1}} \ar{rd}{\Fin_s \wr d_i} \ar{dd} & &
		\Omega_{G,{n+1}} \ar{rr} \ar{rd}{s_{i}} \ar{dd} 
		& & \Fin_s \wr \Omega_{G,{n}} \ar{rd}{\Fin_s \wr s_{i-1}} \ar{dd} &
	\\
		& \Omega_{G,{n+1}} \ar{ld} \ar[crossing over]{rr} 
		& & \Fin_s \wr \Omega_{G,{n}} \ar{ld} &
...
$V_{G,n+1} = \sigma^0 \circ V_{G,n} \circ V_G$ for $n \geq 0$.
\end{notation}
...
that we refer to as the category of $G$-symmetric sequences on $\mathcal{V}$.

This will be done via a two step process, 
by first building a monad $N$ on a larger category
$\mathsf{WSpan}^l(\Sigma_G^{op},\mathcal{V})$
which we then transfer via adjunction.
...
\begin{notation}\label{OMEGAGNA NOT}
	Given a functor $\rho \colon A \to \Sigma_G$, we let $\Omega^{(A)}_{G,n}$ denote the pullback in $\mathsf{Cat}$
...
\end{equation}
such that $\rho(a_{v_{G e}}) = T_{n,v_{G e}}$.
\end{notation}
...
Indeed, Proposition \ref{SUBSASPULL PROP} can be reinterpreted as an identification
$\Omega_{G,0}^{(\Omega_{G,n})} \simeq \Omega_{G,n+1}$.
Moreover, $V_{G,0}^{(\Omega_{G,n})}$ is then identified with 
$V_G$ as defined in Notation \ref{VGDEF NOT} and by Remark \ref{DSCOM REM} the maps
$V_{G,0}^{(d_i)}$, $V_{G,0}^{(s_{i-1})}$ are identified with
$d_{i+1}$, $s_i$ for $i \geq 0$.
...
$d_0^{(A)}\colon \Omega_{G,1}^{(A)} \to \Omega_{G}^{(A,0)}$ and natural isomorphisms
