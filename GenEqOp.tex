...
we mean the notion a weaker, relative notion of model category
% introduced in \cite{Spi01} as a \textit{$J$-semi-model category over $\mathcal M$}
% (interpreting work of \cite{Ho98})
discussed in \cite[\S 2.2]{WY15} and \cite[\S 12.1]{Fre09}\footnote{The notion in \cite{WY15} recovers that from \cite{Fre09} by choosing $\mathcal M$ to be the unique model structure on the terminal category.}
...
% e[\S 2.2]{WY15} or \cite[\S 12.1]{Fre09}.
...
\begin{definition}
      ...
      has weak equivalences and fibrations defined by restricted $H$ to $\F$.
\end{definition}
...
As $\F$ increaes in size, so does to class of (trivial) cofibrations.
...
\begin{remark}\label{CSPP_REM}
      There are several notions which have a similar form to ``cofibrant symmetric pushout powers''
      which appear in the literature, for example
      the \textit{freely powered} condition of \cite[Def. 4.5.4.2]{Lur17} 
      and the \textit{commutative monoid axiom} of \cite[Def. 3.1]{Wh16}.
      % symmetric $h$-monoidal of Pavlov-Scholbach,
      % and the cofibration hypothesis of \cite{MMSS},
      % to name a few.

      The main distinction of our condition from these two is that
      cofibrant symmetric pushout powers concerns cofibrations in the genuine fixed-point model structure $\V^{\Sigma_n}$;
      freely powered is a much stronger condition, as in particular it demands that $f^{\square n}$ is a cofibration in the \textit{projective} model structure,
      while the commutative monoid axiom is mildly weaker by reducing to information in $\V$.
      For our analysis in \S \cite{COFIB SEC}, it is critical that we remain in the genuine equivariant setting.     
\end{remark}
