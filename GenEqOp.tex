...
Any monad $T$ on $\C$ one obtains induced monads $T^{\times n}$ on $\C^{\times n}$, and we will make use of several standard relations between these.
In particular, any map $\alpha \colon \underline{n} \to \underline{m}$ induces a forgetful functor
such that for the forgetul functor 
$\alpha^{\**} \colon \C^{\times m} \to \C^{\times n}$
one has $T^{\times n} \alpha^{\**} \simeq  \alpha^{\**} T^{\times m}$.

Indeed, we will need to make use of a slightly more general setup. Letting $I$ denote the identity monad on $\C$, and $A \subset \underline{n}$ be a subset, there is a monad $T^{\times A} \times I^{\times(\underline{n}-A)}$, which we abusively denote simply as $T^{\times A}$. Identities then determine maps of monads 
$T^{\times A} \to T^{\times B}$
and, moreover, there are identifications
$T^{\times A} \alpha^{\**} \simeq \alpha^{\**} T^{\times \alpha(A)}$.
...
\begin{equation}\label{MONADFUNCTORALPHA EQ}
	T^{\times A} \Rightarrow \alpha^{\**} T^{\times \alpha(A)} \alpha_{!}
\end{equation}
...
$(FG)^{\times A} \simeq F^{\times A} G^{\times A}$ which are compatible with the identifications
$F^{\times A} \alpha^{\**} \simeq \alpha^{\**} F^{\times \alpha(A)}$
in the sense that the identification
$(F G)^{\times A} \circ \alpha^{\**} \simeq 
\alpha^{\**} (FG)^{\times \alpha(A)}$
matches the composite identification
$F^{\times A} G^{\times A} \alpha^{\**} \simeq
F^{\times A}\alpha^{\**}G^{\times \alpha(A)} \simeq
\alpha^{\**} F^{\times \alpha(A)} G^{\times \alpha(A)}$.
...
\[
	T^{\times A} \xrightarrow{\eta} 
	T^{\times A} \alpha^{\**} \alpha_{!} \simeq
	\alpha^{\**} T^{\times \alpha(A)}\alpha_{!}.
\]
That this is a monad map is the condition that multiplication and unit diagrams below commute.
\[
\begin{tikzcd}
	T^{\times A} \circ T^{\times A} \ar{d} \ar{r} &
	\alpha^{\**} T^{\times \alpha(A)} \alpha_{!} \circ 
	\alpha^{\**} T^{\times \alpha(A)} \alpha_{!} \ar{d} &
	I^{\times n} \ar{d} \ar{rd}& 	
\\
	T^{\times A} \ar{r} &
	\alpha^{\**} T^{\times \alpha(A)} \alpha_{!} &
...
\[
\begin{tikzcd}
	T^{\times A}  T^{\times A}  \alpha^{*} \ar{r}{\simeq} \ar{dd}&
	T^{\times A}  \alpha^{*}  T^{\times \alpha(A)} \ar{r}{\eta}
	\ar[equal]{rd} &
	T^{\times A} \alpha^{\**} \alpha_{!} \alpha^{*} T^{\times \alpha(A)} \ar{d}{\epsilon} \ar{r}{\simeq}&
	\alpha^{\**} T^{\times \alpha(A)} \alpha_{!} \alpha^{\**}  T^{\times \alpha(A)} \ar{d}{\epsilon}
\\
	& &
	T^{\times A}  \alpha^{*}  T^{\times \alpha(A)} \ar{r}{\simeq} &
	\alpha^{*}  T^{\times \alpha(A)}  T^{\times \alpha(A)} \ar{d}
\\
	T^{\times A}  \alpha^{*} \ar{rrr}{\simeq} &&&
	\alpha^{*} T^{\times \alpha(A)}
\end{tikzcd}
\]
\end{proof}
...
\begin{remark}
	Since $T^{\times B} \circ \alpha_{!}$ is a right 
	$\alpha^{\**} T^{\times B}\alpha_{!}$-module,
	Proposition \ref{MONADICFUN PROP} implies that it is also a 
	right $T^{\times \alpha^{-1}(B)}$-module or, moreover, a 
	$T^{\times A}$-module whenever $\alpha(A) \subset B$.
\end{remark}

{\color{red} HERE}

\begin{proposition}
	The right module action natural transformation
\[
\begin{tikzcd}[row sep = 0.5em]
	T \circ \amalg \circ T^{\times 2} \ar{r} & T \circ \amalg
\\
	T(T X_1 \amalg T X_2) \ar{r} & T(X_1 \amalg X_2)
\end{tikzcd}
\]
encodes the multiplication map
\[
	T T X_1 \amalg_a T T X_2 \to T X_1 \amalg_a T X_2.
\]
\end{proposition}

\begin{proof}
	It suffices to check that the diagrams on the leftmost diagrams below commute for each $k$ or, equivalently, that the central diagram commutes. By adjunction, this is equivalent to the commutativity of the rightmost diagram, and thus follows directly from Proposition \ref{MONADICFUN PROP}.
\[
\begin{tikzcd}[row sep=1em]
	TT X_k \ar{r} \ar{d} & 
	T(T X_1 \amalg T X_2) \ar{d} &
	\amalg \circ T^{\times 2} \circ T^{\times 2} \ar{r} \ar{d} &
	T \circ \amalg \circ T^{\times 2} \ar{d} &
	T^{\times 2} \circ T^{\times 2} \ar{r} \ar{d} &
	\Delta \circ T \circ \amalg \circ T^{\times 2} \ar{d} 
\\
	T X_k \ar{r} & 
	T(X_1 \amalg X_2) &
	\amalg \circ T^{\times 2} \ar{r} &
	T \circ \amalg &
	T^{\times 2} \ar{r} &
	\Delta \circ T \circ \amalg 
\end{tikzcd}
\]
\end{proof}
