...
However, applying such results when $\mathcal{V}=\mathsf{Op}$
...
(i.e. using the notion of equivalence in (\ref{NAIVEOPEQ EQ})),
and in doing so one forgoes the ability to distinguish 
the many types of $N \mathcal{F}$-operads.

The correct replacement for
$\mathsf{Top}^{\mathsf{O}_G^{op}}$
in the context of operads with norm maps will then be 
a new category $\mathsf{Op}_G$,
whose objects are novel algebraic structures we dub \textit{genuine equivariant operads} (as opposed to the category $\mathsf{Op}^G$ of (regular) equivariant operads), and which we formally define in \S \ref{GENUINE_OP_MONAD_SECTION}.
To motivate this category, we note first that the desideratum that weak equivalences in $\mathsf{Op}_G$
be detected ``at each level''
suggest that each $\mathcal{P} \in \mathsf{Op}_G$
should consist of a list of spaces for each of the graph subgroups in (\ref{GENGEINFTY EQ}), along with 
obvious restriction maps and, more importantly, 
suitable \textit{multiplication maps}.
