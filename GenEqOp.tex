...
We will make multiple uses of the following straightforward result.
...
The adjunction (\ref{TWOADJOINTS EQ}) identifies
$\mathsf{Sym}^G$ 
as a reflexive subcategory of
$\mathsf{Sym}_G$, with the counit map $X(C) \to \left(\prod_G X(C)\right)^G$ given by 
the isomorphism between $X(C)$ and the diagonal of $\prod_G X(C)$.
...
\begin{itemize}
	...
	is a monad;
	\item[(ii)] there is a natural isomorphism of monads
	$\mathbb{F} \to \iota^{\**} \mathbb{F}_G \iota_{\**}$.
\end{itemize}
\end{proposition}

\begin{proof}
Throughout this proof we write 
$\Sigma_G^{\text{free}} \hookrightarrow \Sigma_G$,
$\Omega_{G,0}^{\text{free}} \hookrightarrow \Omega_{G,0}$,
for the full subcategories of 
those $G$-corollas/trees with free $G$-actions.
We note that the leaf-root and vertex functors
$\mathsf{lr} \colon \Omega_{G,0} \to \Sigma_G$,
$V_G \colon \Omega_{G,0} \to \Fin \wr \Sigma_G$
then both restrict to 
free trees, i.e., one has
$\mathsf{lr} \colon \Omega_{G,0}^{\text{free}} \to \Sigma_G^{\text{free}}$,
$V_G \colon \Omega_{G,0}^{\text{free}} \to \Fin \wr \Sigma_G^{\text{free}}$.
Furthermore, for any $C \in \Sigma_G^{\text{free}}$ one has an equality of rooted undercategories between
$C \downarrow_{\mathsf{r}} \Omega_{G,0}$
and
$C \downarrow_{\mathsf{r}} \Omega_{G,0}^{\text{free}}$,
so that
$\iota^{\**} \mathbb{F}_G A$ is computed by the Kan extension of
\begin{equation}\label{IFGI EQ}
\begin{tikzcd}
	\Omega_{G,0}^{\text{free}} \ar{d} \ar{r} &
	\Fin \wr \Sigma_G^{\text{free}} \ar{r}{\Fin \wr A} &
	\Fin \wr \mathcal{V}^{op} \ar{r} &
	\mathcal{V}^{op}
\\
	\Sigma_G^{\text{free}}
\end{tikzcd}
\end{equation}
so that (i) follows from the obvious claim 
$A \to \iota_{\**} \iota^{\**} A$
is an isomorphism when restricted to $\Sigma_G^{\text{free}}$
(indeed, $\Sigma_G^{\text{free}}$ is the closure of the image of $\iota$ under isomorphism).

To show (ii), we first set some notation. We will slightly abuse notation to write 
$G \times \Sigma \hookrightarrow \Sigma_G^{\text{free}} \hookrightarrow \Sigma_G$ for the image of $\iota$
and similarly
$G \times \Omega_0 \hookrightarrow \Omega_{G,0}^{\text{free}} \hookrightarrow \Omega_{G,0}$ for the image of the obvious analogue functor
$\iota \colon G \times \Omega_0 \to \Omega_{G,0}$.
Just as before the leaf-root and vertex functors
factor as
$\mathsf{lr} \colon G \times \Omega_0 \to G \times \Sigma$,
$V_G \colon G \times \Omega_0 \to \Fin \wr (G \times \Sigma)$
. In fact, this notation extends to strings $G \times \Omega_{n}$ compatibly with all vertex functors and simplicial functors.

Under this identification, $\mathbb{F}B$ is then computed as the Kan extension of
\begin{equation}\label{F EQ}
\begin{tikzcd}
	G \times \Omega_{0} \ar{d} \ar{r} &
	\Fin \wr (G \times \Sigma) \ar{r}{\Fin \wr B} &
	\Fin \wr \mathcal{V}^{op} \ar{r} &
	\mathcal{V}^{op}
\\
	G \times \Sigma
\end{tikzcd}
\end{equation}
The map $\mathbb{F} \to \iota^{\**} \mathbb{F}_G \iota_{\**}$ is then induced by the obvious ``inclusion'' natural transformation of (\ref{F EQ}) into (\ref{IFGI EQ}) when $B=\iota_{\**} A$.
To check that $\mathbb{F} \to \iota^{\**} \mathbb{F}_G \iota_{\**}$ is an isomorphism, 
we first note that there are compatible retractions
$\rho \colon \Omega_{G,0}^{\text{free}} \to G \times \Omega_0$,
$\rho \colon \Sigma_{G}^{\text{free}} \to G \times \Sigma$
defined by 
$\rho(\amalg_{1\leq i \leq |G|} T_i) = G \cdot T_1$
together with uniquely determined isomorphisms 
$T \simeq \rho(T)$ which are given by the identity
on the first tree component $T_1$.
It thus now follows that for each $C \in G \times \Sigma$ the undercategory $C \downarrow G \times \Omega_0$ is initial (as well as final) in 
$C \downarrow \Omega_{G,0}^{\text{free}}$,
showing that 
$\mathbb{F} \to \iota^{\**} \mathbb{F}_G \iota_{\**}$
is indeed an isomorphism.

The final claim that $\mathbb{F} \to \iota^{\**} \mathbb{F}_G \iota_{\**}$ is a map of monads 
follows from the intuitive fact that the composite 
$\mathbb{F} \mathbb{F}
	\to 
...
$
is encoded by the analogous natural transformation of diagrams for strings $G \times \Omega_1 \hookrightarrow \Omega_{G,1}^{\text{free}}$.

{\color{red} HERE}

However, since a rigorous direct proof of this last claim can be fairly cumbersome due to the presence of left Kan extensions in the definitions of $\mathbb{F}$, $\mathbb{F}_G$, we sketch here an alternate argument.
...
%\[
%\begin{tikzcd}
%	G \times \Omega_1	\ar{dd} \ar{rd} \ar{rr} &&
%	\Fin \wr (G \times \Omega_0) \ar{rd} \ar[dashed]{dd} \ar{rr}%&&
%	\Fin \wr \mathcal{V}^{op} \ar{r} &
%	\mathcal{V}^{op}
%\\
%	& \Omega_{G,1}^{\text{fr}} \ar{dd} \ar{rr} &&
%	\Fin \wr \Omega_{G,0}^{\text{fr}} \ar{dd} \ar{ru}
%\\
%	G \times \Omega_0	\ar{dd} \ar{rd} \ar[dashed]{rr}&&
%	\Fin \wr (G \times \Sigma) \ar[dashed]{rd}
%\\
%	& \Omega_{G,0}^{\text{fr}} \ar{dd} \ar{rr} &&
%	\Fin \wr \Sigma_G^{\text{fr}}
%\\
%	G \times \Sigma \ar{rd}
%\\
%	& \Sigma_{G}^{\text{fr}}
...
Proposition (\ref{MONADADJ PROP}) now gives the following.

\begin{corollary}
	The adjunction (\ref{TWOADJOINTS EQ}) extends to an adjunction
\begin{equation}\label{JUSTONEDJOINTOP EQ}
\begin{tikzcd}
	\iota^{\**} \colon 
	\mathsf{Op}_G \ar[shift left =2pt]{r} 
	&
	\mathsf{Op}^G 
	\colon \iota_{\**} \ar[shift left =2pt]{l}
\end{tikzcd}
\end{equation}
identifying $\mathsf{Op}^G$ as a reflexive subcategory of 
$\mathsf{Op}_G$.
\end{corollary}
