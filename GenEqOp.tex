...
families $\mathcal F_n$ of graph subgroups of $G \times \Sigma_n$, so that there is an equivalence of categories
$\Sigma_\F \simeq \coprod \mathsf{O}_{\F_n}$.
...
However, since these results are particular instances of subtler results in \S \ref{COFIB SEC},
which require a far more careful analysis,
we defer the harder proofs to those
of the results in \S \ref{COFIB SEC}.
...
The exact condition, originally from \cite{Ste16}, can be found in Definition \ref{CELL DEF}. Moreover, note that this is condition (iii) in our main theorems.
...
Moreover given a family $\mathcal{F}$ (or even collection of subgroups) of $\Sigma$, there exists a model structure 
...
a level genuine (trivial) cofibration if each of the maps
...
yields that the required conditions need only be checked for the
...
When $T \in \Omega^a[k]$ is a non-equivariant alternating tree, $\mathcal{P}$ is cofibrant in $\mathsf{Sym}^G$
$u \colon X \to Y$ is a (trivial) cofibration in $\mathsf{Sym}^G$,
and writing $C_n = \mathsf{lr}(T)$,
...
However, the regular (i.e. not trivial) cofibration case in the previous paragraph together with a routine induction argument over the cell decomposition of cellular $\P$ shows that cellular  
...
are genuine projective equivalences and hence also projective equivalences, showing that the condition in \cite[Lemma 2.3(1)]{SS00} holds,
...
one replaces \cite[Lemma 2.3(1)]{SS00} with the obvious analogue (unfortunately, we know of no direct reference for this analogue, but its proof is identical).
...
Remark \ref{GENCOFGEN REM}. This analysis is the subject of \S \ref{FAMILY DEF} and \S \ref{PUSHPOW SEC}, the results of which are converted 
...
$(X_e)_{e \in E}$ in $(\Fin_s \wr \mathcal{V})^K$.
...
$X_e \to X_{g e}$.
Moreover, writing $K_e$ for the isotropy of $e \in X$,
note that the induced fixed point isomorphism
$X_e^{K_e} \to X_{g e}^{K_{ge}}$
does not depend on the choice of coset representative $g \in g K_e$,
and we will thus abuse notation
by writing
$X_{[e]}^{K_{[e]}} = X_f^{K_f}$ for an arbitary choice of representative $f \in [e] = Ge$
...
$X^H \otimes Y^H \xrightarrow{\simeq} (X \otimes Y)^H$, for $X,Y \in \mathcal{V}^H$.
