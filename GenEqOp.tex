...
We now recall the following basic notion and result concerning model structures induced by collections of subgroups.

\begin{definition}\label{CELL_DEF}
  We say a cofibrantly-generated model category $\mathcal{V}$ has 
...
\begin{proposition}[{\cite[Prop. 2.6]{Ste16}}]
  \label{FMODELEXIST_PROP}
...
We will also need extensions of these model structures to diagram categories, as we now explain.
...
\todo[inline]{come back: continue to correctly identify SS00 over Kan}
...
\begin{definition}
  ...
  \end{enumerate} 
  If $\V$ is additionally Cartesian, we say it is \textit{Cartesian} strongly cellular.
\end{definition} 


For our model structures on $\Op_G(\V)$ and $\Op^G(\V)$, we will need to consider a particular class of (sub)groups.
\todo[inline]{come back: better transition}

\begin{definition} 
  A \textit{$G$-graph subgroup} of $G\times \Sigma_n$ is a subgroup $\Gamma \leq G\times \Sigma_n$ such that $\Gamma \cap \Sigma_n = e$. 
\end{definition} 
\begin{remark}
  It is an easy observation $\Gamma \leq G\times\Sigma_n$ is a $G$-graph subgroup iff $\Gamma = \Gamma(\phi)$ is the graph of some partial homomorphism $G \geq H \xrightarrow{\phi} \Sigma_n$.  
\end{remark}

\begin{definition} 
  A \textit{$G$-vertex system} is a collection 
  \[ 
  \F = \set{F_n}
  \] 
...
Now, fix a $G$-vertex system $\F$. %, and throughout this subsection we suppose that $\V$ is a cofibrantly-generated Cartesian monoidal model category with cellular fixed points (with Cartesian necessary for the monad $\mathbb F_G$).

\begin{definition} 
  A map $f: \O \to \P$ in $\Op_G(\V)$ is called an  
  \begin{itemize} 
...
\begin{definition} 
  The \textit{$\F$-(semi) model structure} on $\V\Op_G$, if it exists, is the unique (semi) model structure with $\F$-fibrations, $\F$-weak equivalences, and $\F$-cofibrations. Equivalently, it is the transfered (semi) model structure along the adjoints 
  \[ 
  \begin{tikzcd} 
...
\begin{definition} 
  If $\F$ is the complete $G$-vertex system --- that is, each $\F_n$ is the family of \textbf{all} graph subgroups of $G\times \Sigma_n$ --- we refer to the $\F$-model structure as the \textit{genuine} model structure. 
\end{definition} 
...
\begin{theorem}\label{GENUINE_SEMI_VOP_G_THM} 
  The genuine semi-model structure on $\V\Op_G$ exists for all Cartesian strongly cellular $\V$.
\end{theorem} 

The key technical result is a particular case of the stronger result from Section \ref{COFIB SEC}

\begin{proposition}[c.f. {Proposition \ref{AUTTCOFPUSH PROP}}]
\label{GENUINE_TREE_BOX_COFIBRANT_PROP} 
  Suppose $\V$ is Cartesian strongly cellular, $T\in \Omega_G$, and we are given cofibrations $f(Gv) \in \V^{\mathrm{Aut}(T_{Gv})}_{gen}$ for all $Gv\in V_G(T)$, such that $f(Gv) = f(\alpha(Gv))$ for all $\alpha\in \mathrm{Aut}(T)$. Then the iterated box product 
  \[ 
...
\begin{proof}[Proof of Theorem \ref{GENUINE_SEMI_VOP_G_THM}] 
  Using \cite[Lemma 2.3]{SS00} and the fact that transfinite compositions are constructed levelwise, this follows directly from Proposition \ref{CELLULAR_LEVEL_COFIB_PROP} and Remark \ref{LEVEL_COFIB_REMARK},  
  % , and all genuine (trivial) cofibrations in $\V^{\Sigma_G^{op}}$ and $\V\Op_G$ are so levelwise. 
\end{proof} 
...
\begin{corollary} 
  ...
  For any $G$-vertex system $\F$, the $\F$-semi-model structure on $\V\Op_G$ exists if $\V$ is Cartesian strongly cellular. \qed
\end{corollary} 
...
\begin{theorem} 
  ...
  \end{itemize}
  then the $\F$-model structure on $\V\Op_G$ exists for any $G$-vertex system $\F$.
\end{theorem} 
\begin{proof} 
...
We now turn our consideration back to regular $G$-operads $\O \in \V\Op^G$. Fix a $G$-vertex system $\F = \set{\F_n}$.
...
\begin{definition}
  ...
  Suppose $\V$ has cellular fixed points. A map $f: \O \to \P$ in $\V\Op^G$ is called a
  \begin{itemize}
...
\begin{definition}
  The \textit{$\F$-(semi) model structure} is the unique (semi) model structure on $\V\Op^G$, if it exists, with $\F$-fibrations, $\F$-cofibrations, and $\F$-weak equivalences. If $\F$ is the complete $G$-vertex family, we call this the \textit{genuine} model structure on $\V\Op^G$.
\end{definition}
...
\begin{theorem}
  The genuine semi-model structure on $\V\Op^G$ exists whenever $\V$ is Cartesian strongly cellular.
\end{theorem}
\begin{proof}
  As in the proof of Theorem \ref{GENUINE_SEMI_VOP_G_THM}, by \cite[Lemma 2.3]{SS00} it suffices to show all free $\mathbb F$-extensions over level genuine trivial cofibrations on a genuine level cofibrant $G$-operad are themselves genuine level cofibrantions.

  We note that if $u: X \to Y$ is a genuine cofibration in $\V^{G\times \Sigma}_\F$, then $i_! u$ is certainly a genuine level cofibration in $\V^{\Sigma_G^{op}}$. 
...
\end{proof}
\begin{remark}
  The Cartesian condition can be dropped from the above theorem. Indeed, if we restrict the machinery of Section \ref{GENUINE_OP_MONAD_SECTION} for $G = *$ and $\V = \V^G$, then Remark \ref{NEED_WREATH_REMARK} no longer applies, and hence we may reform the monad on spans from Definition \ref{WSPAN_MONAD_DEFINITION} by replacing $\Fin$ with $\Fin_{\mathsf{Iso}}$ and using the natural map $\mathsf F_{\mathsf{Iso}} \wr \V^{op} \xrightarrow{\otimes} \V^{op}$ for \textit{any} symmetric monoidal product $\otimes$ on $\V$.
\end{remark}
...
\begin{remark}
It follows from (the argument in the proof of) Proposition \ref{STRONGCELL PROP} that the following
\[
\begin{tikzcd}
	\mathcal{V}^{\mathsf{O}_G^{op}} \ar{d}[swap]{\phi_!} &
	\mathcal{V}^{G} \ar{l}[swap]{\iota_{\**}} \ar{d}{\phi_!}& & &
	\mathcal{V}^{\mathsf{O}_{\mathcal{F}}^{op}} \ar{d}[swap]{\phi_!} &
	\mathcal{V}^{G}_{\mathcal{F}} \ar{l}[swap]{\iota_{\**}} \ar{d}{\phi_!}&
\\
	\mathcal{V}^{\mathsf{O}_{\bar{G}}^{op}}  &
	\mathcal{V}^{\bar{G}} \ar{l}{\iota_{\**}}& & &
	\mathcal{V}^{\mathsf{O}_{\mathcal{\bar{F}}}^{op}}  &
	\mathcal{V}^{\bar{G}}_{\mathcal{\bar{F}}} \ar{l}{\iota_{\**}}&
\end{tikzcd}
\]
commute when applied to genuine cofibrant/$\mathcal{F}$-cofibrant objects.
\end{remark}
...
\begin{theorem}
  ...
  Suppose $\F$ is a weak indexing system and $\V$ is strongly cellular. If $\O \in \Op^G_\F(\V)$ is cofibrant, then so is $U(\O) \in \Sym^G_\F(\V)$.
\end{theorem}

The bridge between the previous section and this result is the following:
\begin{proposition}
  If $\O\in \Op0^G(\V)$ is level $\F$-cofibrant, and $u: X \to Y$ is a cofibration in $\Sym^G_\F(\V)$, then the $\mathbb F$-cellular extension
  \[
  \begin{tikzcd}
...
\end{proposition}
\begin{proof}
  Using the observation that $\Op^G(\V) = \Op(\V^G)$, we use the filtration (\ref{FILTRATION_LAN_LEVEL}) with $G= \set{e}$ and $\V = \V^G$. Thus, it suffices to that that, for all $T\in \Omega$, the map
\[
\left( 
  \mathop{\bigotimes}\limits_{v \in V^{ac}(T)}\P(T_v) \otimes \mathop{\Box}\limits_{v\in V^{in}(T)}u(T_v)
\right) \cdot_{\mathrm{\Sigma_T}}\Sigma_n
\]
...
Lastly, since $\$P$ is level fibrant, the given map in $\V^{G\times \Sigma_T}$ is a pushout product of the form in Proposition \ref{AUTTCOFPUSH PROP}, and hence is an $\F_T$-cofibration.
...
\begin{corollary}
  ...
  The class of level $\F$-cofibrant operads in $\Op^G(\V)$ is closed under cellular extensions. \qed
\end{corollary}

\begin{proof}
  [Proof of Theorem \ref{COFIBRANT_G_OPS_THEOREM}]
  Since the initial operad is always $\F$-cofibrant, and any $\F$-cofibrant operad can be built out of a retract of a composite of cellular extensions starting withe initial operad, this follows from Corollary \ref{F_COFIB_OP_CLOSURE_THM} and the observation that $\F$-cofibrations are closed under retracts.
\end{proof}

Combining the existence of $\F$-model structures and the above cofibrancy result, we give our first proof of the following conjecture of Blumberg-Hill \cite{BH15}.
...
\begin{proof}
  ...
  \end{tikzcd}
  \]
  in $\Op^{G}_\F(\sSet)$ of the unique map into a cofibration and trivial fibration. Since the initial operad is cofibrant, Theorem \ref{COFIBRANT_G_OPS_THEOREM} implies that $\varnothing \to N\F$ is a level $\F$-cofibration, and hence each $N\F(n)$ is cofibrant in $\sSet^{G\times \Sigma_n}_{\F_n}$; hence, for all $\Gamma \not\in\F_n$, $N\F(n)^\Gamma = \varnothing$. Further, since $N\F$ is $\F$-equivalent to $*$, $N\F(n)^\Gamma \simeq *$ for all $\Gamma \in \F_n$. Hence, each $N\F(n)$ is a universal space for $\F_n$, as desired.
\end{proof}
...
\subsubsection{Cofibrant Genuine $G$-Operads}
