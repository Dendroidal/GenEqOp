...
\subsection{Genuine equivariant operads versus equivariant operads}
...
We now consider the associated adjunction.
\begin{equation}\label{JUSTONEDJOINT EQ}
\begin{tikzcd}
	\iota^{\**} \colon 
	\mathsf{Sym}_G \ar[shift left =2pt]{r} 
	&
	\mathsf{Sym}^G 
...
As a side note, we note that the formula for $\iota_{\**}$ is independent of the chosen order of $G$.

The adjunction (\ref{JUSTONEDJOINT EQ}) identifies
$\mathsf{Sym}^G$ 
...
\end{tikzcd}
\end{equation}
The map $\mathcal{F} \to \iota^{\**} \mathbb{F}_G \iota_{\**}$ is then induced by the obvious ``inclusion'' natural transformation of (\ref{F EQ}) into (\ref{IFGI EQ}) when $B=\iota_{\**} A$, with the fact that this is a map of monads following from the analogous diagram for strings $\Omega_{G,1}$.
\end{proof}
...
\begin{corollary}
	The adjunction (\ref{JUSTONEDJOINT EQ}) extends to an adjunction
