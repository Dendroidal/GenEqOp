...
\begin{equation}\label{D6SMALLER EQ}
	...
	\end{tikzpicture}%
\end{equation}%
We note that $G$ acts on the expanded representation of $T$ as indicated by the edge labels (so that the edges $a,b,c,d$ have stabilizers $L$, $K$, $K$ $H$), and the orbital representation is obtained by collapsing the edge orbits of the expanded representation. As explained in \cite[Example 4.9]{Pe17}, $T$ then encodes the fact that for any 
...
\O(H/L \amalg H/K)^{H}
\end{equation}
where $\O(X)$ for an $H$-set/$K$-set $X$ denotes $\O(|X|)$ together with a suitable combined $H$-action/$K$-action.
...
\times \cdots \times
	...
	\to
\\
\to & \mathcal{P}(H / L_{11} \amalg \cdots \amalg H/L_{1 m_1}
\amalg \cdots \amalg
H / L_{n1} \amalg \cdots \amalg H/L_{n m_n}
)
\end{align}
generalizing (\ref{INTFIXPTCOMP EQ}) (which corresponds to the case $n=1$, $m_1 = 2$, $L_1 = L$, $L_2 = K$).
Lastly, the multiplication maps (\ref{FIXEDPOINTMUL EQ})
must satisfy associativity, unitality, compatibility with restriction maps and equivariance conditions as encoded by the theory of $G$-trees (though we note that, rather than make such compatibilities explicit, it will be more convenient for our purposes to simply define genuine equivariant operads intrinsically in terms of $G$-trees).
...
\[
\begin{tikzcd}
	\mathcal{E} \ar{rr} \ar{rd} && \bar{\mathcal{E}} \ar{dl}
\\
	& \mathcal{B}
\end{tikzcd}
\]
such that the top functor preserves cartesian arrows (pullbacks).

There is a well known equivalence between Grothendieck fibrations over $\mathcal{B}$ and pseudo-functors
$\mathcal{B} \to \mathsf{Cat}$
with split fibrations corresponding to (regular) functors, and we now recall how this works in the latter case.
...
given a functor $F \colon \mathcal{E} \to \mathcal{C}$ into a complete category $\mathcal{C}$ one has
\[
	\mathsf{Ran}_{\pi}F (b)
\simeq
...
(in fact, pull $\mathcal{E}_b$ is a )
