...
% along the homomorphism \eqref{GRAPHMAP_EQ}\footnote{Here, as in \eqref{GENGENMULT EQ}, our descriptions are only up to isomorphism, as there is no total order given on $X$.
%   Formally, we are detailing $i_{\**}\O$, which is indexed over the category $\Sigma_G$ of ``planar $G$-corollas''; these encode (slightly more than) an $H$-set equipped with a total order (see \S \ref{LRVERT SEC} for more information).}
...
for example \cite[Defn. 5.8]{BP_edss} and \cite[Prop. 4.47]{BP_TAS}.
...
the operadic nerve functor $N^\otimes \colon \sOp \to \Op_\infty$ \cite[Defn. 2.1.1.3]{Lu09}.
...
%   \item $\Op_G(\sSet)$ are the natural source for the genuine equivariant generalization of the operadic nerve $\Op(\sSet) \to \Op_\infty$, used in {\color{red} Horev} for equivariant factorization homology
...
Note that, in light of the identifications
$\mathbb{F} \simeq 
\iota^{\**} \mathbb{F}_G \iota_! \simeq
\iota^{\**} \mathbb{F}_G \iota_{\**}$,
\eqref{KEYNONISO EQ} is identified with a map
$\mathbb{F}_G \iota_{\**} \to  \iota_{\**} \mathbb{F}$,
so whenever \eqref{KEYNONISO EQ} is an isomorphism
at $X \in \mathsf{Sym}^G(\mathcal{V})$
one has $\mathbb{F}_G \iota_{\**} X \simeq  \iota_{\**} \mathbb{F}X$.

Thus, the claim that \eqref{KEYNONISO EQ} is an isomorphism
at $X \in \mathsf{Sym}^G(\mathcal{V})$ implies that
$\mathbb{F}_G \iota_{\**} X \simeq  \iota_{\**} \mathbb{F}X$.

{\color{red} HERE}

is equivalent to $\mathbb{F}_G \iota_{\**} \simeq  \iota_{\**} \mathbb{F}$.
Informally, the latter means that,
at
$X \in \mathsf{Sym}^G(\mathcal{V})$
is equivalent, by \eqref{IOTAFUNSALT EQ},
to the claim that the graph fixed points
...
Hence, the claim that \eqref{KEYNONISO EQ}
is an isomorphism is tantamout to the claim that the graph fixed points of $\mathbb{F} X$
are computed in terms of $G$-trees via 
$\mathbb{F}_G \iota_{\**}$
...
\begin{remark}
      ...
      The \textit{power cofibration axiom} of \cite[Def. 4.5.4.2(iii)]{Lur17}
      has a similar form to cofibrant symmetric pushout powers,
      except it asks for the cofibrancy of $f^{\square n}$ in the \textit{projective} model structure $\V^{\Sigma_n}_{\text{proj}}$.
      This axiom, along with some technical conditions,
      % along with the \textit{monoid axiom} of \cite{SS00} and some small technical conditions,
      is sufficient for the construction of a variety of projective (semi-)model structures
      on categories of algebraic objects,
      ...
      However, the power cofibration axiom is very demanding; in particular, it is not satisfied by $\sSet$.
      This has lead to a number of more general variants,
      including
      ...
      \cite[Defn. 2.1]{PS18b}.
      % and \cite[Defn. 4.2.8, Remark 4.2.10]{PS18a}.
      However, neither the power cofibration axiom nor any of these variants
      see any fixed-point information in $\V^{\Sigma_n}$, and thus cannot be used to prove our main results.
