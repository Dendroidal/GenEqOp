...
a colored operad $\Omega(T)$,
as follows.
The colors/objects of $\Omega(T)$
are simply the edges $a,b,c,\cdots,i$
while the generating operations,
corresponding to the nodes,
are 
$ab \to f$, $cde \to g$, $\emptyset \to h$, $fgh \to i$
(i.e., for each node, incoming edges are viewed as inputs 
and the edge below as an output).
...
so that
\eqref{COMPEX EQ}
is recovered as the induced map of mapping sets
(for $\mathsf{Op}_{\bullet}$ the category of colored operads)
$\mathsf{Op}_{\bullet}
...
Analogously, the role of equivariant trees is, in the context of equivariant operads, to encode operadic compositions
as in \eqref{COMPEX EQ}
...
%
A detailed introduction to (and motivation of) 
equivariant trees can be found in \cite[\S 4]{Pe17}, where the second author develops the theory of equivariant dendroidal sets (a parallel approach to equivariant operads), though here we include only a single representative example.
...
which is labeled with the corresponding edge set of orbits in the expanded representation
(one may also reverse this process,
though we will not need to do so).
We note that orbital representations always ``look like a tree''.


As explained in \cite[Example 4.9]{Pe17},
$T$ encodes the fact that, for $\O \in \mathsf{Op}^G$ a $G$-operad,
the composition 
...
\O(H/L \amalg H/K)^{H}
\end{equation}
(we further unpack \eqref{INTFIXPTCOMP EQ}
in the next paragraph)
where $\O(X)$ for $X$ an $H$-set denotes $\O(|X|)$ 
with the $H$-action
given by the identification $H \simeq \Gamma_X$
(the graph subgroup $\Gamma_X$ is discussed after 
\eqref{NORMMAPS EQ}),
...
To make the connection between $T$ and \eqref{INTFIXPTCOMP EQ}
precise, note that, as in the non-equivariant case,
one has a $G$-equivariant colored operad
$\Omega(T)$,
as well as a $G$-corolla $C$
formed by the leaves and roots of $T$,
together with a map 
$\Omega(C) \to \Omega(T)$
of $G$-equivariant colored operads.
Then, writing 
$\mathsf{Op}_{\bullet}^G$
for the category of $G$-equivariant colored operads,
\eqref{INTFIXPTCOMP EQ}
is simply the induced map
$\mathsf{Op}_{\bullet}^G
