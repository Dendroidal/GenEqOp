...
\subsection{Transferring simplicial colimits of left Kan extensions}
...
In practice, we will need to know that $s$ satisfies a stronger  finality condition with respect to left Kan extensions, stated as Corollary \ref{SOURCELANFINAL COR}. The necessary lemma follows.
...
\begin{example}
Let $\O$ be a $G$-genuine operad.
Each level of its free resolution $\mathbb{F}^{\bullet+1} \O$ can be realized as a left Kan extension over the simplicial category $(\Omega^{q}_{G,\bullet})^{op}$. Moreover, noting that the multiplication 
$m \colon \mathbb{F} \O \to \O$
is a map
$m \colon 
	\Lan_{\mathsf{lr}}
	\Pi \circ \Fin \wr \O \circ V
\to
	\O
$
and thus equivalent to a natural transformation
$\varphi_0 \colon 
\Pi \circ \Fin \wr \O \circ V \to \O \circ \mathsf{lr}$,
one sees that $\mathbb{F}^{\bullet+1} \O$
can be viewed as obtained by applying $\mathsf{Lan}$
to an object 
$(\Omega^{q}_{G,\bullet})^{op} \in 
\mathsf{Span}(\Sigma_G^{op},\sSet)$,
or, to ease notation, by applying $\mathsf{Ran}$
to the opposite object 
$\Omega^{q}_{G,\bullet} \in 
\mathsf{Span}(\Sigma_G,\sSet^{op})$.

However, this object does not quite satisfy 
(\ref{TWISTEDARROWBLA EQ}): indeed, that would be equivalent to the statement that the top sections of both (\ref{FMULT EQ})
and (\ref{FID EQ}) are strictly commutative diagrams, but in the case of (\ref{FMULT EQ}) this commutativity is only up to isomorphism. 
However, on can instead simply ``twist'' this simplicial object  by the natural isomorphisms in (\ref{FMULT EQ}) to obtain a simplicial object, which we will also denote $\Omega^{q}_{G,\bullet} \in 
\mathsf{Span}(\Sigma_G,\sSet^{op})$
now satisfying the conditions of (\ref{TWISTEDARROWBLA EQ}).
Note that in this new object the map $F_n$ is the composite
\[
	\Omega_{G,n}^q
		\xrightarrow{d_{0,\cdots,n-1}}
	\Omega_G
		\xrightarrow{V}
	\Fin \wr \Sigma_G
		\xrightarrow{\Fin \wr \O}
	\Fin \wr \sSet^{op}
		\xrightarrow{\Pi}
	\sSet.
\]
