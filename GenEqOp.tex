...
An upshot of this discussion is that there are multiple possible notions of equivariant homotopy commutativity. Blumberg-Hill \cite{BH15} characterized all possible options, using the following definition of $N_\infty$ (a term they coined):
\begin{definition}
...
\end{definition}
As each homomophism $H \to \Sigma_n$ encodes an $H$-set structure on $\underline{n} = \set{1,2,\ldots, n}$, a collection of families $\F = \set{\F_n}$ is equivalent to a collection of $H$-sets $\F(G/H)\subseteq \Fin^H$ for each $H\leq G$. However, if $\F$ came from an $N_\infty$-operad, these $\F_n$, and hence the $\F(G/H)$, were subject to compatibility conditions coming from the operadic structure. 
