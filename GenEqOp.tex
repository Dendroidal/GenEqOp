...
\begin{proof}
All claims will be shown in parallel.
Further, since the argument for the ``essential image'' claim for the symmetric sequence adjunction in 
(\ref{COFADJ EQ}) is simpler than that for the
operad adjunction, we will mainly refer to the latter. 

Note first that since $\iota_{\**}$ identifies 
$\mathsf{Op}^{G}(\mathcal{V})$ as a reflexive 
subcategory of $\mathsf{Op}_{\mathcal{F}}(\mathcal{V})$, 
it is 
$X \simeq \iota^{\**}Y$ for some 
$Y \in \mathsf{Op}^{G}(\mathcal{V})$
(i.e. $X \in \mathsf{Op}_{\mathcal{F}}(\mathcal{V})$ is in the essential image of $\iota^{\**}$)
iff both $\iota_{\**}X \simeq Y$ and the unit map 
$X \xrightarrow{\simeq} \iota_{\**} \iota^{\**}X$
is an isomorphism.
...
where we slightly abuse notation by writing 
$\iota_{\**}(\minus)$ for the essential image rather than the image.
Since $C_{\mathcal{F}}$ is characterized as the smallest class closed under retracts and transfinite composition of cellular extensions
containing the initial genuine operad $\mathbb{F}_G (\emptyset)$,
and suffices to show that 
$\iota_{\**}(C^{\mathcal{F}})$
satisfies this characterization.
An immediate verification shows that 
$\mathbb{F}_G (\emptyset) \simeq \iota_{\**} \mathbb{F} (\emptyset)$
while the alternate characterization of the essential image of $\iota_{\**}$ in the previous paragraph shows that 
$\iota_{\**}(C^{\mathcal{F}})$
is also closed under retracts.
Since $\iota_{\**}$ commutes with transfinite compositions, 
it suffices to check that cellular extensions of 
objects in $\iota_{\**}(C^{\mathcal{F}})$, such as on the left below
...
(\ref{TWOCELLEXT EQ})
are in fact adjoint, so that there is a bijection between such data. 
...
\iota_{\**} \mathbb{F} Z
$ for $Z=X,Y$
and since $\iota_{\**}$ reflects colimits (i.e. any diagram that becomes a colimit upon applying $\iota_{\**}$ must have already been a colimit diagram), it must then be
indeed be 
$(\iota_{\**} \mathcal{O})[\iota_{\**} u]
\simeq \iota_{\**} (\mathcal{O}[u])$, 
so that the leftmost diagram in 
(\ref{TWOCELLEXT EQ})
is indeed isomorphic to the image of the rightmost diagram under $\iota_{\**}$.

To establish the remaining claim that the leftmost diagram in
(\ref{TWOCELLEXT EQ})
is indeed in the essential image of $\iota_{\**}$, it suffices to check that this is the case for the bottom right corner 
$(\iota_{\**} \mathcal{O})[\iota_{\**} u]$.
Indeed, the claim concerning $\mathbb{F}_G \iota_{\**}Y$
can be reduced to the claim concerning to check that this is the case for the bottom right corner 
$(\iota_{\**} \mathcal{O})[\iota_{\**} u]$ by setting $X=\emptyset$
and $\mathcal{O}=\mathbb{F} \emptyset$ the initial operad, and similarly for $\mathbb{F}_G \iota_{\**}X$.
...
(\ref{FILT EQ}) for the map
$ \mathcal{P} \to \mathcal{P}[\iota_{\**} u]$, and it suffices to verify by induction that each $\mathcal{P}_k$ is in the essential image of 
$\iota_{\**} \colon \mathsf{Sym}^G(\mathcal{V})
\to \mathsf{Sym}_{\mathcal{F}}(\mathcal{V})$.
Using the inductive description of the $\mathcal{P}_k$ in
