...
This paper is part of a series of papers by the authors \cite{Pe17,BP_edss,BP_HGOP,BP_TAS}
analyzing and understanding different models for the homotopy theory of equivariant operads with norm maps.
These papers generalize the program of Cisinski, Moerdijk, and Weiss \cite{MW07,MW08,CM11,CM13a,CM13b},
and a major result is the existance of a Quillen equivalence
\begin{equation}
      \label{DSETSOP_EQ}
      \begin{tikzcd}[column sep =4em]
...
model category of equivariant dendroidal sets of \cite{Pe17}, and 
$\mathsf{sOp}^G$ is the category of equivariant many-colored simplicial operads equipped with the many-colored variant of the model structure from Theorem \ref{MAINEXIST1 THM} from \cite[Theorem III]{BP_HGOP}.
...
Nonetheless, we conjecture that the remaining
Theorems \ref{MAINEXIST2 THM} and \ref{MAINQUILLENEQUIV THM}
...
While (many-colored) genuine equivariant operads as defined in this paper
...
%
In every model category realizing equivariant operads with norms,
(co)fibrations and weak equivalences are described using $G$-trees.
%
Moreover,
recall that a category (resp. operad) is equivalent, under the nerve functor, to a strict inner Kan complex in $\sSet$ \cite[Prop. 1.1.2.2]{Lu09} (resp. $\dSet$ \cite[Prop. 5.3, Thm. 6.1]{MW08}).
Similarly, we will show in a sequel that
a genuine $G$-operad is equivalent, under the nerve functor $\Op_G \to \dSet_G = \Set^{\Omega_G^{op}}$,
to a strict inner Kan complex (cf. \cite[Remark 3.42]{BP_edss}),
and this combinatorial notion of a genuine $G$-operad appears and underlies
key steps in the proofs of the main results from each of \textit{loc cite}
(e.g. \cite[Defn. 5.8]{BP_edss} \cite[Prop. 4.47]{BP_TAS}).
...
for any $G$-operad $\O$ and $G$-space:
while it is clear that $X^H$ is an algebra over $\O^H$, and hence
$\pi_{\**}(X^H)$ is an algebra over $\pi_{\**}(\O^H)$,
the genuine equivariant operad paradigm tells us that in fact there are twisted structure maps, and instead
the coefficient system $\pi_{\**}(\iota_{\**}X)$ is an algebra over the genuine equivariant operad (of sets) $\pi_{\**}(\iota_{\**}\O)$.

\begin{example}
      Let $G = \mathbb Z/2 = \set{1,\tau}$ be the cyclic group of order 2,
      and consider the three (orbital representations of) $G$-trees below:
      \begin{equation}
            \begin{tikzpicture}
            ...
            \end{tikzpicture}
      \end{equation}
      These encode certain algebraic structure maps,
      by placing a 2-ary operation in $\O$ of appropriate isotropy in the root node,
      and then elements of $X$ of appropriate isotropy in the stumps.
      The first two trees encode the algebra structure maps
      \[
            \O(2) \times X^{\times 2} \to X,
            \qquad
            \O(2)^G \times (X^G)^{\times 2} \to X^G
      \]
      (where we note that $\Gamma_{G/G \amalg G/G} = G \leq G \times \mathbb Z/2$).
      However, this last map encodes the twisted structure map
      \[
      ...
      \[
            ...
            X^G
      \]
      where $\Delta_{\mathbb Z/2}$ sends $x$ to $(x, \tau x)$.
      This induces a map on homotopy groups, for example
      \[
...
can be applied in many contexts to define new equivariant algebraic notions.
...
reconfiguring the description of $\mathbb F_G$ used here to synthesize with the monad $\Sigma \wr (-)$ defining symmetric monoidal categories.
...
Finally, the comparison between $G$-operads $\sOp^G$ and the $G$-$\infty$-operads of {\color{red}Barwick and co}
factors most naturally through the category of genuine $G$-operads $\sOp_G$.
Non-equivariantly, this comparison is given by
the operadic nerve functor $N^\otimes \colon \sOp \to \Op_\infty$ \cite[]{Lu09}.
In \cite{BNerve}, the first-named author extends this to a functor $N^{\otimes} \colon \sOp^G \to sOp_G \to \Op_{\infty,G}$.
In particular, this builds a $G$-$\infty$-operad associated to any topological operad $\O$,
and moreover gives a comparison of categories of algebras $\Alg(\O) \to \Alg(N^{\otimes})$.
When $\O$ is the genuine equviariant little disks operad,
this relation has been used by {\color{red} Horev} to give a characterization of equivariant factorization homology.
