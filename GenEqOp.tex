...
\begin{customthm}{I}\label{MAINEXIST1 THM}
        ...
        the $\mathcal F$ semi model structure on $\Op^G(\V)$, 
        where weak equiavlences and fibrations determined as in \ref{NFINFTY EQ} --- that is, the model structure with weak equivalences and fibrations determined by the forgetful functor
        \[
...
\[
\Op_{G / \F}(\V) \xrightarrow{\mathmakebox[5em]{\mathsf{fgt}}} \Sym_{G / \F}(\V).
\]
...
\begin{customthm}{III}\label{MAINEXIST3 THM}
        For any weak indexing system $\F$,
...
We begin by describing general notions of projective model structures on diagram categories, starting with a categorically simple case.
...
\begin{remark}
        ...
        \item [(i)] If $\phi: \Pi \to \bar \Pi$ is a group homomorphism, then 
                both the induction map $\phi_!$ and the forgetful functor $\phi^*$ in the adjunction
                \[ 
                \phi_!: \bar\Pi\cdot_\Pi(-): \V^\Pi_{gen} \leftrightarrows \V^{\bar \Pi}_{gen}: \phi^*
                \] 
                are left Quillen for the genuine model structures 
                ($\phi^*$ is left Quillen against the coinduction map $\phi_*$). 
...
\begin{definition}
      ...
      If each $\F_d$ is the set of all subgroups of $\mathrm{Aut}(d)$, we refer to the above as the $\bar{\mathcal D}$-projective \textit{genuine} model structure.%, and write $\V^{\mathcal D}_{\bar{\mathcal D}}$. 
...
We note that if we instead are analyzing presheaf categories $\V^{\mathcal D^{op}}$, the result above follows if $\bar{\mathcal D}\subseteq \mathcal D$ has the dual property of being a \textit{seive}.
...
\begin{example}
      ...
      for any family $\mathcal F$ of subgroups of $G$,
      ...
      exist, and the transfer adjunction
      \[
...
\begin{remark}
      ...
      $\V^{\mathcal D^{op}}_{\text{gen}}$ (and hence any in $\V^{\mathcal D^{op}}_{\F}$ for any $\F$), 
...
The key technical result is an easy particular case of the stronger Proposition \ref{AUTTCOFPUSH PROP}.
...
\begin{proof}[Proof of Theorem \ref{G_OP_SEMI_MODEL_THM}]
        Using \cite[Lemma 2.3]{SS00}, the fact that weak equivalences and transfinite compositions are levelwise implies that Corollary \ref{CELLULAR_LEVEL_COFIB_PROP2} and Remark \ref{GENUINE_FGTRIGHT_REMARK} are sufficient.  
\end{proof}
...
\begin{remark}
        ...
        we may reformulate the monad on spans from Definition \ref{WSPAN_MONAD_DEFINITION} by replacing $\Fin_s$ with $\Fin_{\mathsf{Iso}}$ and using the natural map $\mathsf F_{\mathsf{Iso}} \wr \V^{op} \xrightarrow{\otimes} \V^{op}$ for \textit{any} symmetric monoidal product $\otimes$ on $\V$.
\end{remark}

Somewhat surprisingly, the existance of $\F$-model structures for non-complete $\F$ also follow from \ref{CELLULAR_LEVEL_COFIB_PROP2}, as level (trivial) $\F$-cofibrations are also level (trivial) genuine cofibrations.
\begin{corollary} 
...
\begin{corollary}
        ...
        both $\Sym_\F(\V)$ and $\Sym_{G / \F}(\V)$ exist and are Quillen equivalent.
\end{corollary}

\begin{remark}
        The above holds for any two nested sieve subcategories $\Sigma_{\bar \F}\subseteq \Sigma_\F$ of  $\Sigma_G$, not just $\Sigma_\F$ and $\Sigma_G$ itself.
\end{remark}
...
\begin{proof}
        ...
        \[
        \beta^*: V^{\Pi \Sigma_{\llbracket v \rrbracket} \wr \mathsf{Aut}(T_{[v]})} 
        \to 
        \V^{\mathsf{Aut}(T)}
        \]
        preserves genuine cofibrations. 
        Thus, it suffices to show that $f^{\Box V_G(T)}$ is cofibrant on the left hand side. 
        This follows immediately by Proposition \ref{POWERF PROP} and Remark \ref{GENUINE_FGTRIGHT_REMARK}$(ii)$.
        % \[
...
Again, we find that the existance of the $\F$ semi model structures follows identically as the genuine. 
...
\begin{example} 
        The $\F$-model structure exists on $\s\Op_G$ for any sieve subcategory $\Sigma_\F \subseteq \Sigma_G$.
\end{example} 
...
In order to homotopically analyze this modified monad, we would like a result mimicking the filtration (\ref{FILTRATION_LAN_LEVEL}) but for free $\mathbb F_\F$-extensions. Here, we will need to use the new closure condition on weak indexing systems.
...
\end{lemma}
\begin{proof}
        This follows identically to the proof of Proposition \ref{MONAD_COMPARISON_PROP},
...
\begin{proof}
        ...
        \left(
                \prod\limits_{v \in V^{ac}_G(T)}\upgamma_{\**}\P(T_v) \times \mathop{\square}\limits_{v \in V^{in}_G(T)} \upgamma_! u(T_v)
        \right)
        \]
        ...
        and thus this is isomorphic to the left Kan extension over $\Omega_\F^a[k]^{op}$ of the same functor, 
...
\begin{definition}
        Let $\Sigma_{\bar\F} \subseteq \Sigma_\F$ be a sieve subcategory. 
        ...
        \end{enumerate}
        The \textit{$\bar\F$ (semi) model structure} on $\Op_\F(\V)$, if it exists, is the (semi) model structure denoted $\Op_{\F/\bar\F}(\V)$ with the above $\bar\F$ weak equivalences and fibrations. 
...
% \end{corollary}
Thus, mimicking the proofs of Theorem \ref{OP_G_SEMI_MODEL_THM}, Corollary \ref{OP_F_SEMI_MODEL_THM}, and Theorem \ref{OP_F_Q_MODEL_THM} yields the following result.
\begin{theorem}
...
\[
\iota_G^{\**} \mathbb{F}_{G} \iota_{G,\**}
\simeq  \mathbb{F} \iota_G^{\**} \iota_{G,\**} \simeq \mathbb{F}
\qquad
