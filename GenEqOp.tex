...
However, 
we have the following adjustments of Proposition \ref{MONAD_COMPARISON_PROP} and Corollary \ref{TWOADJOINTSOP_COR}) for highly-structured $\F$.
...
\end{corollary}  
\begin{proof} 
        As we know $\upgamma_!$ preserves pushouts of operads by Corollary \ref{F_TWOADJOINTSOP_COR},
        applying $\upgamma_!$ to the free $\mathbb F_\F$-extension above yields a pushout
        \[
        \begin{tikzcd}
                \mathbb F_G \upgamma_! X \arrow[r] \arrow[d] & \upgamma_!\P \arrow[d]\\
                \mathbb F_G \upgamma_! Y \arrow[r] & \upgamma_! \P[u].
        \end{tikzcd}
        \]
        It is clear that, given the starting assumptions, $\upgamma_! u$ and $\upgamma_! \P$ are level cofibrations and level cofibrant, respectively. 
        Hence, since $\upgamma^{\**}$ also preserves pushouts of operads (and level cofibrations), 
        the result follows by using the levelwise filtration from %(\ref{FF_FREE_EXTENSION}),
        (\ref{FILTRATION_LAN_LEVEL}), 
        and applying Corollary \ref{GENUINE_TREE_BOX_COFIBRANT_PROP} and Remark \ref{GEN_FGTRIGHT_REMARK}(i). 
\end{proof} 
...
\begin{proof}
        ...
        \begin{equation}\label{TWOCELLEXT EQ}
                ...
                \end{tikzcd}
        \end{equation}
        {(where the equalities are given by the observation, based on Remark \ref{F_MODEL_REM}, that the unit maps
          \begin{equation}
        ...
        $(\upgamma^{\**} \iota_{\**} \mathcal{O})[\upgamma^{\**} \iota_{\**} u]$ when $u \colon X \to Y$ is a general cofibration between cofibrant objects in 
        $\mathsf{Sym}^{G}_{\mathcal{F}}(\mathcal{V})$.
        Indeed, setting  $X=\emptyset$ and $\O=\mathbb{F} (\emptyset)$, 
        one has 
        $(\upgamma^{\**} \iota_{\**} \mathcal{O})[\upgamma^{\**} \iota_{\**} u] = 
