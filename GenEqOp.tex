...
determined by either composite of forgetful functors
\[
\begin{tikzcd}
...
\begin{notation}
        ...
        (equivalently, this is the model structure transfered from $\Sym_\F(\V)$).
\end{notation}
...
Now, from the inclusion map $\upgamma: \Sigma_\F \into \Sigma_G$, we have an adjunction
\[
\begin{tikzcd}[column sep =5em]
      \mathsf{Sym}_G(\mathcal{V})
      \ar{r}[swap]{\upgamma^{\**}} 
...
$\mathsf{Sym}_{\mathcal{F}}(\mathcal{V}) = \mathcal{V}^{\Sigma_{\mathcal{F}}^{op}}$.
Now, let $\mathbb F_\F$ denote the endofunctor $\upgamma^{\**} \mathbb F_G \upgamma_!$.
The following is immediate from Proposition \ref{MONADAJ1 PROP}.
\begin{lemma}
      ...
      \end{tikzcd}
      \]
where $\Op_\F(\V)$ is the category $\mathsf{Alg}_{\mathbb F_\F}(\V)$ of \textit{$\F$-genuine equivariant operads}.
\end{lemma}
...
\begin{lemma}
      ...
      \[
      \upgamma^{\**} \upgamma_{\**} \to id \qquad \upgamma^{\**} \mathbb F_G \to  \upgamma^{\**} \mathbb F_G \upgamma_{\**} \upgamma^{\**}
      \]
      are isomorphisms, and thus (by Proposition \ref{MONADAJ PROP}) $\upgamma^{\**} \mathbb F_G \upgamma_{\**}$ is a monad, and we have an induced adjunction
      \[
      \begin{tikzcd}[column sep =5em]
            \mathsf{Op}_G(\mathcal{V})
...
\end{lemma}
\begin{proof}
      This follows identically to the proof of Proposition \ref{MONAD_COMPARISON_PROP}. Indeed, using the definition that $\Omega_\F^0$ is a sieve of $\Omega_G^0$, it is immediate that for each $C \in \Sigma_\F$, one has an equality of rooted undercategories between
...
\begin{proposition}
      ...
      where $\Omega_\F^a[k]$ denotes alternating $\F$-trees with exactly $k$ passive vertices. 
          \end{proposition}
\begin{proof}
      Any span defining the pushout in (\ref{FF_FREE_EXTENSION}) is equivalent to the data of the span in the following pushout diagram in $\Op_G(\V)$.
      \[
      \begin{tikzcd}
            \mathbb F_G \upgamma_! X \arrow[d, "u"'] \arrow[r] & \upgamma_{\**} \P \arrow[d]\\
            \mathbb F_G \upgamma_! Y \arrow[r] & (\upgamma_{\**}\P)[u]
      \end{tikzcd}
      \]
      Since $\upgamma^{\**}$ is a left adjoint into $\Op_\F(\V)$, the filtration and its pushout description from (\ref{FILTRATION_LAN_LEMMA}) induce a filtration and pushout description on the map
      \[
      \P \to \P[u] = \upgamma^{\**}((\upgamma_{\**} \P)[u]).
      \]
      Thus, we see that for any $C \in \Sigma_\F$, $\P_{k-1}(C) \to \P_k(C)$ is given by the pushout over the map (cf. (\ref{FILTINTALT EQ}))
      \[
...
\begin{definition}
      Let $\bar\F \subseteq \F$ be a subfamily of corollas. We say a map $f: \O \to \P$ in $\Op_\F(\V)$ is a 
      \begin{enumerate}
          \item \textit{$\bar\F$-weak equivalence} (resp. \textit{fibration}) iff $f(C): \O(C) \to \P(C)$ is one in $\V^{\mathsf{Aut}(C)}_{\text{gen}}$ for all $\bar\F$-corollas $C \in \Sigma_{\bar \F}$.
...
(where we midly change notation and let $\iota_G$ denote the composite injection $G \times \Sigma \to \Sigma_{G}$)
and recalling that there are natural isomorphisms 
$\iota_G^{\**} \mathbb{F}_{G} \iota_{G,\**}
\simeq  \mathbb{F} \iota_G^{\**} \iota_{G,\**} \simeq \mathbb{F}$, we see that the two solid subdiagrams in 
(\ref{TWOCELLEXT EQ})
