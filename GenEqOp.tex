...
(recording ``reordering the inputs of the operations''),
...
homotopical replacements for the commutative operad
 ...
 ``commutative monoids up to higher homotopy''.
...
usually that of \textit{naive equivalence},
...
In this context, $E_\infty$-operads are equivalent to the commutative operad $\mathsf{Com}$
\index{categories!Com@$\mathsf{Com}$}
and, moreover, 
any cofibrant replacement of $\mathsf{Com}$
is $E_{\infty}$.
These naive equivalences differ from the equivalences in \textit{genuine equivariant homotopy theory},
where for a finite group $G$, a map of $G$-spaces $X \to Y$ is deemed a $G$-equivalence only if
...
First, we note that for any finite group $G$ and a $G$-operad $\O$ (i.e. an operad $\O$ together with a $G$-action commuting with all the structure),
the $n$-th level $\O(n)$ has a $G \times \Sigma_n$-action.
Now, one might guess that a map of $G$-operads
$\O \to \O'$
...
However, the notion of equivalence suggested in \eqref{NAIVEOPEQ EQ} turns out to not
...
On the other hand, $\Gamma \leq G$ if and only if $\Gamma$ is the graph subgroup \eqref{GRAPHSUBIN EQ} associated to a trivial homomorphism $\phi$. 
% As it turns out,
...
Therefore, in order to differentiate such operads, one needs to replace the notion of weak equivalence from \eqref{NAIVEOPEQ EQ} 
...
ring multiplication also induces so called \textit{norm maps}
...
\todo[inline]{these don't actually appear anywhere, correct?}
...
$\mathcal{O}(n)^{\Gamma_X}$.
\todo[inline]{I would say more here, i.e. include mapping sets/spaces. This takes a while to write in \cite{BH15}, and is not illuminating there}
The flaw of the $G$-trivial $E_{\infty}$-operads
described in \eqref{NAIVEGEINFTY EQ} is then that
the algebras they encode
lack all norms maps other than those for $H$-trivial $X$, thus lacking 
...
Noting that the fixed point characterization of $E \mathcal{F}$-spaces defines an object 
...
indexed
on all graph subgroups of $G \times \Sigma_n$ for all $n \geq 0$,
% in the same way as in 
% \eqref{GENEOPEQ EQ},
% \todo[inline]{this is not indexed}
...
\begin{equation}\label{D6SMALLER EQ}
	...
	\end{tikzpicture}%
\end{equation}%
Here, the expanded representation of $T$ is
a forest diagram, with edges labeled to indicate the $G$-action.
...
which is then labeled by said edge orbit %the corresponding  edge set of orbits in the expanded representation
(one may also reverse this process, though we will not need to do so).
We note that orbital representations always look like a (labeled) tree.
...
from the expanded representation of $T$,
as well as ga map
$\Omega(C) \to \Omega(T)$
...
Notice that now the arity of such a node (i.e. the associated ``type of input'')
does not just count incoming edge orbits,
but depends on the labels of both 
incoming and outgoing edge orbits
(in particular, the latter determines the fixed point condition).
...
Slightly modifying their construction by taking 
colors to be $G$-free corollas and
operations to be encoded by $G$-free trees 
one obtains a colored operad $\mathcal{T}_G^{fr}$.


Moreover,
$\mathcal{T}_G^{fr}$ comes with a $G$-action
and there is a map of $G$-colored operads
(where $G$ acts on $\mathcal{T} \times G^{op}$
via the trivial action on $\mathcal{T}$ and the conjugation action on $G^{op}$)
\[
	\mathcal{T} \times G^{op} \to \mathcal{T}_G^{fr}
\]
...
\begin{conjecture}
	...
	$N \colon \mathsf{Op}_G
	\to 
	\mathsf{dSet}_G$
	whose essential image 
	consists of those $X \in \mathsf{dSet}_G$
	such that
	\begin{itemize}
		\item $X$ is pointed, i.e. 
		$X(G/H \cdot \eta) = \**$ for all $H \leq G$;
		\item $X$ is a strict Segal object, i.e.
		the natural maps
