...
(namely Propositions \ref{PLANARPULL EQ} and  \ref{SUBDATAUNDERPLAN PROP})
...
and $H = \langle j \rangle$.
The following illustrates a root pullback along the twist map
$\tau \colon G/H \to G/H$
(or, more precisely, noting that in our model the underlying set 
of $G/H$ is actually $\{1,2\}$,
$\tau$ is the permutation $(12)$).
In the top depiction of $\tau^{\**} T$ we use the edge orbit generators induced from $T$, 
while the bottom depiction 
chooses generators that are minimal with regard to the planar structure, so that in $\tau^{\**}T$ it is
$d = i c$, $e = i b$, $f = i a$.
\begin{equation}
	\begin{tikzpicture}[grow=up,auto,level distance=2.1em,
	every node/.style = {font=\footnotesize,inner sep =2pt},
...
We now turn to discussing the substitution operation.
...
\begin{proof}
	If either of $T_1,T_2$ consists only of the root or a leaf  the result is obvious. Otherwise, one can necessarily choose $e$ to be an inner edge of $T$, in which case all of $T_1,T_2,T$ admit compatible decompositions (\ref{DECOMPPROOF EQ}) and the result follows by induction on $|V(T)|$.
\end{proof}
