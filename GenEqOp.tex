...
Let us now write any input path as 
...
\end{tikzcd}
\]
When $f$ is a quotient map, $\mathsf{lr}_X(f)$ is simply the pullback of $f$. When $f$ is a planar tall map, writing $T=\mathsf{lr}_X(U)$ one has a map of $T$-substitution data
$\{\mathsf{lr}_X(V_{v_{G_e}})\} \to \{V_{v_{G_e}}\}$, which after converted to a tree map yields the desired map 
$\mathsf{lr}_X(f)$.
\end{proof}
