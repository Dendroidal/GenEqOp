...
should consist of a list of spaces, one for each of the graph subgroups in (\ref{GENGEINFTY EQ}), along with 
obvious restriction maps and, more importantly, 
suitable \textit{multiplication maps}.
Correctly encoding such multiplication maps is one of the main challenges of our theory, and again the motivation for these is provided by the work in \cite{BH15}.
Indeed, when analyzing the proofs of the results in 
\cite[\S 4]{BH15}
that establish the closure properties for indexing systems 
...
$f(g_1,\cdots,g_n)$.
The desired multiplications for genuine equivariant operads
are then an abstraction
