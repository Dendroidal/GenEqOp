...
note that the subgroups such that $\Gamma \cap \Sigma_n = \{\**\}$ 
...
For any $\mathsf{Comm}$-algebra, i.e. strictly commutative ring $G$-spectrum, $R$, 
...
will consist of a list of spaces indexed as in (\ref{GENEOPEQ EQ}) along with obvious restriction maps and, more importantly, 
...
Let $\V$ be strongly cellular, and fix a collection ${\mathcal F} = \set{{\mathcal F}_n}$ of sets ${\mathcal F}_n$ of subgroups of $G\times \Sigma_n$, each closed under conjugation. 
...
While we note that the above hold for general sets of subgroups of $G\times \Sigma_n$, when considering our genuine $G$-operads, we must restrict to collections ${\mathcal F} = \set{{\mathcal F}_n}$ of sets of graph subgroups, or equivalently full subcategories $\Sigma_{\mathcal F}$ of $\Sigma_G$. Letting $\Op_{\mathcal F}(\V)$ denote the category of $\mathbb F_G$-algebras in $\Sym_{\mathcal F}(\V)$, we have the following, proved in Section \ref{OP_G_EXISTS_SECTION}.

\begin{customthm}{II}\label{MAINEXIST2 THM}
Let $\V$ be Cartesian strongly cellular, and fix any full subcategory $\Sigma_{\mathcal F} \subseteq \Sigma_G$.
Then the projective semi-model structure on  
$\mathsf{Op}_{\mathcal{F}}(\mathcal{V})$ exists. 
Equivalently, the $\Sigma_{\mathcal F}$-semi-model structure, denoted $\Op_G^{\mathcal F}(\V)$, on $\Op_G(\V)$ exists.

If moreover $\V$ is underlying strongly cellular, these are actual model structures.
\end{customthm}

While these model structures always exist, they are not necessarily well-behaved with respect to cofibrancy unless the collections ${\mathcal F}$ are highly structured.
...
We lastly return to the conjecture of \cite{BH15}, which we resolve in the affirmative using two different methods in Section \ref{NINFTY_SECTION}. In fact, we show the existence of a larger class of operads, realizing any \textit{weak} indexing system (see Definition \ref{INDEXSYS DEF}).
...
There are multiple distinct steps along the path to proving our main result.  Specifically, we need a better understanding of the interplay between the subtle equivariance structures we just discussed, and the combinatorics which underly operads. To that end, we exploit the equivariant generalization $\Omega_G$ of the dendroidal category $\Omega$ discussed in \cite{Pe17}. First, we observe that the free operad $\mathbb F X$ generated by a symmetric sequence $X \in \Sym(\V)$, discussed in \cite{Spitz01, BM03, Rub17} etc., can be repackaged as a left Kan extension out of $\Omega$, given on the left-hand side:
\[
\begin{tikzcd}
...
This can be equivariantly generalized, replacing $\Omega$ and $\Sigma$ with $\Omega_G$ and $\Sigma_G$, as seen on the right-hand-side. This yields our ``genuine $G$-operad'' monad $\mathbb F_G$, acting on the new category of $G$-sequences $Y\in \Sym_G(\V) = \V^{\Sigma_G^{op}}$. Now, having defined our notion of ``coefficient systems'', we begin the process of analyzing this new functor. 
First, we produce multiple categorical manipulations of this functor. We modify both $\Omega_G$ and $\mathbb F_G$ to construct a description of coproducts of $\mathbb F_G$-algebras as a similar looking left Kan extension. Then, as our true goal is to understand free $\mathbb F_G$-extensions, we ``add in relations'' by attaching new maps to this modified $\Omega_G$ by realizing a certain simplicial category of strings of such maps. Finally, using this description, we build a filtration of such free extensions.

Secondly, we consider the homotopical relations of this functor, studying the subtle interactions of different versions of cofibrancy observed when grafting tree components together. This analysis produces our model structures, as well as proves many of their desired properties. 
Lastly, we compare $\mathbb F$ and $\mathbb F_G$, categorically and homotopically, in order to display the Quillen equivalence.

We now give a brief overview of each remaining chapter.
\textbf{Section \ref{PRELIM_SECTION}: Preliminaries.} Here, we identify the major constructions we will be using throughout the paper, as well as establish notion and background material.

\textbf{Section \ref{PLANAR_SECTION}: Planar and tall maps.} Our machery requires our trees to record an additional structure, namely a planarization. In this chapter, we define this for $G$-trees, and explore the notion of ``substitution'', a second operad-like structure on the set of trees.

\textbf{Section \ref{GENUINE_OP_MONAD_SECTION}: The genuine equivariant operad monad.} Defining $\mathbb F_G$ as an endofunctor is fairly straightforward, as is defining composition and unit maps. However, showing that this data combines to form a monad requires a more subtle analysis. In particular, we define a more general monad $N$ on the category of weak spans $\mathsf{WSpan}(\Sigma_G, \V)$ over $\Sigma_G^{op}$ and $\V$, and through the flexibility of this more general category are able to prove the required properties. It is also here that the concept of ``planar strings'' of maps are introduced, which will also be useful in the next chapter

\textbf{Section \ref{FREE_EXTENSIONS_SECTION}: Free extensions.} In this chapter, we develop the technology to produce free $T$-extensions for particular monads $T$, utilizing the realizations of simplicial categories of planar strings.

\textbf{Section \ref{MODEL_STRUCTURES_SECTION}: Model structures.} Using the description of free extensions developed in the previous chapter, we produce a filtration of free extensions, and exploit this to transfer model structures from the categories of sequences $\Sym^G$ and $\Sym_G$ onto our categories of operads $\Op^G$ and $\Op_G$.

\textbf{Section \ref{COFIB SEC}: Cofibrancy and Quillen equivalences.} This chapter contains the bulk of the homotopical analysis, and uncovers the interactions of different notions of cofibrancy with tree grafting. As such, it is here that we show weak indexing systems provide structure so that their associated model structures are well-behaved, in particular providing the desired Quillen equivalences. We end this chapter, and this paper, by providing two proofs of the $N_\infty$ realization conjecture.
...
We end this introduction with a brief discussion on the context of this paper. As stated earlier, the main goal of the authors' current project is to detail the full story of the homotopy theory of equivariant operads. This paper, along with \cite{Pe17}, form the basis of a generalization of the work of Cisinski-Moerdijk-Weiss, describing the homotopy theory of non-equivariant operads, into the $G$-equivariant context. In particular, in sequels, we will extend the definitions of ``dendroidal complete Segal spaces'' and ``Segal pre-operads'', as well as the comparison Diagram $(\filledstar)$ of Quillen equivalences from \cite{CM13b}, reproduced here below, to the $G$-equivariant context. 
\begin{equation}
  \label{CM_EQ}
  \tag{$\filledstar$}
  \begin{tikzcd}
    \mathsf{PreOp} \arrow[d] & \mathsf{sOp} \arrow[l, "N_d"] \arrow[d, "hcN_d"]\\
...
We end by interpreting these different models of $G$-operads, focusing on the right half of \ref{CM_EQ}. We recall that, intuitively, $\infty$-operads can be thought of as operads where composition is ``weakly defined''; similiarly, $G$-coefficient systems as spaces with a ``relaxed'' fixed-point condition. In this fashion, genuine $G$-operads can be thought of as $G$-operads where composition is still rigidly defined, but with relaxed fixed-point conditions. Comparitively, the $G$-$\infty$-operads of \cite{Pe17} have rigid fixed-point conditions but weak composition. The remaining missing link is then the suitable notion of $G$-$\infty$-operad (that is, a suitable model structure) in the true pre-sheaf category $\mathsf{dSet}_G$, yielding an expanded equivariant right half of \ref{CM_EQ}, given by the following.
\[
\begin{tikzcd}
        \Op^G_{\mathcal F}(\V) \arrow[r, "i_*", "\simeq_Q"'] \arrow[d, "hcN^G"', "\simeq_Q ?"] & \Op_G^{\mathcal F}(\V) \arrow[d, "hcN_G", "\simeq_Q ?"']\\
        \mathsf{dSet}^G_{\mathcal F} \arrow[r, "i_*"', "\simeq_Q ?"] & \mathsf{dSet}_G^{\mathcal F}
\end{tikzcd}
\]
We expect that, analogous to the work of Cisinski-Moerdijk, the direct vertical comparisons between $G$-operads and $G$-dendroidal sets will be challenging. 
...
\begin{remark}
	...
	for the subcategories of surjections,
	we define a 
