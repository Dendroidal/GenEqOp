...
Further, we define the \textit{degree} of $T$, denoted $|T|$, to be the sum $|T|_X + |T|_Y$, where $|T|_X$ is defined by
\[
...
\begin{definition}
  ...
  \begin{enumerate}
  \item relabelling all $X$-nodes by $\P$ (yielding a $(\P,Y)$-labeled tree); then
...
\begin{proof}[Proof of Lemma \ref{MINUS_LAN_FINAL_LEMMA}]
  ...
  \begin{equation}
    ...
    \begin{tikzcd}
      ...
      & C \arrow{ur}[swap]{q_S} \arrow{ul}{q_S}
    \end{tikzcd}
  \end{equation}
...
In order to encode the homotopical information inspired by $N_\infty$-operads dicussed in the introduction, we will introduce (semi) model structures on the categories $\V\Op_G$ and $\V\Op^G$ for a wide range of $\V$, which are true model categories in the cases we are most interested in, in particular for $\V = \sSet$.  These model structures will be determined by a choice of weak indexing system, a generalization of the notion defined in \ref{BH16}. 


\subsection{Weak Indexing Systems}

We recall certain constructions found in \ref{Per17}, which relate graph subgroups, finite $H$-sets, and systems of categories.
\begin{definition}
...
\begin{lemma}
  A weak indexing system $\F$ is \textit{strong} (in the sense of \ref{BH16}) if and only if all trivial $H$-sets are $\F$-admissible, for all $H\leq G$.
\end{lemma}
\todo[inline]{come back}

We will define $\F$-model structures on $\V\Op_G$ for any weak indexing system $\F$; the $\F$-model structure on $\V\Sym_G$ will exist for any $\F$, but transfering requires the additional closure properties.

\subsection{$\F$-Model Structures}

Fix a $G$-vertex family $\F = \set{\F_n}$. More, we fix the following about our base category $\V$:
\begin{definition}
  We say $\V$ satisfies \textsc{Assumption 1} (for $\F$) if the following hold:
  \begin{enumerate}
  \item $\V$ is a cofibrantly-generated closed symmetric monoidal model category, and
  \item for each $n$, $\V^{G\times \Sigma_n}$ is $\F_n$-cellular.
  \end{enumerate}
\end{definition}


\subsubsection{Genuine Operads}
\begin{definition}
  ...
  \item \textit{level $\F$-cofibration} if $f(C)$ is a cofibration in $\V$ for all $C\in \Sigma_\F$.
  \end{enumerate}
\end{definition}
In particular, $\P\in \V\Op_G$ is called \textit{$\F$-cofibrant} if $\varnothing \to \P$ is an $\F$-cofibration.
\begin{definition}
...
\[
\begin{tikzcd}
  \V\Op_G \arrow[r, shift right, "\mathsf{fgt}"] & \V^{\Sigma_G^{op}} \arrow[l, shift right, "\mathbb F_G"] \arrow[r, shift right] & \prod_{\mathrm{Ob}(\Sigma_G)}\V \arrow[l, shift right] \arrow[r, shift right] & \prod_{\mathrm{Ob}(\Sigma_\F}\V \arrow[l, shift right]
\end{tikzcd}
\]
\end{definition}

Using general arguments of \ref{Hir03}, this structure would be cofibrantly generated, with generating arrows
\begin{align*}
...
\end{theorem}
\begin{proof}
  We use the filtration from the previous section. In particular, it suffices to show that
\[
\Lan_{\Omega_{G,e}[k,-]^{op}}N^e \to \Lan_{\Omega_{G,e}[k]^{op}}N^e
\]
is an $\F$-cofibration in $\V\Sym_G$, trivial if $u$ is. If we consider the string of adjunctions below
\[
\begin{tikzcd}
  \V^{\Omega_{G,e}[k,-]^{op}}_\F \arrow[r, shift left, "{\Lan = p[k]_!}"] & \V^{\Omega_{G,e}[k]^{op}}_\F \arrow[l, shift left, "{\mathsf{fgt} = p[k]^*}"] \arrow[r, shift left, "\Lan = \mathsf{lr}_!"] & \V^{\Sigma_G^{op}}_\F \arrow[l, shift left, "\mathsf{fgt} = \mathsf{lr}^*"]
\end{tikzcd}
\]
where each category is equipped with the $\F$-projective model structure, we can repackage this as the following two claims:
\begin{enumerate}
\item The left Kan extension functor $\mathsf{lr}_!$ is left Quillen if $\F$ is a weak indexing system.
\item The counit $\epsilon_k: \Lan_{p[k]}N^ep[k] \to N^e$ is a projective cofibration in $\V_\F^{\Omega_{G,e}[k]^{op}}$,
\end{enumerate}
where here we have denoted $p[k]: \Omega_{G,e}[k,-] \into \Omega_{G,e}[k]$. Claim (1) is a direct consequence of Lemam \ref{WEAK_INDEXING_VALENCE_LEMMA}, and Claim (2) is the content of Proposition BLANK. 
\end{proof}
