...
$\lambda_{l,a} = \{-\infty\}$,
$\lambda_{l,i} = \langle l \rangle - \{-\infty\}$,
(\ref{LANLEVELFOR EQ})
...
\end{tikzpicture}
\end{equation}
The term ``alternating'' comes from the fact that no adjacent nodes have the same color. Note, however, that there is an additional restriction: the ``outer'' vertices, i.e. those immediately below a leaf or the one immediately above the root, are necessarily black/active
(note, however, that this does \textit{not} apply to stumps).
