...
We recall that a map of $G$-trees is called 
...
\begin{definition}\label{SUBSTITUTIONDATUMG DEF}
	...
	$\left(U_{v_{G e}}\right)_{V_G(T)}$ together with rooted (planar) tall maps 
	$T_{v_{Ge}} \to U_{v_{G e}}$.
	
	Further, a map of rooted (resp. planar) $T$-substitution data 
	$\left(U_{v_{G e}}\right) \to \left(V_{v_{G e}}\right)$ is a tuple of rooted (resp. planar) tall maps 
	$\left(U_{v_{G e}} \to V_{v_{G e}}\right)$.
	
	We denote the category of rooted (resp. planar) $T$-substitution data 
	by $\mathsf{Sub}_{\mathsf{r}}(T)$
...
\begin{remark}\label{SUBSDATUMCONV REM}
	...
	so that writing $T = (T_x)_{x \in X}$ as usual
	one obtains (non-equivariant) $T_x$-substitution data 
	$U_{x,(\minus)}$ for each $T_x$.
	We again write
	$U_{x,(\minus)} \colon \mathsf{Sc}(T_x) \to \Omega$
	and note that these are compatible with $G$-action in the sense that the obvious diagram
...
\\
	\left(U_{v_{G e}}\right) \ar[r,mapsto] & 
	\left(T \to \colim_{\mathsf{Sc}(T)} U_{(\minus)}\right) &
	\left(U_{v_{G e}}\right) \ar[r,mapsto] & 
	\left(T \to \colim_{\mathsf{Sc}(T)} U_{(\minus)}\right)
\end{tikzcd}
...
\begin{remark} 
In the planar case the maps 
$U_{v_{G e}} \to U = \colim_{\mathsf{Sc}(T)}U_{(\minus)}$
are themselves planar maps of $G$-trees.
Indeed, writing
$U_{v_{G e}} = (U_{f^{\uparrow} \leq f})_{f \in v_{Ge}}$
each of the individual maps
$U_{f^{\uparrow} \leq f} \to U$ 
is planar by Proposition \ref{SUBDATAUNDERPLAN PROP}, 
and the order of the root tuple,
which we can identify with $Ge$,
is preserved since $T \to U$ is a planar map of $G$-trees.

However, the maps $U_{v_{G e}} \to U$ need not be 
planar in the rooted case. 
Indeed, while each map
$U_{f^{\uparrow} \leq f} \to U$ \textit{is} planar by  Corollary \ref{SUBDATAUNDERPLAN COR},
the order of the roots $Ge$ needs not be preserved.
In fact, requiring such maps to be planar would overdetermine the planar structure on $U$,
which is why in the equivariant case
substitution data are required to be rooted.
\end{remark}
...
\begin{remark}\label{PULLCOMP REM}
	...
	in $\mathsf{O}_G$. The definition of pullback then allows us to uniquely fill any diagram
\[
\begin{tikzcd}[row sep =10pt]
	(\psi^{\**} T)_{v_{Ge}} \ar{d} \ar[dashed]{r} &
	\bar{\psi}_{G e}^{\**}U_{v_{G \bar{\psi}(e)}} \ar{d}
\\
...
\begin{tikzcd}[row sep =12pt,column sep= 14pt]
	...
	\mathsf{Sub}_{\mathsf{r}}(\psi^{\**} T) \ar[r,shift left=2pt] &
...
\\
	...
	\mathsf{Sub}_{\mathsf{r}}(T) \ar[r,shift left=2pt] \ar{u}{(\bar{\psi}_{Ge}^{\**})} &
...
though we caution that from this perspective
...
For any $n\geq -1$ the commutative diagram
	\begin{equation}\label{PTPULL EQ}
	\begin{tikzcd}
		\Omega_{G,n+1} \ar{r}{V_G} 
		\ar{d}[swap]{d_{1,\cdots,n+1}} & \Fin_s \wr \Omega_{G,n} 
...
\begin{proof}
Let us write 
$P = \Omega_{G,0} \times_{\Fin_s \wr \Sigma_G} \Fin_s \wr \Omega_{G,n}$ for the pullback,
so that our goal is to show that the canonical map
$\Omega_{G,n+1} \to P$ is an isomorphism. 

{\color{red} HERE}

It is straightforward to show that Lemma \ref{VGPULL LEM}
implies that $P$ is a split fibration over 
$\mathsf{O}_G$, with $P \to \Omega_{G,0}$
a map of split fibrations. 
Indeed, pullback arrows in $P$ are defined in the obvious way as compatible pairs
$(f,g)$ with $f$ a pullback in 
$\Omega_{G,0}$ over $\mathsf{O}_G$
and $g$ a pullback in 
$\Fin_s \wr \Omega_{G,n}$ over 
$\Fin_s \wr \mathsf{O}_G$.

Combining the alternative description of $V_G$ in 
Notation \ref{VGDEF NOT} with (\ref{SUBDATAUNDERPLANG2 EQ}) 
then shows that 
the map $\Omega_{G,n+1} \to P$ preserves pullbacks,
and one thus needs only show that this is an isomorphism
on each of the fibers over $\mathsf{O}_G$.
