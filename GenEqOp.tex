...
More generally, for $\mathcal{F} = \{\mathcal{F}_n\}_{n \geq 0}$ with $\mathcal{F}_n$ an arbitrary collection of subgroups of $G \times \Sigma_n$ there exists a model structure on 
$\mathsf{Op}^G(\mathcal{V})$,
which we denote
...
$(\mathcal{V},\otimes)$:
\begin{inparaenum}
\item[(i)] is a strongly cofibrantly generated model category;
\item [(ii)] is a closed monoidal model category;
...
is proven in {\color{red} \S \ref{G_OP_EXISTS_SECTION},
where the additional conditions are also discussed.
}

Our next result concerns the model structure
on the category 
$\mathsf{Op}_G (\mathcal{V})$ of genuine equivariant operads
introduced in this paper, the construction of which we now briefly outline.
...
$G$-corollas, introduced in \S \ref{LRVERT SEC},
...
and partial symmetric sequences
...
More generally, for $\mathcal{F}$ a weak indexing system, a projective model structure on 
...
\item[(iii)] has cellular fixed points;
\item[(iv)] has cofibrant symmetric pushout powers;
\item[(v)] $(\mathcal{V},\otimes)$ has diagonals.
\end{inparaenum}
\end{customthm}

Theorem \ref{MAINEXIST2 THM} is proven in 
{\color{red} \S \ref{OP_G_EXISTS_SECTION}}.
...
\item[(iv)] has cofibrant symmetric pushout powers;
\item[(v)] $(\mathcal{V},\otimes)$ has diagonals;
\item[(vi)] $(\mathcal{V},\otimes)$ has cartesian fixed points.
\end{inparaenum}
\end{customthm}

Theorem \ref{MAINQUILLENEQUIV THM}
is proven in 
{\color{red} \S \ref{MAINTHM_PROOF_SECTION}}.

Lastly, our techniques also confirm 
the main conjecture of \cite{BH15},
as we discuss in \S \ref{NINFTY_SECTION}.
