...
%Letting $\Op_{\mathcal F}(\V)$ denote the category of $\mathbb F_G$-algebras in $\Sym_{\mathcal F}(\V)$, we have the following, proved in Section \ref{OP_G_EXISTS_SECTION}.
\todo[inline]{come back: $\Op_{\mathcal F}$ vs $\Op_G^{\mathcal F}$}

\begin{customthm}{II}\label{MAINEXIST2 THM}
Let $\V$ be a strongly cellular model category with projections.
...
If additionally $\V$ is underlying strongly cellular, all of these are actual model structures.
\end{customthm}
...
Let $\mathcal{V}$ be a underlying strongly cellular model category with projections.
...
\todo[inline]{maybe mention that ``cat with projections'' is the same as semi-cartesian}
...
\begin{definition}
  ...
  In particular, this will hold if $\V$ has \textit{cellular fixed points} (see Definition \ref{CELL DEF}). 
\end{definition}
...
\begin{definition}
 ...
 If each $\F_d$ is the set of all subgroups of $\mathrm{Aut}(d)$, we refer to the above as the $\bar{\mathcal D}$-projective \textit{genuine} model structure, and just write $\V^{\mathcal D^{op}}_{\bar{\mathcal D}}$. 

  If $\bar{\mathcal D} = \mathcal D$, we refer to the above as simply the \textit{projective $\F$-model structure}, and just write $\V^{\mathcal D^{op}}_{\mathcal F}$.
\end{definition}
...
\end{proposition}
\begin{proof}
  It is easy to check, using the fact that pushouts are levelwise, that the machinery of Kan \cite[11.3.2]{Hi03} applies, and so the model structure $\V^{\mathcal D^{op}}_{\bar{\mathcal D}, \F}$ exists, with generating cofibrations similarly of the form
$\mathcal D(-,d)/H \cdot i$.

Moreover, is it similarly easy to show, using that the image of $\varnothing$ is again $\varnothing$, that cofibrant objects in $\V^{\mathcal D}_{\bar{\mathcal D}, \F}$ are precisely the essential image of cofibrant objects in $\V^{\bar{\mathcal D}^{op}}_\F$, and hence, as $i_!i^*$ is then the identity on cofibrant objects, the verification of the Quillen equivalence is immediate.
\end{proof}
...
\begin{example}
  ...
  fix any family $\mathcal F$ of subgroups of $G$.
...
\begin{remark}
  ...
  $\V^{\mathcal D}_{{\bar{\mathcal D}}, \F}$, 
  $f(d)$ is a cofibration in $\V^{\mathrm{Aut}(d)}_{\F_d}$ for all $d\in {\bar{\mathcal D}^{op}}$. 
  Indeed, the map
  \[
...
In what follows, we will need our base category $\V$ to be well-behaved, in particular \textit{strongly cellular} or \textit{underlying strongly cellular} (see Definitions \ref{STRONGLY_CELLULAR} and \ref{UNDERLYING_STRONGLY_CELLULAR}).




\todo[inline]{come back: move this}
{\color{blue}
We will require our base category $\V$ to be sufficiently nice.
\begin{definition}
...
\begin{theorem}\label{OP_G_SEMI_MODEL_THM} 
  For $\V$ strongly cellular with projections, the projective semi model structure on $\Op_G(\V)$ exists.
\end{theorem} 
...
\begin{corollary} 
  ...
  If $\V$ is strongly cellular with projections, for any full subcategory $\Sigma_\F \subseteq \Sigma_G$, the $\F$-semi-model structure on $\Op_G(\V)$ exists. \qed
\end{corollary} 
...
\begin{theorem} 
  ...
  For $\V$ underlying strongly cellular with projections, % and moreover
  % \begin{itemize}
...
\begin{theorem}
  \label{G_OP_SEMI_MODEL_THM}
    For $\V$ strongly cellular with projections, the genuine semi model structure on $\Op^G(\V)$ exists.
\end{theorem}
...
\begin{corollary}\label{CELLULAR_LEVEL_COFIB_PROP2} 
  For $\V$ strongly cellular with projections, and for any free $\mathbb F$-extension  
  \[ 
  \begin{tikzcd} 
...
\begin{remark}
  The ``with projections'' condition can be dropped from the above theorem. Indeed, when restricting the machinery of Section \ref{GENUINE_OP_MONAD_SECTION} to $G = *$ and $\V = \V^G$, Remark \ref{NEED_WREATH_REMARK} no longer applies, and hence we may reformulate the monad on spans from Definition \ref{WSPAN_MONAD_DEFINITION} by replacing $\Fin$ with $\Fin_{\mathsf{Iso}}$ and using the natural map $\mathsf F_{\mathsf{Iso}} \wr \V^{op} \xrightarrow{\otimes} \V^{op}$ for \textit{any} symmetric monoidal product $\otimes$ on $\V$.
\end{remark}
...
\begin{theorem}
  ...
  If $\V$ is underlying strongly cellular,% , satisfies the conditions in Theorem \ref{GENUINE_OP_Q_MODEL_THM}, 
...
\todo[inline]{come back: define $\mathbb F_{\mathcal F}$}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We note that the machinery defining the monad $\mathbb F_G$ in Section \ref{GENUINE_OP_MONAD_SECTION} and filtration in Section \ref{FILTRATION_SECTION} is robust enough that replacing the key pieces 
\[
\Omega_G^0, \qquad \mathsf{lr}: \Omega_{G,0} \to \Sigma_G
\]
with
\[
\Omega_\F^0, \qquad \mathsf{lr}: \Omega_\F \to \Sigma_\F
\]
...
\begin{corollary} 
        \label{F_STRUCTURES_COR}
  Let $\F$ be a weak indexing system. 
Then we have the follow results, which agree with those from Section \ref{GENUINE_OP_MONAD_SECTION} in the case $\Sigma_\F = \Sigma_G$. 
...
\begin{enumerate}
  \item The endofunctor
    ...
    \]
    is a monad on $\Sym_\F(\V)$, where $\mathsf{Lan}$ and $\iota$ are adjoint maps 
(cf. Definition \ref{THEMONAD DEF} and (\ref{LANIOTAADJ EQ})).
    ...
    \colon \iota
    \]
  \item There exists an adjunction (cf. (\ref{TWOADJOINTS EQ}))
    \[
    \begin{tikzcd}[column sep =5em]
...
\item There are isomorphisms of monads $\mathbb F_{\mathcal F} \to \upgamma^* \mathbb F_G \upgamma_! \to \upgamma^* \mathbb F_G \upgamma_*$ (cf. Proposition \ref{MONAD_COMPARISON_PROP})

\item The above adjunction extends to algebras over the respective monads (cf. (\ref{TWOADJOINTSOP EQ}))
  \[
  \begin{tikzcd}[column sep =5em]
...
\item Any free $\upgamma^*\mathbb F_G \upgamma_*$-extension $\P \to \P[u]$ has a filtration
\[
\P = \P_0 \to \P_1 \to \P_2 \to \cdots \to \colim_k \P_k = \P[u]
\]
such that, for any $\F$-corolla $C\in \Sigma_\F$, the map $\P_{k-1}(C) \to \P_{k}(C)$ is given a replica of (\ref{FILTRATION_LAN_LEVEL}) replacing all instances of $\Omega_G$ (and its derivatives) with $\Omega_\F$ (and its derivatives).
\end{enumerate} \qed
\end{corollary}
...
\begin{remark}
  We note that the monad $\upgamma^* \mathbb F_G \upgamma_*$ exists for any family of corollas $\F$. However, the model structure on $\Op_\F$ is only well-behaved in the case where $\F$ is in fact a weak indexing system. 
\end{remark}
...
% \end{tikzcd}
% \]
% \todo[inline]{come back: $\Sym_{(-)}^\F$ all Quillen equivalent? Go back to Section 6.2}
% \end{definition}

\begin{corollary}
  Suppose $\V$ is a strongly cellular model category with projections, 
  and $\F$ a weak indexing system. 
  Then the projective semi model structure on $\Op_\F(\V)$ exists. 

  If additionally $\V$ is underlying strongly cellular, this is an actual model structure.
%   \begin{enumerate}
...
% \todo[inline]{come back: this is wrong. If $\bar \F = \F$, this adjunction goes the wrong way.}
%   \end{enumerate}
\end{corollary}
\begin{proof}
 This again follows from Corollary \ref{CELLULAR_LEVEL_COFIB_PROP}. 
 Indeed, we claim $j_*$ sends level genuine cofibrant objects (resp. level genuine cofibrations between cofibrant objects) in $\Sym_\F(\V)$ to those in $\Sym_G(\V)$. 
 It suffices to check that for any genuine cofibrant $G$-object $X\in \V^G$, $X^H$ is genuine cofibrant in $\V^{W_G(H)}$, which follows from Proposition \ref{COFESSIM PROP} and the square of left Quillen identity functors below.
\[
\begin{tikzcd}
        \V^{O_G^{op}} \arrow[r] \arrow[d] & \V^{O_G^{op}}_{\text{gen}} \arrow[d] \\
        \V^{O_G^{op}}_{\text{inj}} \arrow[r] & \V^{O_G^{op}}_{\text{inj, gen}}
\end{tikzcd}
\]
Now, the result follows as $\upgamma^*$ reflects colimits and is a left Quillen functor
\[
\Sym_G(\V) \to \Sym_\F(\V).
\]

The additional claim holds as before.
\todo[inline]{come back: some of this would make sense using the first paragraph of the proof of \ref{MAINLEM LEM}, so it may make sense to move this directly before it.}
\end{proof}
\todo[inline]{come back: end}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Noting that $\gamma \iota = \iota$
(where we abuse notation and let $\iota$ denote any injection $G \times \Sigma \to \Sigma_{(-)}$)
...
\upgamma^* \mathbb{F}_G \iota_{\**} Z \simeq 
\upgamma^* \iota_{\**} \iota^{\**} \mathbb{F}_G \iota_{\**} Z \simeq
\upgamma^* \iota_{\**} \mathbb{F} Z \simeq
\iota_{\**} \mathbb F Z
$ for $Z=X,Y$
