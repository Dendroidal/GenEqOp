...
$H \to \Sigma_n$ for varying subgroups $H \leq G$,
...
but which is equipped with a ``mixed'' $G$-action that
acts diagonally on $R$ and permutes the indices via the action on $X$.
...
\begin{proof}
      The double coset formula gives a $\phi(G)$-orbit decomposition of
      $\mathsf{res}^{\bar G}_{\phi(G)}(\bar G/H)$ as
...
\begin{remark}
      ...
      is sufficient for the construction of a variety of \textit{projective} model structures (resp. semi-model categories)
      on categories of algebraic objects enriched in $\sSet$ (resp. in more general monoidal model categories),
      including operads and commutative monoids \cite[Prop. 4.5.4.6]{Lur17} (resp. \cite[Prop. 6.2.2, Thm. 6.2.3]{WY18}).
...
% partial
homomorphism
$H \to \Sigma_n$, for a subgroup $H \leq G$,
...
$H_i \to \Sigma_n$,
defining the action of the stabilizer $H_i \leq G$ of $i$ on $C_i$,
is in $\mathcal F_n$,
