...
While this paper focuses soley on \textit{single coloured} operads, we expect the theory to extend to the coloured setting. 
...
Moreover, sequels will extend the definitions of ``dendroidal complete Segal spaces'' and ``Segal pre-operads'', as well as the comparison Diagram $(\filledstar)$ of Quillen equivalences from \cite{CM13b} reproduced below, to the $G$-equivariant context. 
\begin{equation}
  \label{CM_EQ}
  \tag{$\filledstar$}
  \begin{tikzcd}
...
{\color{red} Maybe mention that ``cat with projections'' is the same as semi-cartesian}
...
\begin{equation}
	...
	\begin{scope}[yshift=4em,xshift=-5em]
		\node at  (11,0) {}
			child{node [dummy] {}
				child[sibling distance = 4.25em]{node [dummy] {}
					child[sibling distance = 1em]{
		...
		\node at  (8,0) {}
			child{node [dummy] {}
				...
				edge from parent node {$i b$}}
			edge from parent node [swap] {$i c$}};
		\draw[decorate,decoration={brace,amplitude=2.5pt}] (11.1,0) -- (7.9,0) node[midway]{$\tau^{\**}T$};
	\end{scope}
	\begin{scope}[yshift=-4em,xshift=-5em]
		\node at  (8,0) {}
			child{node [dummy] {}
...
\subsection{Leaf-root and vertex functors}\label{LRVERT SEC}
...
Combining the existence of $\F$-model structures and the above cofibrancy result, we give our first proof of Conjecture \ref{NINFTY_REAL_COR_MAIN} from \cite{BH15}.
