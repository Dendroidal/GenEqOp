...
\usepackage{imakeidx}
\makeindex[columns=2, title=Glossary of Notation, intoc]
...
\index{categories!operadsi@$\sOp = \Op(\sSet)$}
...
semi-model structures\footnote{
  We note that a Quillen adjunction between semi-model categories must defined by the usual condition on the right adjoint.
  This implies, but is not equivalent to, an appropriate condition on the left adjoint, namely that it
  preserves cofibrations and preserves trivial cofibrations with cofibrant source.
  The definition of a Quillen equivalences remains unchanged.
  A relevant discussion can be found in \cite[\S 12.1.8]{Fre09}.}
...
\Sigma_{\lambda_k}$
\index{subgroups!SigmaLambda@$\Sigma_{\lambda}$}
...
\index{categories!Gtrees1@$\Omega_G$}
...
\index{Gtreesleafroot0@$\mathsf{r}$}
...
\begin{definition}\label{PLANARIZE DEF}
      ...
      \index{Gtreesorderplanar@$\leq_p$}
      \index{Gtreesorderdescendancy@$\leq_d$}
	\begin{itemize}
...
$e^{\uparrow} \leq e$\index{vertices/outer subtrees!Broadvert@$e^{\uparrow} \leq e$}.
...
$\underline{e} \leq e$\index{vertices/outer subtrees!Broadvert@$\underline{e} \leq e$} holds 
...
\begin{notation}\label{INPUTPATH NOT}
	...
	\[ I(e) =\{f \in T \colon e \leq_d f \} \]
        ...
        \index{vertices/outer subtrees!inputpath@$I(e)$}
\end{notation}
...
\begin{proposition}\label{PLANARIZATIONCHAR PROP}
	...
	with $V(T)$ its set of vertices.
        \index{vertices/outer subtrees!vertices@$V(T)$}
...
\end{equation}
\index{vertices/outer subtrees!planarouterface@$T_{\underline{e} \leq e}$}
...
\begin{notation}\label{STICKTRE NOT}
	...
	consisting of a single edge and no vertices.
        \index{Gtreesstick@$\eta \in \Omega$}
\end{notation}
...
\begin{notation}\label{UNIQCOR NOT}
	...
	\textit{leaf-root} of $T$ (this name is motivated by the equivariant analogue, discussed in \S \ref{LRVERT SEC}).
        \index{Gtreesleafroot1@$\mathsf{lr}$}
...
\begin{definition}
	...
	The \textit{Segal core poset $\mathsf{Sc}(T)$} is the poset with objects the single edge subtrees $\eta_e$ and vertex subtrees $T_{e^{\uparrow} \leq e}$, ordered by inclusion.
        \index{GtreesSegalCore@$\mathsf{Sc}(T)$}
\end{definition}
...
\begin{notation}\label{UEUPE NOT}
	...
	as simply
	$U_{e^{\uparrow} \leq e}$\index{vertices/outer subtrees!UEUPE@$U_{e^{\uparrow} \leq e}$}.
\end{notation}
...
\index{categories!Gtrees0stringsn@$\Omega_G^n$}
\end{definition}
...
\begin{definition}\label{GSYMCAT DEF}
	...
	(cf. Notation \ref{UNIQCOR NOT}).
        \index{categories!SigmaG@$\Sigma_G$}
\end{definition}
...
\begin{remark}
	...
	where $\Omega^{\mathsf{t}}_G$ is the category of tall maps, defined exactly as in Definition \ref{QUOT DEF}, but not to a functor defined on all arrows of $\Omega_G$.
        \index{categories!Gtrees0tall@$\Omega_G^{\mathsf t}$}
...
\index{vertices/outer subtrees!vertices@$V(T)$}
\index{vertices/outer subtrees!verticesG@$V_G(T) = V(T)/G$}
...
We will prefer to write $G$-vertices as $v_{Ge}$\index{vertices/outer subtrees!Gvertex@$v_{Ge}$}, 
and write
...
\begin{equation}
	  ...
	  \end{tikzcd}	
\end{equation}
\index{vertices/outer subtrees!vertices@$\boldsymbol{V}_G$}
...
The $G$-vertex functor \eqref{GVERT_EQ}
% \[
% 	\boldsymbol{V}_G \colon \Omega_{G}^0 \longto \Fin_s \wr \Sigma_G
% \]
...
\begin{notation}\label{SIMPOPERATORS NOT}
	...
	with faces given by composition and degeneracies by inserting identities.
        \index{Gtreessimpdegen@$s_i$}
        \index{Gtreessimpface@$d_i$}
...
\begin{notation}\label{ROOTUNDER NOT}
	...
	as $C \downarrow_{\mathsf{r}} \Omega^n_G$,
        \index{categories!undercat@$C \downarrow_{\mathsf r} \Omega^n_G$}
...
\]
\index{vertices/outer subtrees!vertices@$\boldsymbol{V}_G^k$}  
