...
\begin{corollary}
      $\mathcal P \coprod\limits_{\mathbb F X} \mathbb F Y \simeq \Lan_{(\Omega_G^e \to \Sigma_G)^{op}}\tilde N^{(\mathcal P, X,Y)}$. \qed
\end{corollary}
...
Recall that for any group $\Sigma$, the genuine model structure (if it exists)
on $\mathcal{V}^{\Sigma}$,
...
\[
\left( \varnothig \to \bigotimes\limits_{v \in V_{G}^{ac}(T)}\P(T_v) \right)
=
