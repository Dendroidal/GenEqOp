...
$\alpha \colon \underline{n} \to \**$ is the unique map to the simpleton $\** = \underline{1}$. In this case we can instead write $\alpha_{!} = \coprod$, $\alpha^{\**}=\Delta$,
...
\[
\coprod^a_J TT A_j \amalg^a \coprod^a_{\underline{n}-J} T A_j
	\to
\coprod^a_J T A_j \amalg^a \coprod^a_{\underline{n}-J} T A_j.
\] 
\end{remark}


\subsection{Mixed strings}
