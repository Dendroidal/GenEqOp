...
\begin{remark}\label{FILTCOMP REM}
	...
	The frameworks in \cite{WY18} and \cite{BB17}
	are general enough to produce such filtrations
	in the usual category $\mathsf{Op}(\V)$
	of operads,
	so one might hope to apply those frameworks here.
	However, there are two key issues:
	by Remark \ref{NEED_WREATH_REMARK},
	the definition of genuine operads $\mathsf{Op}_G(\V)$
	requires $\V$ to have diagonal maps,
	which are not used in either of \cite{WY18},\cite{BB17};
	...
	,
	so one might hope that 
	
	
	which builds filtrations
	for algebras over colored operads,
	and \cite[\S 7]{BB17},
