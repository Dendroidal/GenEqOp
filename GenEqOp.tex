...
More generally, for any sieve subcategory $\Sigma_{\mathcal F} \subseteq \Sigma_G$, 
there exists a $\Sigma_{\mathcal F}$-projective semi model structure on $\Op_G(\V)$, denoted $\Op_{G / {\mathcal F}}(\V)$,
determined by the forgetful functor 
\[
\Op_{G / \mathcal F}(\V) \xrightarrow{\mathmakebox[5em]{\mathsf{fgt}}} \Sym_{G / \mathcal F}(\V).
\]
...
\begin{customthm}{III}\label{MAINEXIST3 THM}
        For any weak indexing system $\mathcal F$,
...
\begin{customthm}{IV}\label{MAINQUILLENEQUIV THM}
...
More generally, for $\mathcal{F}$ a (weak) indexing system, both analogue adjunctions
\begin{equation}
      \begin{tikzcd}[column sep =5em]
            \mathsf{Op}_{G / \mathcal F}(\V) 
            \arrow[r, shift right=1.5, "\upgamma^*"'] 
            &
            \mathsf{Op}_{\mathcal F}(\mathcal{V}) 
            \ar[shift left=1.5]{r}{\iota^{\**}} 
            \arrow[l, shift right=1.5, "\upgamma_!"']
            &
            \mathsf{Op}^G_{\mathcal{F}}(\mathcal{V})
            \ar[shift left=1.5]{l}{\iota_{\**}} .
      \end{tikzcd}
...
\begin{notation}
        ...
        \footnote{In Remark \ref{FAMILY_COROLLAS_REMARK}, we show this is equivalent to a collection $\F = \set{\F_n}$ of families $\F_n$ of graph subgroups of $G\times \Sigma_n$.},
...
% \[
% \mathbb F_G(\Sigma_G(-,C)/H \cdot i) \qquad C\in \Sigma_\F,\ H\leq \mathsf{Aut}(C),\ i \mbox{a generating (trivial) cofibration in $\V$}.
% \]
% % \begin{align*} 
% %         I_\F &= \sets{\mathbb F_G(\Sigma_G(-,C)/H \cdot i)}{C \in \Sigma_\F,\ H\leq \mathsf{Aut}(C),\ i\in I}\\ 
% %         J_\F &= \sets{\mathbb F_G(\Sigma_G(-,C)/H \cdot j)}{C \in \Sigma_\F,\ H\leq \mathsf{Aut}(C),\ j\in J}, 
% % \end{align*} 
...
Extending (\ref{SYM_F_ADJ_EQ1}), we have a pair of adjunctions
\[
\begin{tikzcd}[column sep =5em]
        \mathsf{Sym}_G(\mathcal{V})
        \ar{r}[swap]{\upgamma^{\**}} 
...
\end{tikzcd}
\]
% where $\Sym_\F(\V)$ denotes the category of \textit{$\F$-symmetric sequences}
% $\mathsf{Sym}_{\mathcal{F}}(\mathcal{V}) = \mathcal{V}^{\Sigma_{\mathcal{F}}^{op}}$.
\begin{definition}
        Let $\mathbb F_\F$ denote the endofunctor $\upgamma^{\**} \mathbb F_G \upgamma_!$ on $\Sym_\F(\V)$.
\end{definition}

The following is immediate from Proposition \ref{MONADADJ1 PROP}.
\begin{lemma}
        For any family of corollas $\F$, the endofunctor $\mathbb F_\F$ is in fact a monad, and the above adjunction lifts to an adjunction on algebras
        \[
        \begin{tikzcd}[column sep =5em]
                \mathsf{Op}_G(\mathcal{V})
                \arrow[r, shift right, "\upgamma^{\**}"'] 
        ...
        \end{tikzcd}
        \]
        where $\Op_\F(\V)$ is the category $\mathsf{Alg}_{\mathbb F_\F}(\V)$ of \textit{$\F$-genuine equivariant operads}. \qed
\end{lemma}

In order to homotopically analyze this modified monad, we would like to mimic the filtration (\ref{FILTRATION_LAN_LEVEL}) but for free $\mathbb F_\F$-extensions. Here, we will need to use the new closure condition on weak indexing systems.

We first analyze the compositions of $\mathbb F_G$ with the $\upgamma$ derivatives.
\begin{lemma}
        ...
        \item[(i)]
                ...
                is a monad, and we have an induced adjunction
                \[
                \begin{tikzcd}[column sep =5em]
                        \mathsf{Op}_G(\mathcal{V})
                        \arrow[r, shift left, "\upgamma^{\**}"] 
                        &
                        \mathsf{Op}_\F(\mathcal{V}) 
                        \arrow[l, shift left, "\upgamma_{\**}"];
                \end{tikzcd}
                \]
        \item[(ii)] the map 
                $\upgamma^{\**} \mathbb{F}_G \upgamma_{!}
...
\begin{proof}
        ...
        Thus for $X\in \Sym_G(\V)$, $\upgamma^{\**} \mathbb F_G X$ is computed by the Kan extension of the following diagram,
        \[
        \begin{tikzcd}
        ...
        The remaining results follow through similarly analogous proofs.
\end{proof}

\begin{proposition}
        \label{FF_LEVEL_FILTRATION}
        For any $\mathbb F_\F$-free extension
        \begin{equation}
...
\begin{proof}
        ...
        \[
        \begin{tikzcd}
                \mathbb F_G \upgamma_! X \arrow[d, "u"'] \arrow[r] & \upgamma_{\**} \P \arrow[d, dashed]\\
                \mathbb F_G \upgamma_! Y \arrow[r, dashed] & (\upgamma_{\**}\P)[u]
        \end{tikzcd}
        \]
...
\begin{definition}
        ...
        \begin{tikzcd}
                ...
                \\
                \Sym_{\F/\bar\F}(\V) 
                \arrow[u, shift right, "\mathbb F_\F"'] 
                \arrow[r, shift right, "\nu^{\**}"'] 
                & 
                \Sym_{\bar \F}(\V) 
                \arrow[l, shift right, "\nu_!"']
                \arrow[u, shift right, "\mathbb F_{\bar F}"']
        \end{tikzcd}
        \]
\end{definition}
...
% \begin{align*} 
%         I_{\F/ \bar \F} &= \sets{\mathbb F_\F(\Sigma_\F(-,C)/H \cdot i)}{C \in \Sigma_{\bar \F},\ H\leq \mathsf{Aut}(C),\ i\in I}\\
%         J_{\F/ \bar \F} &= \sets{\mathbb F_\F(\Sigma_\F(-,C)/H \cdot j)}{C \in \Sigma_{\bar \F},\ H\leq \mathsf{Aut}(C),\ j\in J}.
% \end{align*} 

If $\bar \F = \F$, this is referred to as the \textit{projective} (semi) model structure on $\Op_\F(\V)$. 

Replacing (\ref{FILTRATION_LAN_LEVEL}) with (\ref{FF_LEVEL_FILTRATION}), we have that the statement analogous to Corollary \ref{CELLULAR_LEVEL_COFIB_PROP} but for free $\mathbb F_\F$-extensions remains true.
% \begin{corollary}
%         For $\V$ strongly cellular with diagonals, and for any free $\mathbb F_\F$-extension (\ref{FF_FREE_EXTENSION})
...
\qquad
\upgamma^{\**} \mathbb F_G \upgamma_{\**} \simeq \upgamma^{\**} \mathbb F_G \upgamma_!,
\]
...
\begin{proof}[proof of Theorem \ref{MAINQUILLENEQUIV THM}]
       ...
       \mathsf{Op}_{\mathcal{F}}(\mathcal{V})$
        ...
        The final part, that the adjunction
        \[
        \Op_{G / \F}(\V) \leftrightarrows \Op_\F(\V)
        \]
        is a Quillen equivalence,
        follows by building on Example \ref{TWOCOEFFS_MODEL_EX}, and combining Corollary \ref{SYM_F_Q_COR} and the above Lemma \ref{MAINLEM LEM}.
        From the cited results, it is immediate that the category of cofibrant objects in $\Sym_{G / \F}(\V)$ is precisely the essential image under $\upgamma_!$ of $\Sym_\F(\V)$, and hence of $\Sym^G_\F(\V)$. 
        We now show that the analogous result hold for operads, that the cofibrant objects in $\Op_G^\F(\V)$ are the essential image under $i_!$ of $\Op_\F(\V)$. 
        ...
        However, by Proposition \ref{F_MONAD_COMPARISON_PROP}(iii), we have
        ...
        For the counit, if $X\in \Op_G^\F(\V)$ is bifibrant then $X \simeq \upgamma_! Y$ for some cofibrant $Y \in \Op_\F(\V)$, and hence $\upgamma_!\upgamma^*$ is already derived, and so we may identify the derived counit with the counit isomorphism $\upgamma_!\upgamma^*\upgamma_! Y \to \upgamma_! Y$. 
