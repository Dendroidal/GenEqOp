...
As noted in (\ref{}), 
one possible interpretation of the discussion surrounding indexing systems from \cite{BH15}
is that these conditions are required in order to be able to.

In a parallel vein, one interpretation of the previous section, 
in particular (\ref{TWOADJOINTSOP EQ}), is that we may think of 
$\Sym^G \xrightarrow{\iota_!} \Sym_G$
as the inclusion of a full subcategory, and that
the monad $\mathbb F_G$ on $\Sym_G$ \textit{restricts} to a monad on $\Sym^G$, yielding
the inclusion of an algebraic subcategory
$\Op^G \xrightarrow{\iota_!} \Op_G$.
