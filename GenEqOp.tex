...
{\color{blue}
  Applications:
  \begin{enumerate}
  \item algebraically: if $X$ is an $\O$-algebra in $G$-spaces, then $\pi_{\**}X$ is an $\pi_{\**}\O$-algebra;
        but we have more structure: $\pi_{\**}\iota_{\**}X$ is a $\pi_{\**}\iota_{\**}\O$ algebra.
  \item any time $G$-operads with norms appear, $G$-trees appear:
  ...
  \item the analogue combinatorial notion of a strict genuine equivariant inner Kan complex in $\sdSet_G$
  ...
  \item $\Op_G(\sSet)$ are the natural source for the genuine equivariant generalization of the operadic nerve $\Op(\sSet) \to \Op_\infty$, used in {\color{red} Horev} for equivariant factorization homology
        \end{enumerate}
  \item $\Op_G(\Set)$ has a good notion of symmetric monoidal category
  \end{enumerate}

  Context:
  \begin{enumerate}
  \item $hcN \colon \sOp^G \to \dSet^G$ is a Quillen equivalence.
  \end{enumerate}

  
  Future Work:
  \begin{enumerate}
  \item formally defining $\P$-algebras
  \item formally defining many-colored variants
  \item $hcN \colon \sOp_G \to \dSet_G$ is a Quillen equivalence.
  \item more work with $\Sym_G$.
  \end{enumerate}

  Machine applications:
  \begin{enumerate}
  \item This machine/presentation of the free operad monad can be used in great generality to define algebraic notions,
  ...
  \item Similarly, we expect the bookkeeping used here will also be helpful to produce appropriately genuine equivariant models for these algebraic structures.
  \item EXAMPLE: $\Sym_G$
  \end{enumerate}
}
