...
We denote the subcategory of $G$-trees and quotients by $\Omega_{G,0}$.
\end{definition}
...
\begin{definition}
	...
	$\Sigma_G \subset \Omega_{G,0}$ of those $G$-trees that are corollas, i.e. $G$-trees such that each edge is either a root or a leaf (but not both).
\end{definition}
